...
app.get("/api/ig-account-id", requireRole("super-admin", "admin"), async (req, res) => {
  try {
    ...
    if (!pageId || !token) return res.status(400).json({ error: "pageId और token चाहिए" });
    // FB Page से linked Instagram Business Account ID लो
    const r = await fetch(`${GRAPH}/${pageId}?fields=instagram_business_account&access_token=${token}`);
    const d = await r.json();
    ...
    res.json({ found: true, igId, msg: `Instagram Business Account ID: ${igId} — यह Settings में Instagram User ID में डालें।` });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/api/deliveries", async (req, res) => {
  try { const q = {}; if (req.query.brand) q.brand = req.query.brand; if (req.query.status) q.status = req.query.status;
    const docs = await Delivery.find(q).select("-imageData").sort({ createdAt: -1 }).limit(50).lean();
    // restart-safe image URL — content की तरह (disk मिटे तो DB से serve होगा)
    docs.forEach((d) => {
      const hasImg = d.images && (d.images.square || d.images.landscape);
      if (hasImg) {
        d.imgUrl = `/api/image/delivery/${d._id}/square`;
        d.imgUrlLandscape = `/api/image/delivery/${d._id}/landscape`;
      }
    });
    res.json(docs); } catch (e) { res.status(500).json({ error: e.message }); }
});
