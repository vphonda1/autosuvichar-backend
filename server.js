...
// ⚠️ poster पर stickers/सजावट की जगह तय करने वाला — टकराव रोकता है।
//    यह file न मिले तो भी server चलता रहे — सिर्फ़ posters पुराने तरीक़े से बनें।
//    (पहले यह सीधा require था; file न होने पर पूरा server शुरू ही नहीं होता था
//     और app में "Failed to fetch" आता था। एक poster की चीज़ पूरे app को
//     नहीं गिरानी चाहिए।)
let PL = null;
try {
  PL = require("./poster-layout");
} catch (e) {
  console.error("[BOOT] poster-layout.js नहीं मिली — posters पुराने तरीक़े से बनेंगे।", e.message);
}
...
// ⚠️ आख़िर का slash हटा दो। Render में अगर PUBLIC_URL इस तरह भरा हो —
//    "https://autosuvichar-backend.onrender.com/" — तो जोड़ने पर पता
//    "...com//vehicle-photo/abc" बन जाता है। दो slash से Express उस रास्ते
//    को पहचानता ही नहीं (404), और poster में "photo नहीं खुली" दिखता है।
...
const BRANDS = {
  vp_honda: {
    ...
    financePartners: ["HDB", "IDFC First", "Shriram Finance", "Jana Small Finance", "Muthoot"],
    loanPercent: "90-95%",   // dealer-नीति — कितने % तक लोन देते हैं (गाड़ी-विशेष नहीं, सब पर लागू)
    loanTenure: "60 महीने",  // dealer-नीति — लोन की अवधि (गाड़ी-विशेष नहीं, सब पर लागू)
    minDownPayment: "5999",  // dealer-नीति — सबसे कम डाउन पेमेंट (गाड़ी में ख़ाली हो तो यही दिखेगा)
    emiStart: "1999",        // dealer-नीति — आसान किस्तें कितने से शुरू (🎁 badge में)
    minCashback: "5000",     // dealer-नीति — त्यौहार वाले poster में कैशबैक
    minExchange: "2000",     // dealer-नीति — त्यौहार वाले poster में एक्सचेंज ऑफर
  ...
  yakuza: {
    ...
    phone: "9713394738", whatsapp: "9713394738",
    place: "नियर VP Honda, परवलिया सड़क, भोपाल (म.प्र.)",
    products: ["Yakuza Pro", "Yakuza Lite", "Yakuza Max"],
    financePartners: [],   // ⚠️ इन brands पर बैंकों के नाम नहीं दिखाने
    ...
    emiStart: "1999",        // 🎁 आसान किस्तें — बाएँ badge
    loanTenure: "24 महीने",  // लोन अवधि — दाएँ badge
  },
  minimetro: {
...
async function resolveLogos(brandId) {
  ...
  //  ⚠️ पहले owner_logo.png (VP Honda) हर brand पर बाईं तरफ़ लग जाता था —
  //     Yakuza/Mini Metro के poster पर भी VP Honda दिखता था। अब वो सिर्फ़
  //     VP Honda पर लगता है; MD Automobile का logo "Logo सेटिंग" से चुनिए।
  const owner = brandId === "vp_honda"
    ? pick(cfg.ownerLogo, OWNER_LOGO_FILE, b.logo)
    : pick(cfg.ownerLogo);
...
// ⚠️ CommandCenter/तीन-विकल्प से "गणेश चतुर्थी की बधाई" कहने पर पहले कोई
//    त्यौहार पहचाना ही नहीं जाता था — festive-poster.js को हमेशा ख़ाली
//    "festival" नाम मिलता, इसलिए वो "general" (सादी) theme चुन लेता, "ganesh"
//    कभी नहीं। कमांड में जो भी त्यौहार का नाम मिले, उसे यहीं पहचान लो।
function detectFestivalName(text) {
  const t = String(text || "");
  const hit = FESTIVALS.find((f) => t.includes(f.name));
  return hit ? hit.name : "";
}
...
const Content = model("Content", new mongoose.Schema({
  ...
  // ⚠️ text हमेशा सादा रहता है — poster की तस्वीर पर यही अक्षर-दर-अक्षर
  //    छपता है। WhatsApp पर bold/italic के साथ भेजने के लिए एक अलग नक़ल —
  //    "अभी ख़ुद भेजें" दबाने पर यही जाएगी, अगर बनी हो।
  whatsappText: String,
...
const Vehicle = model("Vehicle", new mongoose.Schema({
  ...
  imageUrl: String,
  // ⚠️ photo अब database में रखते हैं, disk पर नहीं।
  ...
  //    इसलिए disk वाली photo 15 मिनट में ग़ायब हो जाती थी: database में
  //    imageUrl बचा रहता (इसीलिए "✓ सेव है" दिखता था) पर file नहीं होती,
  //    और तस्वीर की जगह काला डिब्बा दिखता था।
...
// ⚠️ MEMORY FIX — पहले यह object कभी साफ़ नहीं होता था। Render का server
//    हफ़्तों चलता है, इसलिए हर बने video की entry जमा होती रहती थी और
//    memory धीरे-धीरे भरती जाती थी। अब 1 घंटे पुरानी पूरी हो चुकी entries
//    हर 15 मिनट पर हट जाती हैं। "processing" वाली कभी नहीं छूती।
setInterval(() => {
  try {
    const cutoff = Date.now() - 60 * 60 * 1000;
    let removed = 0;
    for (const [id, j] of Object.entries(VIDEO_JOBS)) {
      if (j.status === "processing") continue;
      const t = j.finishedAt || j.startedAt;
      if (t && new Date(t).getTime() < cutoff) { delete VIDEO_JOBS[id]; removed++; }
    }
    if (removed) log("INFO", "[MEMORY] पुराने video jobs हटाए", { removed, बचे: Object.keys(VIDEO_JOBS).length });
  } catch (_) {}
}, 15 * 60 * 1000).unref?.();
...
// ⚠️ AI कभी पूरी तरह बंद न पड़े — env में जो क्रम हो वो पहले, पर उसके बाद
//    बाक़ी सारे providers भी अपने-आप जुड़ जाते हैं। पहले अगर env में अधूरी
//    सूची होती (जैसे सिर्फ़ "gemini"), तो gemini फेल होते ही सब रुक जाता,
//    बाक़ी 4 providers कभी आज़माए ही नहीं जाते। अब ऐसा कभी नहीं होगा।
const ALL_TEXT_PROVIDERS = ["gemini", "openai", "anthropic", "groq", "ollama"];
const textOrder = () => {
  const fromEnv = order("AI_TEXT_ORDER", "");
  return [...new Set([...fromEnv, ...ALL_TEXT_PROVIDERS])];
};
...
const TEXT_PROVIDERS = {
  async gemini({ parts, temperature, json, timeout, maxTokens }) {
    ...
    //  ⚠️ env में पुराना model नाम सेट हो (जैसे सिर्फ़ "gemini-1.5-flash",
    //     जो अब Google ने बंद कर दिया — 404 देता है), तो सारी AI बंद पड़
    //     जाती थी — features, caption, सब कुछ। अब env की सूची के बाद
    //     built-in नए models भी अपने-आप आज़माए जाते हैं, इसलिए env ग़लत
    //     होने पर भी app चलता रहेगा।
    //  ⚠️ Google हर कुछ महीनों में पुराने model बंद कर देता है:
    //     gemini-1.5 → बंद | gemini-2.0-flash → 1 जून 2026 को बंद |
    //     gemini-2.5-flash → 16 अक्टूबर 2026 को बंद होने वाला।
    //     इसलिए सूची में नए model पहले, और सबसे आख़िर में "-latest" वाला
    //     उपनाम रखा है — यह हमेशा Google के चालू model पर अपने-आप चला
    //     जाता है, इसलिए आगे भी AI कभी पूरी तरह बंद नहीं पड़ेगी।
    const BUILTIN_MODELS = [
      "gemini-3.7-flash", "gemini-3.5-flash", "gemini-2.5-flash",
      "gemini-flash-latest",
    ];
    const envModels = order("GEMINI_TEXT_MODELS", "");
    const models = [...new Set([...envModels, ...BUILTIN_MODELS])];
    let last = "";
  ...
  async groq({ parts, temperature, json, timeout, maxTokens }) {
    ...
    // ⚠️ Groq ने जून 2026 में llama-3.3-70b को बंद कर दिया। अब gemini की
    //    तरह यहाँ भी कई models की सूची है — एक बंद हो जाए तो अगला अपने-आप
    //    चल जाए, AI कभी पूरी तरह बंद न पड़े।
    const GROQ_MODELS = [
    ...
    for (const model of GROQ_MODELS) {
      try {
        ...
        if (!r.ok) { lastErr = `groq ${r.status}`; continue; }
        const msg = (await r.json())?.choices?.[0]?.message;
        // ⚠️ पहले यहाँ msg.reasoning भी पढ़ा जाता था — वह बहुत बड़ी ग़लती
        //    थी: reasoning में model का *सोचने* वाला text होता है, जवाब
        //    नहीं। इससे poster पर "User wants a list of 11-12 common
        //    features..." जैसी बकवास features बनकर छप गई। अब सिर्फ़ असली
        //    जवाब (content) ही लिया जाता है — ख़ाली हो तो अगला model।
        const t = (msg?.content || "").trim();
        if (t) return { ok: true, text: t, via: `groq/${model}` };
        lastErr = "empty";
      } catch (e) { lastErr = e.message; }
    }
    return { ok: false, error: lastErr || "groq fail" };
  },
...
const AI = {
  ...
  text(prompt, o = {}) {
    ...
    return AI._chain(textOrder(), TEXT_PROVIDERS,
...
function templateContent(brandId, type, festivalName) {
  ...
  const bank = {
    ...
    //  ⚠️ पहले caption बहुत सादा था। अब त्यौहार की शुभकामना के साथ असली
    //     ऑफ़र भी — ताकि Facebook/Instagram पर भी post आकर्षक लगे।
    festival: [(() => {
      const fe = FEST_BY_NAME(festivalName);
      const tag = fe ? fe.greet : "आपका हर सफर सुरक्षित और खुशहाल हो।";
      const nm = festivalName || "त्यौहार";
      const offers = [
        b.minCashback && `💰 कैशबैक ₹${b.minCashback} तक`,
        b.minDownPayment && `🏍️ सिर्फ़ ₹${b.minDownPayment} डाउन पेमेंट`,
        b.emiStart && `📅 आसान किस्तें ₹${b.emiStart} से शुरू`,
        b.loanPercent && `🏦 ${b.loanPercent} तक लोन`,
        b.loanTenure && `⏳ ${b.loanTenure} तक की अवधि`,
      ].filter(Boolean).join("\n");
      return `🎉 ${nm} की हार्दिक शुभकामनाएं! 🎉\n${tag}\n\n✨ इस ${nm} पर स्पेशल ऑफ़र ✨\n${offers}\n\n⏰ ऑफ़र सीमित समय के लिए — आज ही आइए!\n📍 ${b.name}, ${b.place}\n📞 ${b.phone}\n\n*नियम और शर्तें लागू`;
    })()],
...
async function generateText(brandId, type, festivalName, extraCtx) {
  ...
  //  ⚠️ पहले यह प्रॉम्प्ट दो ग़लतियाँ करा रहा था:
  //     1. "यह हमारा WhatsApp सुपरफ़ास्ट संदेश है" जैसे मेटा-वाक्य —
  //        कहीं मना नहीं किया गया था, इसलिए AI अपनी तरफ़ से जोड़ देता था।
  //     2. दुकान का नाम सुविचार के अंदर ही मिलाकर आता था, अलग हाइलाइट
  //        की हुई लाइन नहीं बनती थी — जबकि दुकान का प्रचार अलग दिखना
  //        चाहिए, सुविचार का हिस्सा जैसा नहीं।
  //     अब दोनों साफ़ मना किए गए हैं और दुकान वाला हिस्सा अलग सीमा-रेखा
  //     (✂️ ---) के बाद, अपने इमोजी के साथ आता है।
  //  ⚠️ पहले AI से ही दुकान का पूरा प्रचार (शोरूम/VP Honda/पता वाली 4
  //     लाइनें) भी लिखवाया जाता था, एक ✂️ रेखा से अलग करके। पर AI हमेशा
  //     ठीक वही रेखा नहीं बनाता — जब नहीं बनाता, कविता और दुकान का प्रचार
  //     एक ही पैराग्राफ़ में मिल जाते, और brand का नाम दो-तीन बार अलग-अलग
  //     जगह आ जाता (जैसा screenshot में दिखा)।
  //
  //     अब AI से सिर्फ़ greeting + कविता लिखवाते हैं — दुकान का हिस्सा
  //     बिल्कुल नहीं। दुकान की चार लाइनें (शोरूम → VP Honda → शुभकामना →
  //     पता) कोड ख़ुद, हर बार बिल्कुल एक जैसे क्रम में जोड़ता है — buildSVG
  //     और toWhatsAppMarkdown दोनों जगह — इसलिए अब कभी दोहराव या ग़लत
  //     क्रम नहीं बन सकता, चाहे AI कुछ भी लिखे।
  const suvicharPrompt = `तुम ${brandDesc(b)} (${b.place}) के WhatsApp admin हो।
एक शुभप्रभात संदेश लिखो जिसमें सिर्फ़ यह हो:
1. पहले line में: 🌹🌸🌺 + "शुभप्रभात" + emojis
2. फिर: 4-5 lines की हिंदी कविता/सुविचार (पूरी, बीच में मत काटो) — यह पूरी
   तरह सामान्य होनी चाहिए
सख़्त नियम:
- कविता पूरी होनी चाहिए, बीच में मत काटो
- दुकान, showroom, ${b.name}, पता, फ़ोन — इनमें से कुछ भी मत लिखो। यह
  सन्देश सिर्फ़ शुभप्रभात और सुविचार तक सीमित रहे, आगे कुछ नहीं जोड़ना
- "यह एक WhatsApp संदेश है", "यहाँ एक सन्देश है जिसे आप भेज सकते हैं",
  "as a WhatsApp admin", या ऐसा कोई भी परिचय/मेटा-वाक्य कभी मत लिखो —
  सीधे शुभप्रभात से शुरू करो
- कोई प्रस्तावना नहीं, कोई स्पष्टीकरण नहीं — सिर्फ़ ऊपर बताया गया सन्देश`;
...
function cleanAIText(t) {
  ...
  //  ⚠️ सुरक्षा-जाल — प्रॉम्प्ट में मना करने पर भी कभी-कभी AI अपना परिचय
  //     वाला पहला पैराग्राफ़ जोड़ देता है। पहली बार पकड़ने की कोशिश सिर्फ़
  //     "संदेश है" वाले रूप पर थी — AI ने अगली बार वही बात दूसरे शब्दों में
  //     कह दी ("...पोस्ट यहाँ दी गई है, VP Honda के सोशल मीडिया मैनेजर के
  //     रूप में:") और वो पकड़ में नहीं आई। अब कई तरह के वाक्य-ढाँचे एक साथ
  //     पहचानते हैं — "है" के बाद जो शब्द आए वो नहीं, बल्कि यह देखते हैं कि
  //     पहला पैराग्राफ़ अपने-आप को (post/message/सन्देश) के बारे में बात कर
  //     रहा है या नहीं। असली सुविचार/caption कभी अपने बारे में बात नहीं
  //     करता, इसलिए ऐसा पैराग्राफ़ मिले और छोटा हो, तो उसे पूरा हटा दो —
  //     बाक़ी असली content को हाथ मत लगाओ।
  const paras = s.split(/\n\s*\n/);
  if (paras.length > 1) {
    const first = paras[0];
    const metaSignals = [
      /(पोस्ट|संदेश|सन्देश|message)\s*(यहाँ|यहां)?\s*(है|हैं|दी गई है|दिया गया है|दे दी|दे दिया|dी hai)/i,
      /के\s*(सोशल मीडिया|व्हाट्सएप|whatsapp)?\s*(मैनेजर|एडमिन|admin)\s*के\s*रूप\s*में/i,
      /^(यहाँ|यहां|here'?s?|below is|as (a|the|your))/i,
      /आप\s+.{0,20}(भेज सकते|share कर सकते|इस्तेमाल कर सकते|post कर सकते)/i,
    ];
    const looksLikeMeta = first.length < 260 && metaSignals.some((rx) => rx.test(first));
    if (looksLikeMeta) paras.shift();
  }
  s = paras.join("\n\n");

  //  ⚠️ प्रॉम्प्ट में सीमा-रेखा "----------✍️----------" जैसी दिखनी चाहिए
  //     कहा गया था, पर AI हमेशा ठीक वैसा नहीं लिखता — कभी "^^^^^^^^^^^^",
  //     कभी "____________", कभी "शुभप्रभात ^^^^^^^^" जैसे greeting से चिपकी
  //     हुई भी मिलती है (देखा screenshot में)। इसलिए पूरी लाइन को नहीं,
  //     बल्कि 5+ बार दोहराए गए चिह्न को कहीं भी ढूँढकर अपनी अलग लाइन में
  //     एक जैसी दिखने वाली सीमा-रेखा बना देते हैं — भले वो किसी शब्द से
  //     चिपकी हो या अपने-आप अलग लाइन में हो।
  //
  //  ⚠️ पहले lookbehind/lookahead (?<!✍️) से पहले से बने अपने ही separator
  //     को बचाने की कोशिश की थी — इमोजी के साथ वह भरोसेमंद नहीं निकला
  //     (जाँचने पर ख़ुद अपने बनाए separator को दोबारा तोड़ देता था)। अब
  //     पहले से सही बने separators को एक अस्थायी टोकन से ढँक देते हैं,
  //     बाक़ी raw चिह्न normalize करते हैं, फिर टोकन वापस असली separator
  //     में बदल देते हैं — कोई पेचीदा lookaround नहीं, हर बार भरोसेमंद।
  const TOK1 = "\u0000SEP1\u0000", TOK2 = "\u0000SEP2\u0000";
  s = s.replace(/-{6,}✍️-{6,}/g, TOK1);
  s = s.replace(/✂️-{6,}/g, TOK2);
  s = s.replace(/[ \t]*([\^\-_=~])\1{4,}[ \t]*/g, "\n----------✍️----------\n");
  s = s.split(TOK1).join("----------✍️----------");
  s = s.split(TOK2).join("✂️-----------------");

  // दुकान का नाम/प्रचार वाला हिस्सा हमेशा एक ✂️ सीमा-रेखा के बाद अलग दिखे —
  // AI कभी-कभी यह रेखा जोड़ना भूल जाता और सीधे सुविचार से जोड़ देता था
  // (जैसा screenshot में दिखा — कविता ख़त्म होते ही बिना किसी अलगाव के
  // "VP Honda — VP Honda, ...") चिपक गया था)। सिर्फ़ तभी जोड़ो जब पहले से
  // कोई ✂️ सीमा-रेखा मौजूद न हो — वरना दोहरी बन जाएगी।
  if (!s.includes("✂️")) {
    s = s.replace(
      /([^\n])\n(([^\n]*(आपका अपना (शो\s?रूम|showroom)|VP Honda\s*[-—–]\s*VP Honda)[^\n]*))/i,
      "$1\n\n✂️-----------------\n$2"
    );
  }

  return s
    // ⚠️ असली bullet point में सिर्फ़ एक "-" होता है ("- item")। हमारी
    //    सजावटी सीमा-रेखा में लगातार कई "-" होते हैं ("----------") — अगले
    //    चिह्न पर negative lookahead लगाकर उसे bullet समझने से बचाया, वरना
    //    सीमा-रेखा का पहला डैश कट जाता (जो पहली बार जाँचने पर पकड़ में आया)।
    .replace(/^\s*[-–—•](?![-–—•])\s*/gm, "")   // असली bullet points ही हटें
    .replace(/\n{3,}/g, "\n\n")             // triple newlines → double
    .trim();
}

// ══════════════════════════════════════════════════════════════════════════
//  WhatsApp पर भेजने के लिए — bold/italic markdown जोड़ो
// ──────────────────────────────────────────────────────────────────────────
//  ⚠️ यह क्यों अलग function है, cleanAIText में क्यों नहीं:
//
//  doc.text एक साथ दो जगह इस्तेमाल होता है — (1) poster की तस्वीर पर लिखाई
//  के तौर पर (canvas/SVG draw करता है), और (2) "अभी ख़ुद भेजें" दबाने पर
//  WhatsApp में जाने वाले text के तौर पर (नज़दीकी: Today.jsx → nativeShare)।
//
//  अगर doc.text में ही *bold*/_italic_ जैसे निशान रख दें, तो वो निशान
//  poster की तस्वीर पर भी अक्षर-दर-अक्षर छप जाएँगे (canvas markdown नहीं
//  समझता) — टेढ़े तारे और अंडरस्कोर दिखेंगे। इसलिए doc.text हमेशा सादा रहे,
//  और WhatsApp के लिए एक अलग, सजी हुई नक़ल बनाई जाए।
//
//  ⚠️ AI से सीधे "*bold* लिखो" कहना भरोसेमंद नहीं निकला — पहले भी देखा
//  (सीमा-रेखा "----------✍️----------" की जगह AI ने "^^^^^^^^" बना दिया
//  था)। इसलिए यहाँ AI को दोबारा नहीं बुलाया — cleanAIText पहले से जो साफ़
//  ढाँचा बना चुका है (✂️ रेखा से सुविचार और दुकान का प्रचार अलग), उसी पर
//  सीधे, पक्के तरीक़े से bold/italic लगाई जाती है — कोड से, अंदाज़े से नहीं।
//
//  डिज़ाइन का फ़ैसला (WhatsApp Business के मौजूदा तरीक़ों पर आधारित):
//  - सब कुछ bold करने से असर कम होता है — इसलिए सिर्फ़ ज़रूरी हिस्से bold
//  - शुभप्रभात/नमस्ते वाली पहली लाइन — bold (ध्यान खींचने के लिए)
//  - असली सुविचार/कविता — italic (WhatsApp पर उद्धरण/भाव जताने का तरीक़ा,
//    bold से नरम, पूरा पैराग्राफ़ bold करने से कहीं बेहतर दिखता है)
//  - दुकान का नाम और ब्रांड — bold (यही असली "call-to-action" वाला हिस्सा)
//  - bold+italic साथ चाहिए तो हमेशा *_ऐसे_* — कभी *_ऐसे*_ नहीं (WhatsApp
//    का नियम है कि बंद करने वाले निशान खुलने के ठीक उल्टे क्रम में हों)
// ⚠️ दुकान की चार लाइनें — अब AI से नहीं लिखवाई जातीं, कोड ख़ुद बनाता है।
//    हमेशा यही एक क्रम — कभी दोहराव नहीं, कभी बदलाव नहीं:
//    1. आपका अपना शोरूम   2. VP Honda (असली brand नाम)
//    3. आपका दिन शुभ एवं मंगलमय हो   4. पता
// ⚠️ प्रॉम्प्ट में मना करने पर भी AI कभी-कभी अपनी पुरानी आदत से brand का
// नाम/दुकान का ज़िक्र सुविचार के भीतर ही घुसा देता है — बिना किसी ✂️ रेखा
// के, सीधे बहते हुए वाक्य में ("...VP Honda आपका दिन शुभ एवं मंगलमय हो!!
// VP Honda, नरसिंहगढ़ रोड...")। तब हमारे कोड से जुड़ी अलग, दुकान वाली
// लाइनों के साथ मिलकर brand का नाम दो-तीन बार आ जाता — जैसा screenshot में
// दिखा। सिर्फ़ prompt पर भरोसा करने की बजाय, अब brand का नाम मिलते ही वहीं
// से आगे का हिस्सा काट देते हैं — सुविचार में brand का नाम कभी नहीं आना
// चाहिए, इसलिए मिलना ही इस बात का सबूत है कि यहीं से शोरूम-प्रचार शुरू
// हो गया।
function stripBrandFromPoem(text, brandId) {
  const b = BRANDS[brandId];
  if (!b || !text) return text;
  const names = [b.name, String(b.name).replace(/\s+/g, ""),
    b.company, String(b.company || "").replace(/\s+/g, "")].filter(Boolean);
  let cut = -1;
  ...
  return String(text).slice(0, cut).replace(/[\s,;।!]+$/, "").trim();
}

function shopSignatureLines(b) {
  //  ⚠️ b.company — Yakuza/Mini Metro में "MD Automobile" आए, "Yakuza EV" नहीं।
  //     VP Honda में company और name दोनों "VP Honda" ही हैं, वहाँ कुछ नहीं बदला।
  return ["आपका अपना शोरूम", b.company || b.name, "आपका दिन शुभ एवं मंगलमय हो!!", b.place];
}

function toWhatsAppMarkdown(plainText, brandId) {
  if (!plainText) return plainText;

  // WhatsApp में bold/italic निशान का मतलब बदल न जाए, इसलिए text में अगर
  // पहले से * या _ हों (शायद ही हों, पर सुरक्षा के लिए) तो हटा दो
  let s = String(plainText).replace(/[*_]/g, "");

  //  ⚠️ पहले AI ही दुकान का हिस्सा भी ✂️ रेखा के बाद लिखता था — अब AI सिर्फ़
  //     greeting+कविता भेजता है, दुकान का हिस्सा कोड यहीं जोड़ता है। पुराने
  //     ✂️ ढाँचे वाला content अगर कभी मिल भी जाए (पुराना बना हुआ), उसे भी
  //     सही तरीक़े से संभाल लेते हैं — बस उस हिस्से को अनदेखा करके अपनी तरफ़
  //     से नया, पक्का signature जोड़ते हैं।
  const CUT = "✂️-----------------";
  const idx = s.indexOf(CUT);
  const poemPart = (idx >= 0 ? s.slice(0, idx) : s).trim();
  ...
  for (const line of poemLines) {
    ...
    // सजावटी सीमा-रेखा ("----------✍️----------") अगर पुराने content में
    // बची हो — इटैलिक में, हल्की-सी
    if (/^-{5,}.*-{5,}$/.test(t)) { out.push(`_${t}_`); continue; }
    if (!doneHeader) { out.push(`*${t}*`); doneHeader = true; continue; }  // पहली असली लाइन = greeting
    out.push(`_${t}_`);   // बाक़ी सुविचार — italic
  }
  ...
  const b = BRANDS[brandId];
  if (b) {
    const shopBold = shopSignatureLines(b).map((l) => `*${l}*`).join("\n");
    result += "\n\n" + shopBold;
  }
...
function addrBar(b, w, h) {
  ...
  return `<rect x="0" y="${y}" width="${w}" height="${barH}" fill="${DARK}"/>`
    ...
    + pin
    + `<text x="${bx + rx + 34}" y="${btnY + btnH * 0.45}" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 30)}" font-weight="800" fill="#fff">${esc(b.company || b.name)}</text>`
...
// हर item अलग रंग के 3D चिप-बटन पर — ⚠️ पहले यह हमेशा एक ही पंक्ति में
// सब कुछ छोटा करके (scale-down) ठूँस देता था — 9 features हों तो सब बहुत
// छोटे, पढ़ने में मुश्किल बन जाते (मैन्युअल editor में यही 9 features
// साफ़-सुथरे दो पंक्तियों में दिखते हैं)। अब ज़रूरत पड़ने पर अपने-आप अगली
// पंक्ति में चला जाता है — जितनी चाहिए उतनी पंक्तियाँ, हर एक ठीक साइज़ में।
// लौटाता है { svg, rows, height } — ताकि नीचे वाले elements (banks/address)
// सही जगह खिसक सकें, टकराएँ नहीं।
function chipRowWrap(labels, cy, ch, maxW, W, gap) {
  if (!labels || !labels.length) return { svg: "", rows: 0, height: 0 };
  gap = gap || ch * 0.32;
  const pal = ["#E4002B", "#1565c0", "#0ca678", "#e8590c", "#7048e8", "#d6336c", "#0a9396", "#f59f00"];
  const fs = Math.round(ch * 0.5);
  const items = labels.map((l, i) => ({ l, cw: String(l).length * fs * 0.62 + ch * 1.0, col: pal[i % pal.length] }));

  const rows = [];
  let cur = [], curW = 0;
  for (const it of items) {
    const addW = it.cw + (cur.length ? gap : 0);
    if (curW + addW > maxW && cur.length) { rows.push(cur); cur = []; curW = 0; }
    cur.push(it); curW += addW;
  }
  ...
  let svg = "";
  rows.forEach((row, ri) => {
    const rowTotal = row.reduce((s, it, i) => s + it.cw + (i ? gap : 0), 0);
    let x = (W - rowTotal) / 2;
    const y = cy + ri * rowH;
    const rx = ch / 2;
    for (const it of row) {
      svg += `<rect x="${x}" y="${y + 4}" width="${it.cw}" height="${ch}" rx="${rx}" fill="#000" opacity="0.3"/><rect x="${x}" y="${y}" width="${it.cw}" height="${ch}" rx="${rx}" fill="${it.col}"/><rect x="${x + 5}" y="${y + 3}" width="${it.cw - 10}" height="${ch * 0.32}" rx="${rx * 0.5}" fill="#fff" opacity="0.18"/><text x="${x + it.cw / 2}" y="${y + ch * 0.68}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="700" fill="#fff">${esc(it.l)}</text>`;
      x += it.cw + gap;
    }
  });
  return { svg, rows: rows.length, height: rows.length * rowH };
}
// पुराना single-row chipRow — सिर्फ़ backward-compat के लिए रखा (कोई और
// जगह इसे नहीं बुलाता, पर हटाना ग़ैर-ज़रूरी जोखिम है)
...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  // ── SAFE ZONE ──────────────────────────────────────────────────
  //   ऊपर  : "VP Honda" हेडर + greeting + कविता
  //   नीचे  : दुकान की 4 लाइनें + address bar (h*0.775 के नीचे कुछ नहीं)
  //   बाएँ/दाएँ : sticker rails            (text सिर्फ़ बीच के 68% में)
  //
  //  ⚠️ यह हिस्सा बार-बार ठीक करना पड़ा — शुरू में brand का नाम ऊपर और
  //     addrBar दोनों जगह था (दो बार)। फिर AI से ✂️ रेखा के बाद दुकान का
  //     प्रचार लिखवाना शुरू किया, तो AI कभी-कभी वो रेखा ठीक से बनाता ही
  //     नहीं था — तब कविता और दुकान का प्रचार एक ही पैराग्राफ़ में मिल
  //     जाते, brand का नाम दो-तीन जगह अलग-अलग रूप में आ जाता (जैसा
  //     screenshot में दिखा — "आपका अपना शो रूम VP Honda ... VP Honda,
  //     नरसिंहगढ़ रोड...")।
  //
  //     अब पक्का समाधान — AI से सिर्फ़ greeting+कविता लिखवाते हैं, दुकान
  //     की चार लाइनें (shopSignatureLines()) कोड ख़ुद, हर बार ठीक एक ही
  //     क्रम में जोड़ता है। AI कुछ भी लिखे, दोहराव अब बन ही नहीं सकता।
  const TOP_BRAND_H = h * 0.075;                // सबसे ऊपर "VP Honda" हेडर
  const SAFE_BOT = h * 0.775;
  //  ⚠️ सूचना वाले poster में greeting नहीं होती, पर ऊपर "आवश्यक सूचना"
  //     का बड़ा लाल बटन लगता है — इसलिए उसे भी उतनी ही जगह चाहिए।
  const HEAD_H = h * 0.075;                     // greeting-header / सूचना-बटन के लिए जगह
  const SUCHNA_PAD = type === "suchna" ? h * 0.075 : 0;   // लाल बटन की अतिरिक्त जगह
  const SAFE_TOP = TOP_BRAND_H + h * 0.015 + SUCHNA_PAD;
  const SIG_H = h * 0.185;                      // दुकान की 4 लाइनों के लिए जगह
  const SAFE_H = (SAFE_BOT - SAFE_TOP) - (type === "suchna" ? 0 : HEAD_H) - SIG_H;
  const TEXT_W = w * 0.68;

  //  ⚠️ पहले stripEmoji(text) से इमोजी हटा दी जाती थीं — यही वजह थी कि
  //     caption में इमोजी दिखतीं (वो अलग text से बनता है) पर poster की
  //     तस्वीर पर बिल्कुल नहीं। font तो अब है (fonts-noto-color-emoji),
  //     पर उस font का रंगीन हिस्सा इस renderer में नहीं दिखता (सिर्फ़
  //     सिल्हूट आकार दिखता है, टेक्स्ट के जिस रंग में लिखा हो उसी रंग
  //     में) — जाँचकर पक्का किया। पूरे पैराग्राफ़ में यह सिल्हूट भारी और
  //     भद्दी दिखती हैं, इसलिए कविता वाले हिस्से से इमोजी हटाकर सिर्फ़
  //     साफ़ शब्द रखे — greeting-header (जहाँ सिर्फ़ 3-4 इमोजी, एक लाइन
  //     में) में इमोजी ठीक-ठाक दिखती हैं, वहीं रहने दीं।
  //
  //  पुराने ✂️ ढाँचे वाला content भी (कभी बचा रह जाए) यहीं सही तरीक़े से
  //  संभल जाता है — ✂️ के बाद का हिस्सा (पुराना AI-लिखा दुकान-प्रचार)
  //  पूरी तरह अनदेखा कर देते हैं, कोड का अपना signature ही इस्तेमाल होता है।
  const CUT = "✂️-----------------";
  const cutIdx = String(text || "").indexOf(CUT);
  const poemBlock = cutIdx >= 0 ? String(text).slice(0, cutIdx) : String(text || "");

  // पहली non-separator लाइन = greeting (इमोजी सहित रहने दो, हेडर में जाएगी)
  // उसके बाद जो भी सजावटी रेखा ("----------✍️----------") मिले, हटा दो
  // बाक़ी सब = असली कविता — यहाँ से इमोजी हटाकर सिर्फ़ शब्द रखो
  //
  //  ⚠️ बड़ी गड़बड़ी पकड़ी — अगर पूरा text सिर्फ़ 1 ही लाइन का हो (जैसे जब
  //     "गुड मॉर्निंग शुभप्रभात पोस्ट" जैसा छोटा सुझाव-वाक्य सीधे content
  //     बन जाए, बिना AI से असली कविता बनवाए), तो वो पूरी लाइन "greeting"
  //     में चली जाती थी, और कविता (poemLines) पूरी तरह ख़ाली रह जाती —
  //     poster पर सिर्फ़ header/address दिखता, बीच में कुछ नहीं (जैसा
  //     screenshot में दिखा)। अब सिर्फ़ 2 या ज़्यादा असली लाइनें हों तभी
  //     पहली को अलग greeting मानते हैं — 1 लाइन हो तो वही पूरी कविता है।
  const rawLines = poemBlock.split("\n").map((l) => l.trim()).filter(Boolean);
  const nonSepLines = rawLines.filter((l) => !/^[\s]*([\^\-_=~])\1{4,}/.test(l));
  let greeting = "";
  const poemLines = [];
  //  ⚠️ सूचना में पहली लाइन को "greeting" मानकर अलग करना ग़लत है — वो
  //     शुभप्रभात जैसा अभिवादन नहीं, सूचना का ही ज़रूरी हिस्सा होती है
  //     (जैसे "आवश्यक सूचना: फ्री सर्विस कैंप...")। अलग करने से वो बड़े
  //     font में अकेली ऊपर चली जाती और कट जाती थी। अब सूचना में पूरा
  //     text एक साथ, सही से बँटकर, बीच में दिखता है।
  if (type !== "suchna" && nonSepLines.length >= 2) {
    let skippedGreeting = false;
    for (const l of rawLines) {
      if (/^[\s]*([\^\-_=~])\1{4,}/.test(l)) continue;   // सजावटी रेखा — छोड़ो
      if (!skippedGreeting) { greeting = l; skippedGreeting = true; continue; }
      poemLines.push(stripEmoji(l));                      // बाक़ी कविता — इमोजी हटाकर
    }
  } else {
    // सिर्फ़ 1 (या 0) लाइन — इसे ही पूरी कविता मानो, कुछ खोने मत दो
    poemLines.push(...nonSepLines.map((l) => stripEmoji(l)));
  }
  const clean = poemLines.join(" ").replace(/\s+/g, " ").trim();
  ...
  // कविता safe zone के बीचोंबीच (header और signature के बीच) रखो
  const poemTop = SAFE_TOP + (type === "suchna" ? 0 : HEAD_H);
  const blockH = lines.length * lineGap;
  const startY = poemTop + (SAFE_H - blockH) / 2 + fontSize * 0.8;
  ...
  // ⚠️ सबसे ऊपर — brand का नाम, एक बार, साफ़-सुथरा header
  const topBrandFont = Math.round(w / 22);
  const topBrandY = TOP_BRAND_H * 0.65;
  const topBrandHeader = `<text x="50%" y="${topBrandY}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${topBrandFont}" font-weight="800" fill="#fff">${esc(b.name)}</text>
    <rect x="${w * 0.5 - w * 0.05}" y="${TOP_BRAND_H * 0.88}" width="${w * 0.10}" height="4" rx="2" fill="${gold}"/>`;

  //  ⚠️ नया — सूचना वाले poster पर "⚠️ आवश्यक सूचना" का लाल, 3D उभरा हुआ
  //     बटन। देखते ही पता चल जाए कि यह शोरूम की तरफ़ से ज़रूरी जानकारी है।
  //     सिर्फ़ suchna type पर बनता है, बाक़ी posters वैसे ही रहते हैं।
  let suchnaBadge = "";
  if (type === "suchna") {
    const sFs = Math.round(w / 24);
    const sTxt = "⚠️ आवश्यक सूचना";
    const sbw = Math.min(w * 0.70, sTxt.length * sFs * 0.62 + w * 0.09);
    const sbh = sFs * 1.75, sbx = (w - sbw) / 2, sby = SAFE_TOP - sbh - h * 0.012;
    suchnaBadge = `<defs><linearGradient id="scg" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#ff4d5e"/><stop offset="50%" stop-color="#e4002b"/><stop offset="100%" stop-color="#9c0019"/>
      </linearGradient></defs>
      <rect x="${sbx + 4}" y="${sby + 6}" width="${sbw}" height="${sbh}" rx="${sbh * 0.3}" fill="#000" fill-opacity="0.40"/>
      <rect x="${sbx}" y="${sby}" width="${sbw}" height="${sbh}" rx="${sbh * 0.3}" fill="url(#scg)" stroke="${gold}" stroke-width="4"/>
      <rect x="${sbx + 12}" y="${sby + 5}" width="${sbw - 24}" height="${sbh * 0.30}" rx="${sbh * 0.15}" fill="#fff" fill-opacity="0.22"/>
      <text x="50%" y="${sby + sbh * 0.70}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${sFs}" font-weight="800" fill="#fff">${esc(sTxt)}</text>`;
  }

  // ⚠️ greeting — अपना header, इमोजी सहित (यहाँ 1 ही लाइन है तो सिल्हूट
  //    भद्दी नहीं दिखती, ठीक-ठाक accent जैसी लगती है)
  const headFont = Math.round(w / 20);
  const headY = SAFE_TOP + HEAD_H * 0.68;
  //  ⚠️ बड़ी गड़बड़ी पकड़ी — इसमें चौड़ाई की कोई जाँच नहीं थी! सूचना की
  //     पहली लाइन लंबी होती है (पता/फ़ोन समेत), इसलिए वो दोनों तरफ़ से
  //     कट जाती थी — screenshot में "...वार VP Honda, नरसिंहगढ़ रोड... भ"
  //     ऐसा ही दिखा। अब लंबी लाइन पर font अपने-आप छोटा होता है, और फिर
  //     भी न समाए तो दो पंक्तियों में बँट जाती है — कभी कटती नहीं।
  const greetingHeader = (() => {
    if (!greeting) return "";
    const gSafeW = w * 0.86;
    let gf = headFont;
    const gMin = Math.round(w / 34);
    while (gf > gMin && greeting.length * gf * 0.52 > gSafeW) gf -= 2;
    // सबसे छोटे font में भी न समाए — तो दो पंक्तियों में तोड़ो
    if (greeting.length * gf * 0.52 > gSafeW) {
      const gl = wrapLines(greeting, Math.max(12, Math.floor(gSafeW / (gf * 0.52))));
      return gl.slice(0, 2).map((l, i) =>
        `<text x="50%" y="${headY - gf * 0.55 + i * gf * 1.15}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${gf}" font-weight="800" fill="#fff">${esc(l)}</text>`
      ).join("");
    }
    return `<text x="50%" y="${headY}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${gf}" font-weight="800" fill="#fff">${esc(greeting)}</text>`;
  })();

  // ⚠️ दुकान की चार लाइनें — कविता के ठीक नीचे, आपके बताए क्रम में:
  //    1. आपका अपना शोरूम   2. VP Honda (सबसे बड़ा-bold, गोल्ड, तिरछा —
  //    यहीं नज़र जानी चाहिए)   3. आपका दिन शुभ एवं मंगलमय हो   4. पता
  //  ⚠️ पहले सभी 4 लाइनों को बराबर जगह दी थी — पर "VP Honda" वाली लाइन
  //     बड़े font में है (बाक़ी से लगभग दोगुनी), इसलिए बराबर जगह देने पर
  //     ऊपर-नीचे की लाइनों से टकरा जाती थी (जाँचने पर पकड़ में आया)। अब
  //     हर लाइन को उसके font-size के अनुपात में जगह मिलती है।
  const sigTop = poemTop + SAFE_H;
  const sigLineY = sigTop + SIG_H * 0.06;
  const shopLines = shopSignatureLines(b);
  const rowWeights = shopLines.map((_, i) => (i === 1 ? 1.9 : 1));   // VP Honda वाली लाइन ~दोगुनी जगह
  const totalWeight = rowWeights.reduce((a, c) => a + c, 0);
  const GAP = SIG_H * 0.025;   // ⚠️ हर लाइन के बीच थोड़ा साँस लेने की जगह — बिना इसके ऊपर-नीचे की लाइनें सट जाती थीं
  const usableH = SIG_H * 0.82 - GAP * (shopLines.length - 1);
  let cursorY = sigTop + SIG_H * 0.18;
  const brandSignature = `
    <line x1="${w * 0.30}" y1="${sigLineY}" x2="${w * 0.70}" y2="${sigLineY}" stroke="${gold}" stroke-width="2" opacity="0.55"/>
    ${shopLines.map((l, i) => {
      const isBrand = i === 1;
      const rowH = (usableH / totalWeight) * rowWeights[i];
      const y = cursorY + rowH * 0.68;
      cursorY += rowH + GAP;
      const fs = isBrand ? Math.round(w / 18) : Math.round(w / 30);
      const fill = isBrand ? gold : "#fff";
      const weight = isBrand ? 800 : 600;
      const style = isBrand ? ` font-style="italic"` : "";
      const deco = isBrand ? `🙏 ${esc(l)} 🙏` : esc(l);
      return `<text x="50%" y="${y}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${fs}" font-weight="${weight}"${style} fill="${fill}">${deco}</text>`;
    }).join("\n    ")}`;
  ...
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- ⚠️ STICKER OVERLAP FIX — पहले यहाँ तीन अलग परतें थीं (stickersSVG +
         decorSVG + autoDecorLayer), तीनों की जगहें कोड में पहले से लिखी हुई
         और किसी को दूसरे की ख़बर नहीं। इसीलिए sticker एक-दूसरे के ऊपर चढ़
         जाते थे। अब एक ही जगह से बँटता है और हर badge रखने से पहले नापा
         जाता है कि टकरा तो नहीं रहा। -->
    ${!PL ? [
    ...
    ].join("") : PL.badgeLayerSVG(PL.planBadges({
      w, h,
      stickerCsv: opts && opts.sticker,
      offerCsv: opts && opts.offer,
      decorCsv: opts && opts.decor,
      amounts: {},
      autoDecor: !!(opts && opts.autoDecor),
      autoTheme: AUTO_THEMES[autoSeed((opts && opts.autoSeed) || text) % AUTO_THEMES.length],
      offerLabel: (key) => {
        const def = OFFERS[key];
        return def ? { l1: def.l1, l2: def.l2def || "ऑफर" } : { l1: "स्पेशल", l2: "ऑफर" };
      },
    }), w, h, buildStickerSVG, buildDecorSVG)}</svg>`);
}
...
// ₹ चिन्ह हटाओ — Vehicle.exShowroom जैसे खानों में पहले से "₹56,900" भरा होता
// है, और buildHondaOfficialSVG ख़ुद ₹ जोड़ता है — दोनों मिलकर "₹₹56,900" न बने
function stripRs(s) { return String(s || "").replace(/₹/g, "").trim(); }

// विज्ञापन के लिए सही गाड़ी ढूँढो — असली photo वाली, और अगर text में किसी
// गाड़ी का नाम हो तो पहले उसे
// ⚠️ नया — "हर गाड़ी के लिए हाथ से features टाइप करना" पुराने ज़माने का
//    तरीक़ा है। अब अगर किसी गाड़ी में features कम/ख़ाली भरे हों, AI ख़ुद
//    गाड़ी के नाम से पहचान कर सामान्य, typical features बना देता है (यह
//    असली, बाज़ार में बिकने वाली गाड़ियाँ हैं — Honda SP125, Activa वग़ैरह
//    — AI को इनकी आम खूबियों की सामान्य जानकारी है)। एक बार AI बना दे,
//    उसे डेटाबेस में हमेशा के लिए सेव कर देते हैं — अगली बार दोबारा AI
//    बुलाने की ज़रूरत नहीं, न पैसा ख़र्च होता है, न समय लगता है।
//  ⚠️ AI को सख़्त हुक्म देने पर भी वो कभी-कभी दूसरी गाड़ी का नाम लिख देता
//    है ("Sp125 STD" का poster, caption में "Dio 125")। यह पहरा AI पर
//    निर्भर नहीं — caption में अगर किसी *दूसरी* गाड़ी का नाम मिले, तो उसे
//    सीधे सही गाड़ी के नाम से बदल देता है।
async function lockVehicleNameInText(text, brandId, chosenName) {
  if (!text || !chosenName) return text;
  try {
    const all = await Vehicle.find({ brand: brandId, active: true }).select("name variant").lean();
    const chosenN = String(chosenName).toLowerCase().replace(/\s+/g, "");
    let out = String(text);
    // लंबे नाम पहले बदलो, वरना छोटा नाम बड़े के अंदर से कट जाएगा
    const names = all.map((v) => [v.name, v.variant].filter(Boolean).join(" ").trim())
      .concat(all.map((v) => String(v.name || "").trim()))
      .filter((n) => n && n.length >= 3)
      .filter((n) => n.toLowerCase().replace(/\s+/g, "") !== chosenN)
      .sort((a, c) => c.length - a.length);
    for (const n of [...new Set(names)]) {
      const esc2 = n.replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/\s+/g, "\\s*");
      const re = new RegExp(esc2, "gi");
      if (re.test(out)) {
        out = out.replace(re, chosenName);
        log("WARN", "[caption] ग़लत गाड़ी का नाम बदला", { मिला: n, सही: chosenName });
      }
    }
    return out;
  } catch (e) { log("WARN", "[caption] नाम जाँच नहीं हो पाई", { msg: e.message }); return text; }
}

async function inferVehicleFeatures(brandId, vehicleName) {
  const b = BRANDS[brandId];
  const prompt = `"${vehicleName}" ${b.oem || b.name} कंपनी की एक ${b.vehicleWord || "गाड़ी"} है। ` +
    `इसकी 15-16 आम खूबियाँ (features) हिंदी में, बहुत छोटे-छोटे (2-3 शब्द के) टैग के रूप में बताओ, comma से अलग करके। ` +
    `जैसे: हाई माइलेज, LED हेडलाइट, डिस्क ब्रेक, ट्यूबलेस टायर, एलॉय व्हील, सेल्फ स्टार्ट, डिजिटल मीटर, मोबाइल चार्जिंग, USB चार्जिंग। ` +
    `सिर्फ comma-separated list दो — कोई नंबरिंग नहीं, कोई व्याख्या नहीं, कुछ और मत लिखो।`;
  try {
    const r = await AI.text(prompt, { temperature: 0.6, maxTokens: 200, timeout: 12000 });
    if (r.ok && r.text) {
      //  ⚠️ पहरा — AI कभी-कभी टैग की जगह पूरा वाक्य लौटा देता है (या अपना
      //     "सोचने" वाला text)। ऐसा कूड़ा poster पर छप गया था ("User wants
      //     a list of 11-12 common features..." वाली पट्टी)। अब हर टुकड़े
      //     को जाँचते हैं: बहुत लंबा (>22 अक्षर), 4+ शब्दों वाला, या
      //     अंग्रेज़ी वाक्य जैसा हो — तो फेंक देते हैं।
      const looksLikeTag = (s) =>
        s.length >= 2 && s.length <= 22 &&
        s.split(/\s+/).length <= 3 &&
        !/[.।:?]/.test(s) &&
        !/\b(user|wants|list|common|features|hindi|tags|words|model|answer)\b/i.test(s);
      const feats = r.text.split(",")
        .map((s) => s.trim().replace(/^[-•\d.)\]]+\s*/, "").replace(/["'`]/g, "").trim())
        .filter(looksLikeTag)
        .slice(0, 16);
      if (feats.length >= 3) return feats;
      log("WARN", "[features] AI ने टैग की जगह कूड़ा भेजा, सामान्य सूची लेंगे", { got: String(r.text).slice(0, 80) });
    }
  } catch (e) { log("WARN", "[features] AI से नहीं बन पाईं", { msg: e.message }); }
  //  ⚠️ AI कॉल कभी-कभार असफल हो सकती है (network/timeout/provider डाउन) —
  //     तब भी poster कभी "सिर्फ़ 1 feature" जैसा ख़ाली न दिखे, इसलिए एक
  //     सुरक्षित, सामान्य (हर दोपहिया में मिलने वाली) सूची fallback में।
  //     कोई specific/मनगढ़ंत आँकड़ा नहीं — सिर्फ़ सामान्य industry-features।
  log("WARN", "[features] AI उपलब्ध नहीं, सामान्य सूची इस्तेमाल हुई", { vehicleName });
  return ["हाई माइलेज", "सेल्फ स्टार्ट", "डिस्क ब्रेक", "ट्यूबलेस टायर", "डिजिटल मीटर", "LED हेडलाइट",
    "एलॉय व्हील", "मोबाइल चार्जिंग", "5-स्टेप सस्पेंशन", "स्टाइलिश ग्राफिक्स", "कम रखरखाव", "भरोसेमंद इंजन",
    "आरामदायक सीट", "मज़बूत बॉडी", "स्मूद गियर", "कम आवाज़"];
}

async function pickVehicleForAd(brandId, hint) {
  const q = { brand: brandId, active: true, inStock: true, photoData: { $nin: [null, "" ] } };
  const sel = "name variant photoData exShowroom onRoad mileage downPayment cashback features roi exchangeBonus";
  let v = null;
  if (hint) {
    //  ⚠️ दो बड़ी गड़बड़ियाँ यहाँ थीं:
    //   1) पहले सिर्फ़ hint का *पहला शब्द* ढूँढा जाता था — "Shine 100 DX"
    //      पर सिर्फ़ "Shine" खोजा जाता, पहली मिलने वाली "Shine100 STD" उठ
    //      जाती। बिल्कुल ग़लत गाड़ी का poster बनता।
    //   2) AI टूटी हो तो गाड़ी का नाम अलग से निकलता ही नहीं, और यहाँ पूरा
    //      caption-text आ जाता (जिसका पहला शब्द "आज" जैसा कुछ होता) — फिर
    //      कोई भी गाड़ी उठ जाती।
    //  अब AI पर बिल्कुल निर्भरता नहीं: database की सारी गाड़ियाँ लेकर,
    //  जिसका नाम दिए गए text में सबसे अच्छी तरह मिलता है वही चुनते हैं।
    const norm = (s) => String(s || "").toLowerCase().replace(/[\s\-_.]/g, "");
    const hintN = norm(hint);
    const all = await Vehicle.find(q).select(sel).lean();
    if (all.length) {
      const score = (c) => {
        const full = norm([c.name, c.variant].filter(Boolean).join(""));
        const nameOnly = norm(c.name);
        if (!full) return -1;
        // पूरा नाम text के अंदर मिल गया — सबसे पक्का मेल (लंबा नाम = बेहतर,
        // ताकि "Shine100" से "Shine100STD" और "Shine100DX" में सही चुने)
        if (hintN.includes(full)) return 1000 + full.length;
        if (nameOnly && hintN.includes(nameOnly)) return 500 + nameOnly.length;
        // text ही छोटा हो (सिर्फ़ गाड़ी का नाम) और वो नाम के अंदर हो
        if (hintN.length >= 4 && full.includes(hintN)) return 300 + hintN.length;
        return -1;
      };
      const scored = all.map((c) => ({ c, s: score(c) })).filter((x) => x.s > 0);
      if (scored.length) {
        // क़ीमत-भरी गाड़ियों को पहले, फिर सबसे अच्छे मेल को
        scored.sort((a, x) => (x.s - a.s) || ((x.c.exShowroom ? 1 : 0) - (a.c.exShowroom ? 1 : 0)));
        const top = scored[0].s;
        const tied = scored.filter((x) => x.s === top);
        v = (tied.find((x) => x.c.exShowroom) || tied[0]).c;
      }
    }
  }
  // नाम से कुछ ना मिला — कम से कम क़ीमत-भरी कोई गाड़ी दे दो
  if (!v) v = await Vehicle.findOne({ ...q, exShowroom: { $nin: [null, ""] } }).select(sel).sort({ updatedAt: -1 }).lean();
  if (!v) v = await Vehicle.findOne(q).select(sel).sort({ updatedAt: -1 }).lean();
  return v;
}

async function generateImages(brandId, id, text, type, opts) {
  //  ⚠️ पहले विज्ञापन को त्यौहार वाले SVG-आधारित नए system पर भेजा जाता था —
  //     वहाँ न असली फ़ीचर आते थे न सही गाड़ी, क्योंकि text (जिसमें AI कभी-कभी
  //     "Dio 125" जैसा मनगढ़ंत नाम लिख देता) से पंक्तियाँ काटकर दिखाई जाती थीं।
  //
  //     असल में एक बेहतर, पहले से बना हुआ system (buildPromoSVG +
  //     generatePromoImages) मौजूद था — वही जो मैन्युअल "विज्ञापन" editor
  //     इस्तेमाल करता है (शीर्षक, गाड़ी की असली फ़ोटो, डाउन पेमेंट/कैशबैक
  //     बॉक्स, फ़ीचर पट्टी, फ़ाइनेंस पार्टनर पट्टी) — बस उसे यहाँ जोड़ा नहीं
  //     गया था। अब विज्ञापन उसी system से बनता है, चाहे कहीं से भी बुलाया जाए।
  if (type === "vigyapan") {
    try {
      const v = opts?.vehicleDoc || await pickVehicleForAd(brandId, opts?.vehicle || text);
      if (v) {
        // ⚠️ आपने कमांड में जो राशि बताई (जैसे "₹5,000 की छूट") उसे भी
        //    देख लो — अगर Vehicle collection में price/cashback ख़ाली है
        //    (dealer ने अभी तक नहीं भरी), तो कम से कम आपकी बताई राशि दिखे,
        //    सिर्फ़ "₹" अकेला ना छपे। पहले यह पूरी तरह अनदेखा हो जाता था।
        const offerAmt = (String(opts?.offerHint || "").match(/₹\s?[\d,]+/) || [])[0] || "";
        const offerAmtNum = offerAmt.replace(/[₹\s]/g, "");

        // ⚠️ यह वही default layout (buildPromoSVG) है जो मैन्युअल "विज्ञापन"
        //    editor से बनता है — नीचे बाएँ एक्स-शोरूम कीमत, दाएँ डाउन
        //    पेमेंट/कैशबैक बटन, फ़ीचर pills, फ़ाइनेंस पार्टनर pills।
        //    "official" layout जान-बूझकर नहीं दिया — वो अलग, कम जँचा हुआ
        //    दूसरा layout है; डिफ़ॉल्ट वाला ही आपके नमूने से मेल खाता है।
        //  ⚠️ पहले हर गाड़ी में हाथ से features टाइप करने पड़ते थे — नहीं
        //     भरे हों तो poster में सिर्फ़ 1 या 0 feature दिखता (जैसा
        //     screenshot में दिखा)। अब अगर features 12 से कम हों, AI ख़ुद
        //     गाड़ी के नाम से पहचान कर बाक़ी बना देता है — dealer ने ख़ुद
        //     जो टाइप किए हैं, वो पहले रहेंगे, बाक़ी AI भरेगा। एक बार बनने
        //     के बाद डेटाबेस में हमेशा के लिए सेव — दोबारा AI नहीं बुलानी।
        let vFeatures = v.features || [];
        if (vFeatures.length < 16) {
          const inferred = await inferVehicleFeatures(brandId, [v.name, v.variant].filter(Boolean).join(" "));
          if (inferred.length) {
            const merged = [...new Set([...vFeatures, ...inferred])].slice(0, 16);
            if (merged.length > vFeatures.length) {
              vFeatures = merged;
              Vehicle.findByIdAndUpdate(v._id, { features: merged }).catch((e) =>
                log("WARN", "[features] डेटाबेस में सेव नहीं हो पाईं", { msg: e.message }));
            }
          }
        }
        // ⚠️ "रेंज की शुरुआत ₹X से" — असली database से। आपने कहा था कि
        //    एक्स-शोरूम में उसी गाड़ी का दाम रहे जिसकी फ़ोटो लगी है, और
        //    रेंज में उसी मॉडल के **STD वाले** variant का दाम। इसलिए अब
        //    पहले उसी मॉडल-परिवार में STD/बेस variant ढूँढते हैं; वो ना
        //    मिले तो परिवार का सबसे सस्ता variant लेते हैं।
        //  ⚠️ गंभीर गड़बड़ी पकड़ी — पहले variant शब्द हटाकर सिर्फ़ *पहला
        //     शब्द* ("Shine") परिवार मान लिया जाता था। इसलिए "Shine 125
        //     DLX" की रेंज में "Shine 100 STD" का दाम आ जाता — 100 और 125
        //     एक ही परिवार मान लिए गए, क़ीमत में बड़ी ग़लती। अब इंजन का
        //     अंक (100/110/125) भी परिवार की पहचान का हिस्सा है, इसलिए
        //     Shine 125 सिर्फ़ Shine 125 से ही मिलेगा।
        let rangeStart = "";
        try {
          const norm2 = (s) => String(s || "").toLowerCase().replace(/[\s\-_.]/g, "");
          const stripVar = (s) => String(s || "").replace(/\b(std|dx|dlx|ae|drum|disc|2b|standard|deluxe)\b/gi, "").trim();
          const famKey = norm2(stripVar(v.name));            // जैसे "shine125"
          const baseWord = stripVar(v.name).split(/\s+/)[0] || String(v.name || "").split(/\s+/)[0];
          if (famKey && baseWord) {
            const siblings = await Vehicle.find({
              brand: brandId, active: true, name: { $regex: baseWord, $options: "i" },
              exShowroom: { $nin: [null, ""] },
            }).select("name variant exShowroom").lean();
            // ⚠️ यहीं असली छँटाई — सिर्फ़ उसी इंजन-परिवार की गाड़ियाँ रखो
            const family = siblings.filter((s) => norm2(stripVar(s.name)) === famKey);
            const num = (s) => parseInt(String(s || "").replace(/[^\d]/g, ""), 10);
            const isStd = (s) => /\b(std|standard)\b/i.test([s.name, s.variant].filter(Boolean).join(" "));
            const stds = family.filter(isStd).map((s) => num(s.exShowroom)).filter((n) => !isNaN(n) && n > 0);
            const all = family.map((s) => num(s.exShowroom)).filter((n) => !isNaN(n) && n > 0);
            const mine = num(v.exShowroom);
            const pick = stds.length ? Math.min(...stds) : (all.length >= 2 ? Math.min(...all) : 0);
            // इसी गाड़ी का दाम ही सबसे कम हो तो "रेंज" दिखाने का मतलब नहीं
            if (pick && pick !== mine) rangeStart = pick.toLocaleString("en-IN");
          }
        } catch (e) { log("WARN", "[रेंज] नहीं निकल पाई", { msg: e.message }); }
        const o = {
          model: [v.name, v.variant].filter(Boolean).join(" "),
          //  ⚠️ क़ीमत में "(without battery)" जैसा text भी लिखा हो तो वो बॉक्स
          //     से बाहर निकलकर "एक्स-शोरूम" पर चढ़ जाता था। अब सिर्फ़ अंक बॉक्स
          //     में, और बाक़ी बात नीचे छोटे अक्षरों में।
          price: (String(v.exShowroom || "").replace(/₹/g, "").match(/\d[\d,]*/) || [""])[0],
          //  ⚠️ नया — क़ीमत में दो अंक हों ("36,500 (without battery) / 54,500")
          //     तो ऊपर पहला, नीचे दूसरा "बैटरी सहित" के साथ। और गाड़ी की
          //     किलोमीटर रेंज (database का mileage) भी अपनी लाइन में।
          price2: (() => {
            const nums = String(v.exShowroom || "").match(/\d[\d,]*/g) || [];
            return nums.length >= 2 ? nums[1] : "";
          })(),
          kmRange: String(v.mileage || "").trim().slice(0, 24),
          //  ⚠️ ऑन-रोड क़ीमत — एक्स-शोरूम बॉक्स के निचले हिस्से में "…तक"
          onRoad: (String(v.onRoad || "").replace(/₹/g, "").match(/\d[\d,]*/) || [""])[0],
          priceNote: (() => {
            const raw = String(v.exShowroom || "");
            const m = raw.match(/\(([^)]*)\)?/);
            let note = m ? m[1] : raw.replace(/₹/g, "").replace(/\d[\d,]*/, "").trim();
            if (/without|w\/o|bina|बिना/i.test(note) && /battery|बैटरी/i.test(note)) note = "बैटरी के बिना";
            else if (/with|सहित|साथ/i.test(note) && /battery|बैटरी/i.test(note)) note = "बैटरी सहित";
            return note.trim().slice(0, 30);
          })(),
          downPayment: stripRs(v.downPayment),
          rangeStart,   // "रेंज शुरू ₹X से" — एक्स-शोरूम box के निचले हिस्से में
          // ⚠️ नया — गाड़ी के नाम के ऊपर एक आकर्षक पंक्ति। त्यौहार चुना हो
          //    तो उसके नाम के साथ, वरना सादा "स्पेशल ऑफर" वाली पंक्ति।
          festivalLine: opts?.festival
            ? `${opts.festival} स्पेशल ऑफर — जल्दी कीजिए!`
            : "इससे बेहतर ऑफर कहीं नहीं — जल्दी कीजिए!",
          // ⚠️ अगर Vehicle collection में कैशबैक ख़ाली है, तो आपकी बताई
          //    राशि (offerAmtNum) यहाँ काम आती है — "छूट" शब्द को cashback
          //    ही माना, चूंकि dealer आमतौर पर इसी अर्थ में कहते हैं।
          cashback: stripRs(v.cashback) || offerAmtNum,
          // ⚠️ नया — एक्सचेंज बोनस और ब्याज दर भी अब poster पर, असली
          //    database वाले (Vehicle.exchangeBonus/roi) — मनगढ़ंत आँकड़ा
          //    नहीं, अगर dealer ने भरा हो तभी दिखेगा
          exchangeBonus: stripRs(v.exchangeBonus),
          roi: v.roi || "",
          // ⚠️ अब 12 तक — मैन्युअल जैसा "भरा-भरा" दिखाने के लिए
          features: vFeatures.slice(0, 16),
          banks: (BRANDS[brandId]?.financePartners || []).slice(0, 5),
          //  ⚠️ पहले autoDecor:true था — इससे हर बार cashback/gift जैसे
          //     "offer-badge" भी जुड़ जाते (हरा तारा "कैशबैक ₹5000", पीला
          //     गोला "फ्री गिफ्ट") — यह जानकारी पहले से डाउन-पेमेंट/कैशबैक
          //     बॉक्स में साफ़ दिखती है, दोहराव भद्दा लगता (screenshot में
          //     मैन्युअल से तुलना करने पर यही फ़र्क़ साफ़ दिखा)। अब सिर्फ़
          //     सजावटी तारे — कोई नंबर/ऑफर दोहराया नहीं जाता।
          decor: "star,star", autoDecor: false,
          bg: "light", cutout: true, autoSeed: String(id),
        };
        // sharp() photoData ka Buffer seedhe le sakta hai — disk file ki zaroorat nahi
        const photoBuf = Buffer.from(v.photoData, "base64");
        const imgs = await generatePromoImages(brandId, id, o, photoBuf);
        log("INFO", "[POSTER] विज्ञापन बना (असली गाड़ी के साथ)", { id, model: o.model });
        return imgs;
      }
      log("WARN", "[POSTER] कोई गाड़ी photo के साथ नहीं मिली — बिना फ़ोटो का विज्ञापन बनेगा");
    } catch (e) {
      log("WARN", "[POSTER] विज्ञापन नहीं बना, सादा बना रहे हैं", { msg: e.message });
    }
  }

  // ⚠️ त्यौहार/गिफ़्ट वाली post के लिए सजा हुआ poster — सादा रंग + text नहीं।
  //    इसमें मेहराब, माला, घंटी, रंगोली, और आपकी अपनी गाड़ियों की असली photo
  //    लगती है। गड़बड़ हो जाए तो नीचे वाला पुराना तरीक़ा चल पड़ेगा।
  const SAJA = ["festival", "gift"];
  //  ⚠️ बड़ा बदलाव — त्यौहार/बधाई वाला poster अब विज्ञापन जैसा पूरा, भरा-भरा
  //     बनता है: सारे features, बैंक, कैशबैक/डाउन पेमेंट/एक्सचेंज/ब्याज/लोन
  //     वाले बॉक्स, दोनों साइड badge, और नीचे पता-पट्टी। फ़र्क़ सिर्फ़ तीन:
  //     (1) गाड़ी का नाम नहीं, त्यौहार की शुभकामना — और वो बटन लाल रंग में
  //     (2) एक्स-शोरूम वाला बॉक्स नहीं (बधाई में दाम दिखाना अजीब लगता)
  //     (3) एक गाड़ी नहीं — आपकी चढ़ाई हुई "सभी गाड़ियाँ एक साथ" वाली PNG
  //     पुराना सजावटी रास्ता (festive-poster.js) तभी चलता है जब आपने वो
  //     PNG अभी तक नहीं चढ़ाई — ताकि कुछ भी टूटे नहीं।
  if (SAJA.includes(type)) {
    let fleetBuf = null;
    try {
      const st = await Setting.findOne({ brand: brandId }).lean();
      if (st?.creds?.fleetPhoto) fleetBuf = Buffer.from(st.creds.fleetPhoto, "base64");
    } catch (_) {}

    //  ⚠️ बड़ी कमी थी — "सभी गाड़ियाँ एक साथ" वाली तस्वीर न चढ़ी हो तो पुराना,
    //     ख़ाली-ख़ाली layout चल जाता था (Yakuza/Mini Metro में यही हुआ)। अब
    //     database की गाड़ियों की अपनी फ़ोटो से वही कतार अपने-आप बन जाती है —
    //     background हटाकर, एक-दूसरे से थोड़ा मिलाकर, showroom जैसी लाइन।
    if (!fleetBuf) {
      try {
        const vs = await Vehicle.find({
          brand: brandId, active: true, inStock: true, photoData: { $nin: [null, ""] },
        }).select("photoData").limit(6).lean();
        if (vs.length >= 2) {
          const cellW = 560, cellH = 420, overlap = 0.84;
          const parts = [];
          for (let i = 0; i < vs.length; i++) {
            let cut = await removeBgSmart(Buffer.from(vs[i].photoData, "base64"), cellW, cellH);
            try {
              cut = await sharp(cut).trim({ threshold: 12 })
                .resize(cellW, cellH, { fit: "inside", withoutEnlargement: false }).png().toBuffer();
            } catch (_) {}
            const m = await sharp(cut).metadata();
            const cw = m.width || cellW, chh = m.height || cellH;
            parts.push({
              input: cut,
              left: Math.round(i * cellW * overlap + (cellW - cw) / 2),
              top: Math.round(cellH - chh),      // सब ज़मीन पर खड़ी दिखें
            });
          }
          const totalW = Math.round(cellW * overlap * (vs.length - 1) + cellW);
          fleetBuf = await sharp({
            create: { width: totalW, height: cellH, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } },
          }).composite(parts).png().toBuffer();
          log("INFO", "[त्यौहार] गाड़ियों की कतार अपने-आप बनाई", { brand: brandId, kitni: vs.length });
        }
      } catch (e) { log("WARN", "[त्यौहार] कतार नहीं बन पाई", { msg: e.message }); }
    }

    if (fleetBuf) {
      try {
        //  ⚠️ opts.festival में अब पूरा वाक्य आ सकता है ("नवरात्रि की बधाई"),
        //     इसलिए पहले उसमें से असली त्यौहार का नाम निकालते हैं। नहीं
        //     मिले तो "की बधाई/शुभकामनाएं" जैसे शब्द हटाकर बचा हुआ नाम।
        const festRaw = String(opts?.festival || "").trim();
        let festName = detectFestivalName(festRaw) || detectFestivalName(text) || "";
        if (!festName && festRaw) {
          festName = festRaw.replace(/\s*(की|के)\s*(बधाई|हार्दिक\s*)?(शुभकामनाएं|शुभकामनाएँ|अवसर पर.*)?\s*$/u, "").trim();
        }
        const greet = festName ? `${festName} की हार्दिक शुभकामनाएं` : "आपको हार्दिक शुभकामनाएं";
        // features/बैंक सबके लिए — किसी एक गाड़ी की नहीं, दुकान की आम खूबियाँ
        let feats = [];
        try {
          const anyVeh = await Vehicle.findOne({ brand: brandId, active: true, features: { $exists: true, $ne: [] } })
            .select("features").lean();
          feats = (anyVeh?.features || []).slice(0, 12);
        } catch (_) {}
        //  ⚠️ पहले 10 पर रुक जाता था, इसलिए सिर्फ़ 12 features दिखते।
        //     अब 16 से कम हों तो बाक़ी अपने-आप भर जाते हैं।
        if (feats.length < 30) {
          feats = [...new Set([...feats,
            "हाई माइलेज", "सेल्फ स्टार्ट", "डिस्क ब्रेक", "ट्यूबलेस टायर", "डिजिटल मीटर", "LED हेडलाइट",
            "एलॉय व्हील", "मोबाइल चार्जिंग", "5-स्टेप सस्पेंशन", "स्टाइलिश ग्राफिक्स", "कम रखरखाव", "भरोसेमंद इंजन",
            "आरामदायक सीट", "मज़बूत बॉडी", "स्मूद गियर", "कम आवाज़",
            "लंबी वारंटी", "आसान सर्विस", "बेहतर पिकअप", "कॉम्बी ब्रेकिंग",
            "लो मेंटेनेंस", "शानदार लुक", "मज़बूत चेसिस", "आसान हैंडलिंग",
            "बड़ा फ्यूल टैंक", "चौड़े टायर", "स्पोर्टी ग्राफिक्स", "आसान स्टार्ट",
            "कम कंपन", "बेहतर माइलेज"])].slice(0, 30);
        }
        const b0 = BRANDS[brandId];
        //  ⚠️ नया — त्यौहार की तस्वीर (जो आपने "गाने और त्यौहार की तस्वीरें"
        //     सेटिंग से चढ़ाई) अब poster के background में लगती है। पहले वो
        //     सिर्फ़ पुराने सजावटी रास्ते में लगती थी, नए भरे-भरे poster में
        //     बिल्कुल नहीं आती थी — इसीलिए आपको दिखी ही नहीं।
        let festArt = "";
        try {
          const FA = mongoose.models.FestiveArt;
          if (FA) {
            const theme = festName ? (global.__FESTIVE?.themeFor?.(festName) || "") : "";
            const doc = theme
              ? await FA.findOne({ festival: theme, active: true }).sort({ createdAt: -1 }).lean()
              : null;
            if (doc?.data) festArt = doc.data;
          }
        } catch (e) { log("WARN", "[त्यौहार] तस्वीर नहीं मिली", { msg: e.message }); }
        const o = {
          festArt,                     // background में लगेगी (हो तो)
          festBg: opts?.festBg === "safed" ? "safed" : "gehra",
          model: greet,
          festivalMode: true,          // ⚠️ यही बताता है: नाम-बटन लाल हो, एक्स-शोरूम छिपे
          price: "",                   // बधाई में दाम नहीं
          //  ⚠️ पहले सिर्फ़ 2 box दिखते थे (बाक़ी ख़ाली थे)। अब चारों —
          //     dealer-नीति वाले आँकड़ों से, ताकि poster भरा-भरा लगे।
          //  ⚠️ पहले यहाँ "5000"/"2000" सीधे लिखे थे — इसलिए Yakuza/Mini
          //     Metro के poster पर भी VP Honda के आँकड़े छप जाते। अब हर brand
          //     के अपने आँकड़े ही लगते हैं; न हों तो वो box बनता ही नहीं।
          cashback: stripRs(b0.festCashback) || stripRs(b0.minCashback) || "",
          downPayment: stripRs(b0.minDownPayment) || "",
          exchangeBonus: stripRs(b0.minExchange) || "",
          roi: "",
          features: feats,
          banks: (b0.financePartners || []).slice(0, 5),
          festivalLine: festName ? `${festName} स्पेशल ऑफर — जल्दी कीजिए!` : "इससे बेहतर ऑफर कहीं नहीं — जल्दी कीजिए!",
          decor: "star,star", autoDecor: false,
          bg: "light", cutout: true, autoSeed: String(id),
        };
        const imgs = await generatePromoImages(brandId, id, o, fleetBuf);
        log("INFO", "[POSTER] त्यौहार का पूरा poster बना (सभी गाड़ियों वाली तस्वीर के साथ)", { id, festName });
        return imgs;
      } catch (e) {
        log("WARN", "[POSTER] त्यौहार का नया poster नहीं बना, पुराने तरीक़े से बनाते हैं", { msg: e.message });
      }
    }
  }
  if (SAJA.includes(type) && global.__FESTIVE) {
    try {
      //  ⚠️ असली गड़बड़ी यहीं थी — जब AI का text सिर्फ़ 2 लाइनों का होता (जो
      //     एक छोटी त्यौहार बधाई के लिए बहुत आम है), तो line1/line2 वही दो
      //     लाइनें दोबारा उठा लेते जो title/sub में पहले से इस्तेमाल हो चुकी
      //     थीं (lines[lines.length-2] और lines[lines.length-1] दोनों ही
      //     lines[0]/lines[1] बन जाते जब सिर्फ़ 2 लाइनें हों)। नतीजा — एक ही
      //     वाक्य poster पर दो जगह, दो अलग साइज़ में, एक-दूसरे पर चढ़ा हुआ।
      //
      //     अब हर लाइन सिर्फ़ एक बार इस्तेमाल होती है — जो title/sub में जा
      //     चुकी, वो line1/line2 में दोबारा नहीं आएगी।
      const lines = String(text || "").split(/\n+/).map((x) => x.trim()).filter(Boolean);
      const b = BRANDS[brandId] || {};

      const kicker = lines.length > 2 ? lines[0] : "";
      const title = opts?.festival || (lines.length > 2 ? lines[1] : lines[0]) || "शुभकामनाएं";
      const sub = lines.length > 2 ? lines[2] : "";   // ⚠️ पहले lines[1] था — sub में मत डालो, नीचे बाक़ी लाइनों में जाएगी
      const usedCount = lines.length > 2 ? 3 : 1;      // kicker+title+sub (या सिर्फ़ title) कितनी लाइनें खा गईं
      const remaining = lines.slice(usedCount);        // बाक़ी बची लाइनें — यही line1/line2 में जाएँगी, दोहराव नहीं

      const imgs = await global.__FESTIVE.buildFestivePoster(id, {
        brand: brandId,
        festival: opts?.festival || lines[0] || "",
        kicker,
        title,
        sub: sub || remaining[0] || "",   // sub ख़ाली रह जाए तो बची पहली लाइन दिखे
        line1: sub ? (remaining[0] || "") : (remaining[1] || ""),
        line2: sub ? (remaining[1] || "") : (remaining[2] || ""),
        line3: b.tagline || b.sub || "",
        tryAIBg: !!opts?.tryAIBg,   // ⚠️ "AI पृष्ठभूमि (प्रयोगात्मक)" — सिर्फ़ आपने माँगा तभी
      });
      log("INFO", "[POSTER] सजा poster बना", { id, type, theme: imgs._meta?.theme, gaadiyan: imgs._meta?.vehicles });
      return imgs;
    } catch (e) {
      log("WARN", "[POSTER] सजा poster नहीं बना, सादा बना रहे हैं", { type, msg: e.message });
    }
  }
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  //  सफ़ेद background पर ऊपर का text गहरा चाहिए, वरना दिखेगा नहीं
  if (o.festArt && o.festBg === "safed") p.textDark = "#141414";
  // ⚠️ अब 12 तक — मैन्युअल जैसा "भरा-भरा" दिखाने के लिए, दो पंक्तियों में
  //  ⚠️ कई बार एक ही feature में पूरा वाक्य लिखा होता है ("Disc break LED
  //     light smart meter") — वो एक बड़ी chip बन जाती थी। अब 3 से ज़्यादा
  //     शब्द हों तो 2-2 शब्दों में बाँट देते हैं: "Disc break", "LED light"…
  const feats = [...new Set((o.features || []).flatMap((f) => {
    const ws = String(f).trim().split(/\s+/).filter(Boolean);
    if (ws.length <= 3) return [ws.join(" ")];
    const out = []; for (let i = 0; i < ws.length; i += 2) out.push(ws.slice(i, i + 2).join(" "));
    return out;
  }))].filter(Boolean).slice(0, 30);
  // background fill (transparent पर skip — AI/photo के लिए)
  let bgRect = "";
  //  ⚠️ त्यौहार की तस्वीर background में लग रही हो तो अपना भरा हुआ रंग मत
  //     बनाओ — सिर्फ़ एक हल्का काला परदा, ताकि तस्वीर दिखे और टेक्स्ट भी
  //     साफ़ पढ़ा जाए।
  if (o.festArt) {
    //  ⚠️ त्यौहार की तस्वीर अब वॉटरमार्क जितनी हल्की — दिखती तो है, पर
      //     ऊपर लिखा हुआ कुछ भी धुँधला नहीं करती। पहले बहुत गाढ़ी थी।
      //  ⚠️ नया — दो तरह का background, आपकी पसंद से (festBg):
      //     "safed" = पुराना सफ़ेद/हल्का — वॉटरमार्क उस पर भी झलकता है
      //     "gehra" = brand-रंग वाला (डिफ़ॉल्ट)
      bgRect = (o.festBg === "safed")
        ? `<rect width="${w}" height="${h}" fill="#f5f5f5" fill-opacity="0.84"/>`
          + `<defs><linearGradient id="fsg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#fff" stop-opacity="0.50"/><stop offset="50%" stop-color="#fff" stop-opacity="0.22"/><stop offset="100%" stop-color="#dcdcdc" stop-opacity="0.55"/></linearGradient></defs>`
          + `<rect width="${w}" height="${h}" fill="url(#fsg)"/>`
        : `<rect width="${w}" height="${h}" fill="${b.accent2}" fill-opacity="0.58"/>`
          + `<defs><linearGradient id="fsg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${b.accent}" stop-opacity="0.30"/><stop offset="50%" stop-color="#000" stop-opacity="0.14"/><stop offset="100%" stop-color="#000" stop-opacity="0.38"/></linearGradient></defs>`
          + `<rect width="${w}" height="${h}" fill="url(#fsg)"/>`;
  } else if (!p.transparent) {
    if (p.kind === "grad") {
      //  ⚠️ पहले सादा ऊपर-नीचे रंग-बदलाव था — "स्पेशल" दिखाने के लिए अब एक
      //     हल्की radial चमक (ऊपर-बीच से) और बारीक़ तिरछी रेखाओं का पैटर्न
      //     जोड़ा — वही brand रंग, बस ज़्यादा गहराई और richness के साथ।
      bgRect = `<defs>
        <linearGradient id="bg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${b.accent}"/><stop offset="100%" stop-color="${b.accent2}"/></linearGradient>
        <radialGradient id="glow" cx="50%" cy="18%" r="65%"><stop offset="0%" stop-color="#fff" stop-opacity="0.16"/><stop offset="100%" stop-color="#fff" stop-opacity="0"/></radialGradient>
        <pattern id="diagLine" width="46" height="46" patternUnits="userSpaceOnUse" patternTransform="rotate(35)"><line x1="0" y1="0" x2="0" y2="46" stroke="#fff" stroke-opacity="0.045" stroke-width="10"/></pattern>
      </defs>
      <rect width="${w}" height="${h}" fill="url(#bg)"/>
      <rect width="${w}" height="${h}" fill="url(#diagLine)"/>
  ...
  //  ⚠️ बड़ा redesign — पहले सिर्फ़ 2 offer-box (डाउन पेमेंट, कैशबैक) दाईं
  //     तरफ़ थे। अब आपके बताए ढाँचे अनुसार: बाईं तरफ़ एक बड़ा एक्स-शोरूम
  //     box, दाईं तरफ़ 4 box ऊपर से नीचे — कैशबैक, डाउन पेमेंट, एक्सचेंज
  //     ऑफर, ब्याज दर (ROI) — जितने का data मौजूद हो उतने ही, बराबर
  //     बाँटकर, कोई ख़ाली जगह न बचे।
  //  ⚠️ "लिमिटेड पीरियड ऑफर" heading पिछली बार ग़लती से हट गई थी — वापस
  //     जोड़ी, offer-boxes के ठीक ऊपर।
  const OFFER_HEAD_Y = o.festivalMode ? h * 0.253 : h * 0.472;
  const OFFER_TOP = o.festivalMode ? h * 0.650 : h * 0.545;
  const OFFER_H = o.festivalMode ? h * 0.052 : h * 0.165;
  const rightBoxes = [];
  // ⚠️ हर आँकड़े के आगे * लगाया — "नियम और शर्तें लागू" (नीचे) से जुड़ा
  if (o.cashback) rightBoxes.push({ label: "कैशबैक", value: "₹" + esc(o.cashback) + "*", dark: "#141414", accent: b.accent, txt: "#fff" });
  //  ⚠️ डाउन पेमेंट — गाड़ी में भरा हो तो वही; ना भरा हो तो dealer की
  //     सबसे कम वाली नीति (BRANDS.minDownPayment)। पहले सिर्फ़ गाड़ी वाले
  //     पर निर्भर था, इसलिए ख़ाली होने पर पूरा box ही ग़ायब हो जाता था।
  const dpValue = o.downPayment || b.minDownPayment || "";
  const dpLabel = o.downPayment ? "डाउन पेमेंट" : "सबसे कम डाउन पेमेंट";
  if (dpValue) rightBoxes.push({ label: dpLabel, value: "₹" + esc(dpValue) + "*", dark: "#141414", accent: "#ffd400", txt: "#b00018" });
  if (o.exchangeBonus) rightBoxes.push({ label: "एक्सचेंज ऑफर", value: "₹" + esc(o.exchangeBonus) + "*", dark: "#141414", accent: b.accent, txt: "#fff" });
  if (o.roi) rightBoxes.push({ label: "ब्याज दर", value: esc(o.roi) + "*", dark: "#141414", accent: "#ffd400", txt: "#b00018" });
  // ⚠️ नया — "90-95% तक लोन" — यह dealer-नीति है (सभी गाड़ियों पर लागू),
  //    इसलिए BRANDS config से (गाड़ी के database से नहीं)
  if (b.loanPercent) rightBoxes.push({ label: "लोन सुविधा", value: b.loanPercent + " तक*", dark: "#141414", accent: b.accent, txt: "#fff" });
  const rbGap = h * 0.010;
  const rbH = rightBoxes.length ? (OFFER_H - (rightBoxes.length - 1) * rbGap) / rightBoxes.length : 0;
  // ⚠️ अब 4 की जगह 5 box भी हो सकते हैं (लोन सुविधा जुड़ने से) — box पतला
  //    होने पर font भी अपने-आप थोड़ा छोटा, ताकि टेक्स्ट कभी बॉक्स से बाहर
  //    न छलके, हमेशा साफ़ पढ़ने लायक़ रहे
  const rbLabelFs = rightBoxes.length >= 5 ? Math.round(w / 60) : Math.round(w / 52);
  const rbValueFs = rightBoxes.length >= 5 ? Math.round(w / 40) : Math.round(w / 34);
  const rightBoxesSVG = rightBoxes.map((rb, i) => {
    const y = OFFER_TOP + i * (rbH + rbGap);
    //  ⚠️ बधाई वाले poster में एक्स-शोरूम बॉक्स नहीं होता — तब ये बॉक्स
    //     पूरी चौड़ाई लेते हैं, ताकि बाईं तरफ़ बड़ी ख़ाली जगह न बचे
    const bx = o.festivalMode ? w * 0.05 : w * 0.52;
    const bwx = o.festivalMode ? w * 0.90 : w * 0.43;
    return btn3d(bx, y, bwx, rbH, rb.label, rb.value, rb.dark, rb.accent, rb.txt, rbLabelFs, rbValueFs);
  }).join("\n  ");
  //  ⚠️ बधाई वाले poster में ये बॉक्स पहले पूरी चौड़ाई के, बहुत मोटे लगते
  //     थे। अब दो-दो की जोड़ी में — पतले, और नीचे काफ़ी जगह बच जाती है
  //     (वहीं features की तीसरी पंक्ति आती है)।
  const rightBoxesSVG2 = !o.festivalMode ? "" : (() => {
    //  ⚠️ चारों box अब एक ही पंक्ति में — पहले 2x2 में बहुत जगह घेरते थे
    const cols = rightBoxes.length <= 4 ? rightBoxes.length : 2;
    const gapX = w * 0.012, bwx = (w * 0.94 - gapX * (cols - 1)) / cols;
    const rows2 = Math.ceil(rightBoxes.length / cols);
    const bh2 = Math.min(h * 0.052, (OFFER_H - (rows2 - 1) * rbGap) / rows2);
    return rightBoxes.map((rb, i) => {
      const cx = w * 0.03 + (i % cols) * (bwx + gapX);
      const cy = OFFER_TOP + Math.floor(i / cols) * (bh2 + rbGap);
      return btn3d(cx, cy, bwx, bh2, rb.label, rb.value, rb.dark, rb.accent, rb.txt, Math.round(w / 80), Math.round(w / 52));
    }).join("\n  ");
  })();

  //  ⚠️ नया — एक्स-शोरूम वाला बड़ा box अब दो हिस्सों में: ऊपर असली क़ीमत,
  //    नीचे "रेंज शुरू ₹X से" — बिल्कुल जैसा आपके दिखाए reference poster
  //    में "Activa Range की शुरुआत ₹77,446 से" था। यह उसी मॉडल-लाइन (जैसे
  //    "SP125" के सारे variant) में सबसे सस्ते की असली क़ीमत है — मनगढ़ंत
  //    नहीं। सिर्फ़ 1 ही variant हो तो नीचे वाला हिस्सा नहीं बनता, सिर्फ़
  //    ऊपर वाला, पूरी height में।
  const priceTxt = o.price ? "₹" + o.price + (o.priceNote ? "*" : "") : "कॉल करें";
  let priceFs = Math.round(w / 16);
  const minPriceFs = Math.round(w / 22);
  while (priceFs > minPriceFs && priceTxt.length * priceFs * 0.56 > w * 0.19) priceFs -= 2;
  //  ⚠️ बधाई वाले poster में दाम नहीं दिखाना — एक्स-शोरूम बॉक्स पूरा छिपा
  const showPriceBox = !o.festivalMode;
  const hasStartFrom = !o.festivalMode && !!o.rangeStart;
  const priceBoxH = hasStartFrom ? OFFER_H * 0.58 : OFFER_H;
  const startBoxH = hasStartFrom ? OFFER_H - priceBoxH - h * 0.010 : 0;
  //  ⚠️ नया — एक्स-शोरूम का बड़ा box अब दो हिस्सों में:
  //     ऊपर  — बैटरी के बिना वाली क़ीमत (या अकेली क़ीमत)
  //     नीचे  — बैटरी सहित वाली क़ीमत, या "रेंज शुरू", या किलोमीटर रेंज
  //     "बैटरी के बिना" अब बॉक्स के *अंदर* छोटे अक्षरों में — बाहर नहीं निकलता।
  //  नीचे वाला हिस्सा — जो भी उपलब्ध हो, इसी क्रम में:
  //  ऑन-रोड क़ीमत → बैटरी सहित क़ीमत → मॉडल-रेंज → किलोमीटर रेंज
  const lowerLabel = o.onRoad ? "ऑन-रोड तक" : (o.price2 ? "बैटरी सहित" : (hasStartFrom ? "रेंज शुरू" : (o.kmRange ? "रेंज" : "")));
  const lowerValue = o.onRoad ? "₹" + esc(o.onRoad) + "*"
    : (o.price2 ? "₹" + esc(o.price2) + "*"
    : (hasStartFrom ? "₹" + esc(o.rangeStart) + "* से" : (o.kmRange ? esc(o.kmRange) : "")));
  const hasLower = !!lowerValue;
  const priceBoxH2 = hasLower ? OFFER_H * 0.56 : OFFER_H;
  const lowerBoxH = hasLower ? OFFER_H - priceBoxH2 - h * 0.008 : 0;
  const exShowroomSVG = !showPriceBox ? "" :
    btn3d(w * 0.05, OFFER_TOP, w * 0.42, priceBoxH2, "एक्स-शोरूम", esc(priceTxt), "#141414", b.accent, "#fff", Math.round(w / 38), priceFs)
    + (o.priceNote ? `<text x="${w * 0.16}" y="${OFFER_TOP + priceBoxH2 * 0.86}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 62)}" font-weight="700" fill="#ffd400">* ${esc(o.priceNote)}</text>` : "")
    + (hasLower ? btn3d(w * 0.05, OFFER_TOP + priceBoxH2 + h * 0.008, w * 0.42, lowerBoxH, lowerLabel, lowerValue, "#141414", "#ffd400", "#b00018", Math.round(w / 50), Math.round(w / 34)) : "");
  const priceNoteSVG = "";

  //  ⚠️ features अब 3 पंक्तियों में — इसलिए थोड़ा ऊपर से शुरू (0.675 →
  //     0.645), ताकि तीसरी पंक्ति + बैंक + पता-पट्टी सब आराम से समाएँ।
  const featTop = o.festivalMode ? h * 0.710 : h * 0.722;
  //  ⚠️ अब 16 features तक — chip थोड़ा छोटा रखा ताकि सब 2 साफ़ पंक्तियों
  //     में आ जाएँ और नीचे बैंक/पता-पट्टी से कभी न टकराएँ (नाप कर तय किया)।
  //  ⚠️ 16 features को 3 साफ़ पंक्तियों में लाने के लिए चौड़ाई घटाई
  //     (0.965w → 0.72w) — इससे हर पंक्ति में 5-6 आते हैं, तीनों भरी
  //     दिखती हैं, और chip भी ठीक-ठाक बड़े रहते हैं।
  //  ⚠️ 16 features अब 2 भरी हुई पंक्तियों में, पूरी चौड़ाई (0.97w)
  //     लेकर — दोनों किनारों पर कोई ख़ाली जगह नहीं बचती।
  const featCh = o.festivalMode ? h * 0.019 : h * 0.021;
  const featGap = featCh * 0.26;
  //  त्यौहार वाले poster में 3 पंक्तियाँ (जगह है), विज्ञापन में 2 (वहाँ
  //  ऊपर गाड़ी + एक्स-शोरूम बॉक्स भी होते हैं, इसलिए कम जगह बचती है)
  const featResult = chipRowWrap(feats, featTop, featCh, o.festivalMode ? w * 0.97 : w * 0.965, w, featGap);

  //  ⚠️ बैंकों के नाम अब पूरे (सिर्फ़ "HDB" नहीं, "HDB फाइनेंस बैंक") और
  //     बड़े font में — "भारी-बड़ी लाइन" जैसा दिखे, जैसा माँगा गया था
  const fullBanks = ((o.banks && o.banks.length) ? o.banks : (b.financePartners || [])).map((n) =>
    /बैंक|finance/i.test(n) ? n : `${n} फाइनेंस बैंक`);
  //  ⚠️ विज्ञापन में यह अंतर 5.4% था — बीच में पूरी एक लाइन की ख़ाली
  //     जगह बचती और बैंक नीचे खिसककर पता-पट्टी में कट जाते थे।
  const bankTop = featTop + featResult.height + (o.festivalMode ? h * 0.004 : h * 0.008);   // ⚠️ थोड़ा और नीचे
  const bankHeadY = bankTop;
  const bankCh = o.festivalMode ? h * 0.021 : h * 0.024;
  const bankGap = bankCh * 0.28;
  const financeLineAlways = true;   // ⚠️ बैंक के नाम न हों तब भी यह लाइन दिखे
  o.financeLineAlways = financeLineAlways;
  const bankResult = fullBanks.length
    ? chipRowWrap(fullBanks, bankTop + (o.festivalMode ? h * 0.010 : h * 0.012), bankCh, w * 0.965, w, bankGap)
    : { svg: "", rows: 0, height: 0 };
  ...
  <!-- ⚠️ नया — गाड़ी के नाम के ठीक ऊपर एक आकर्षक पंत्ति (त्यौहार चुना हो
       तो उसके नाम के साथ)। logo दोनों तरफ़ जगह घेरते हैं, इसलिए यह बीच के
       ~62% में रखी है और लंबी हो तो font अपने-आप छोटा — layout नहीं बिगड़ता। -->
  ${o.festivalLine ? (() => {
    const line = String(o.festivalLine);
    const safeW = w * 0.60;
    let fs = Math.round(w / 34);
    const minFs = Math.round(w / 50);
    while (fs > minFs && line.length * fs * 0.50 > safeW) fs -= 1;
    //  ⚠️ पहले यह सिर्फ़ सादा text था — फीका लगता। अब पूरा उभरा हुआ बटन:
    //     सुनहरी पट्टी, नीचे परछाईं, ऊपर चमक — शीर्षक जैसा दिखे।
    const bw2 = Math.min(w * 0.90, line.length * fs * 0.52 + w * 0.10);
    const bh2 = fs * 1.9, bx2 = (w - bw2) / 2, by2 = h * 0.022;
    return `<defs><linearGradient id="fl" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#ffe870"/><stop offset="55%" stop-color="#ffd400"/><stop offset="100%" stop-color="#e8a900"/>
      </linearGradient></defs>
      <rect x="${bx2 + 3}" y="${by2 + 5}" width="${bw2}" height="${bh2}" rx="${bh2 / 2}" fill="#000" fill-opacity="0.30"/>
      <rect x="${bx2}" y="${by2}" width="${bw2}" height="${bh2}" rx="${bh2 / 2}" fill="url(#fl)" stroke="${b.accent}" stroke-width="3"/>
      <rect x="${bx2 + 10}" y="${by2 + 4}" width="${bw2 - 20}" height="${bh2 * 0.34}" rx="${bh2 * 0.17}" fill="#fff" fill-opacity="0.30"/>
      <text x="${w * 0.5}" y="${by2 + bh2 * 0.68}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="800" fill="#7a0016">✦ ${esc(line)} ✦</text>`;
  })() : ""}
  <!-- गाड़ी का नाम — सबसे ज़्यादा हाईलाइट, 3D उभरे हुए बटन पर -->
  <!-- ⚠️ पहले यह सिर्फ़ सादा text था, और ऊपर वाली पंक्ति से सटा हुआ भी।
       अब गहरा 3D बटन (परछाईं + चमक + सुनहरी किनारी) और ऊपर से साफ़ गैप —
       देखते ही सबसे पहले गाड़ी का नाम नज़र आए। -->
  ${(() => {
    const modelName = String(o.model || "");
    const safeW = w * 0.62;
    let fs = Math.round(w / 13);
    const minFs = Math.round(w / 22);
    while (fs > minFs && modelName.length * fs * 0.56 > safeW) fs -= 2;
    const nbw = Math.min(w * 0.86, modelName.length * fs * 0.60 + w * 0.10);
    const nbh = fs * 1.62, nbx = (w - nbw) / 2, nby = h * 0.108;
    return `<defs><linearGradient id="nmg" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="${o.festivalMode ? "#ff4d5e" : "#2b2b2b"}"/><stop offset="50%" stop-color="${o.festivalMode ? "#e4002b" : "#141414"}"/><stop offset="100%" stop-color="${o.festivalMode ? "#9c0019" : "#000"}"/>
      </linearGradient></defs>
      <rect x="${nbx + 4}" y="${nby + 7}" width="${nbw}" height="${nbh}" rx="${nbh * 0.28}" fill="#000" fill-opacity="0.38"/>
      <rect x="${nbx}" y="${nby}" width="${nbw}" height="${nbh}" rx="${nbh * 0.28}" fill="url(#nmg)" stroke="#ffd400" stroke-width="4"/>
      <rect x="${nbx + 12}" y="${nby + 5}" width="${nbw - 24}" height="${nbh * 0.30}" rx="${nbh * 0.15}" fill="#fff" fill-opacity="0.14"/>
      <text x="${w * 0.5}" y="${nby + nbh * 0.70}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="800" fill="#fff">${esc(modelName)}</text>`;
  })()}
  <!-- बाईं तरफ़ — एक बड़ा एक्स-शोरूम box, दाईं तरफ़ जितने box उतनी ही ऊँचाई का -->
  <!-- "लिमिटेड पीरियड ऑफर" — पहले हट गई थी, वापस जोड़ी -->
  ${(() => {
    // ⚠️ पहले सिर्फ़ सादा पीला text था — फीका लगता। अब लाल 3D उभरा बटन।
    const lt = "✦ लिमिटेड पीरियड ऑफर ✦";
    const lfs2 = Math.round(w / 30);
    const lbw = Math.min(w * 0.62, lt.length * lfs2 * 0.52 + w * 0.07);
    const lbh = lfs2 * 1.7, lbx = (w - lbw) / 2, lby = OFFER_HEAD_Y - lbh * 0.74;
    return `<defs><linearGradient id="lpo" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#ff4d5e"/><stop offset="50%" stop-color="#e4002b"/><stop offset="100%" stop-color="#9c0019"/>
      </linearGradient></defs>
      <rect x="${lbx + 3}" y="${lby + 5}" width="${lbw}" height="${lbh}" rx="${lbh / 2}" fill="#000" fill-opacity="0.38"/>
      <rect x="${lbx}" y="${lby}" width="${lbw}" height="${lbh}" rx="${lbh / 2}" fill="url(#lpo)" stroke="#ffd400" stroke-width="3"/>
      <rect x="${lbx + 10}" y="${lby + 4}" width="${lbw - 20}" height="${lbh * 0.30}" rx="${lbh * 0.15}" fill="#fff" fill-opacity="0.22"/>
      <text x="${w * 0.5}" y="${lby + lbh * 0.69}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${lfs2}" font-weight="800" fill="#fff">${esc(lt)}</text>`;
  })()}
  <!-- बाईं तरफ़ — एक्स-शोरूम + (हो तो) डाउन पेमेंट "शुरू ₹X से" -->
  ${exShowroomSVG}
  ${priceNoteSVG}
  <!-- दाईं तरफ़ — कैशबैक / डाउन पेमेंट / एक्सचेंज ऑफर / ब्याज दर, जितने भी मौजूद हों -->
  ${o.festivalMode ? rightBoxesSVG2 : rightBoxesSVG}
  <!-- feature चिप-बटन — 12 तक, ज़रूरत पड़ने पर अपने-आप अगली पंक्ति में -->
  ${featResult.svg}
  <!-- finance/bank चिप-बटन — पूरे नाम, बड़े font में -->
  ${(bankResult.rows || o.financeLineAlways) ? (() => {
    //  ⚠️ पहले यह सादा text था — background चाहे गहरा हो या सफ़ेद, साफ़
    //     दिखता ही नहीं था। अब पूरा उभरा हुआ बटन, हर background पर दिखेगा।
    const ft2 = "कम ब्याज पर फाइनेंस सुविधा उपलब्ध";
    const ffs = Math.round(o.festivalMode ? w / 52 : w / 40);
    const fbw2 = o.festivalMode ? w * 0.60 : Math.min(w * 0.76, ft2.length * ffs * 0.50 + w * 0.06);
    const fbh2 = ffs * 1.62, fbx2 = (w - fbw2) / 2;
    //  ⚠️ बधाई वाले poster में यह लाइन features के ऊपर चढ़ जाती थी। अब यह
    //     दोनों साइड badge (🎁 ₹1999 और 60 महीने) के *बीच* की ख़ाली जगह
    //     में बैठती है — features को पूरी जगह मिल जाती है।
    //  badges हमेशा 57.8% पर बैठते हैं (गाड़ी की ऊँचाई सीमित है), इसलिए
    //  यह बटन ठीक उनके बीच, 59% पर — पक्की जगह।
    //  ⚠️ विज्ञापन में यह लाइन सबसे नीचे, बैंकों के पास थी — दिखती ही नहीं
    //     थी। अब "लिमिटेड पीरियड ऑफर" के ठीक नीचे, जहाँ नज़र पहले जाती है।
    const fby2 = o.festivalMode ? h * 0.590 : h * 0.492;
    return `<defs><linearGradient id="finb" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#2f2f2f"/><stop offset="55%" stop-color="#141414"/><stop offset="100%" stop-color="#000"/>
      </linearGradient></defs>
      <rect x="${fbx2 + 3}" y="${fby2 + 5}" width="${fbw2}" height="${fbh2}" rx="${fbh2 / 2}" fill="#000" fill-opacity="0.34"/>
      <rect x="${fbx2}" y="${fby2}" width="${fbw2}" height="${fbh2}" rx="${fbh2 / 2}" fill="url(#finb)" stroke="#ffd400" stroke-width="3"/>
      <text x="${w * 0.5}" y="${fby2 + fbh2 * 0.69}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${ffs}" font-weight="800" fill="#ffd400">${esc(ft2)}</text>`
      + (bankResult.svg || "");
  })() : ""}
  <!-- आकर्षक address बटन -->
  ${addrBar(b, w, h)}
  <!-- ⚠️ "नियम और शर्तें लागू" — छोटे अक्षरों में, कोने में (जैसा reference
       poster में हर आँकड़े के साथ * लगा था) -->
  <text x="${w * 0.03}" y="${h * 0.84}" text-anchor="start" font-family="Mukta,sans-serif" font-size="${Math.round(w / 62)}" fill="#999">*नियम और शर्तें लागू</text>
  <text x="${w * 0.97}" y="${h * 0.84}" text-anchor="end" font-family="Mukta,sans-serif" font-size="${Math.round(w / 60)}" fill="#999">AI Generated</text>
  <!-- ⚠️ STICKER OVERLAP FIX — 1080x1080 पर नापा गया था: चुना हुआ sticker और
       ऑटो-डिज़ाइन का sticker 95px तक एक-दूसरे में घुसे हुए थे (लगभग आधा
       sticker ऊपर)। अब एक ही allocator सब बाँटता है, टकराव की जाँच के साथ। -->
  ${!PL ? offerSeal(o, w, h) : PL.badgeLayerSVG(PL.planBadges({
    w, h,
    stickerCsv: o.sticker,
    offerCsv: o.offer,
    decorCsv: o.decor,
    amounts: { cashback: o.cashback, downPayment: o.downPayment },
    autoDecor: !!o.autoDecor,
    autoTheme: AUTO_THEMES[autoSeed(o.autoSeed || o.model || "") % AUTO_THEMES.length],
    offerLabel: (key, amt) => {
      const def = OFFERS[key];
      return def
        ? { l1: def.l1, l2: def.amt && amt[def.amt] ? "₹" + amt[def.amt] : (def.l2def || "ऑफर") }
        : { l1: "स्पेशल", l2: "ऑफर" };
    },
  }), w, h, buildStickerSVG, buildDecorSVG)}
  </svg>`);
}
...
//  ⚠️ नया — रंगीन/gradient background वाली फ़ोटो के लिए। removeWhiteBg सिर्फ़
//     सफ़ेद हटाता है, इसलिए Yakuza जैसी नीली-हरी background वाली फ़ोटो पूरे
//     रंगीन डिब्बे में दिखती थीं। यह किनारों से शुरू करके, धीरे-धीरे बदलते
//     रंग (background) को हटाता है और गाड़ी के तीखे किनारे पर रुक जाता है।
//     सफ़ेद background वाली फ़ोटो पर पुराना तरीक़ा ही चलता है — कुछ नहीं बदलता।
async function removeBgSmart(srcPath, rw, rh) {
  const img = sharp(srcPath).resize(rw, rh, { fit: "inside", withoutEnlargement: false }).ensureAlpha();
  const { data, info } = await img.raw().toBuffer({ resolveWithObject: true });
  const W = info.width, H = info.height, ch = info.channels;
  const at = (x, y) => (y * W + x) * ch;
  // किनारे के pixel रंगीन हैं या सफ़ेद/पारदर्शी?
  let colored = 0, total = 0;
  for (let x = 0; x < W; x += 3) for (const y of [0, H - 1]) {
    const i = at(x, y); total++;
    if (data[i + 3] > 200 && !(data[i] > 225 && data[i + 1] > 225 && data[i + 2] > 225)) colored++;
  }
  for (let y = 0; y < H; y += 3) for (const x of [0, W - 1]) {
    const i = at(x, y); total++;
    if (data[i + 3] > 200 && !(data[i] > 225 && data[i + 1] > 225 && data[i + 2] > 225)) colored++;
  }
  if (colored / Math.max(1, total) < 0.5) return removeWhiteBg(srcPath, rw, rh);   // सफ़ेद — पुराना तरीक़ा
  // किनारों से flood-fill: पड़ोसी का रंग थोड़ा-सा ही अलग हो तो background है
  const TOL = 16;
  const seen = new Uint8Array(W * H), q = new Int32Array(W * H);
  let qh = 0, qt = 0;
  const push = (x, y) => { const k = y * W + x; if (!seen[k]) { seen[k] = 1; q[qt++] = k; } };
  for (let x = 0; x < W; x++) { push(x, 0); push(x, H - 1); }
  for (let y = 0; y < H; y++) { push(0, y); push(W - 1, y); }
  while (qh < qt) {
    const k = q[qh++], x = k % W, y = (k / W) | 0, i = k * ch;
    data[i + 3] = 0;
    for (const [nx, ny] of [[x + 1, y], [x - 1, y], [x, y + 1], [x, y - 1]]) {
      if (nx < 0 || ny < 0 || nx >= W || ny >= H) continue;
      const nk = ny * W + nx; if (seen[nk]) continue;
      const j = nk * ch;
      if (Math.abs(data[i] - data[j]) < TOL && Math.abs(data[i + 1] - data[j + 1]) < TOL && Math.abs(data[i + 2] - data[j + 2]) < TOL) {
        seen[nk] = 1; q[qt++] = nk;
      }
    }
  }
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (o.bg === "ai") {
      ...
      //  ⚠️ त्यौहार की तस्वीर सबसे नीचे, पूरे poster पर फैली — ऊपर से
      //     पारदर्शी SVG (टेक्स्ट/बॉक्स)। इससे गणेश/नवरात्रि की असली
      //     तस्वीर background में साफ़ दिखती है।
      try {
        const art = await sharp(Buffer.from(o.festArt, "base64"))
          .resize(w, h, { fit: "cover", position: "top" }).png().toBuffer();
        frame = await sharp(art).composite([{ input: overlay }]).png().toBuffer();
      } catch (e) {
        log("WARN", "[त्यौहार] background नहीं लगी", { msg: e.message });
        frame = overlay;
      }
    } else {
    ...
    let veh = null;
    try {
      //  ⚠️ अब नीचे 5 offer-box (1 बड़ा + 4 stacked) + 12 features + पूरे
      //     नाम वाले banks — बहुत ज़्यादा content है, इसलिए गाड़ी की फ़ोटो
      //     थोड़ी compact की (0.46h → 0.36h) ताकि सब कुछ बिना टकराए समाए।
      //  ⚠️ नया — चौड़ाई भी थोड़ी घटाई (0.86w → 0.64w) ताकि गाड़ी के दाईं
      //     तरफ़ पक्की, गारंटीशुदा खाली जगह बचे — वहीं "60 महीने तक लोन
      //     अवधि" वाला badge लगेगा, नीचे और जगह घेरे बिना (जैसा आपने कहा
      //     — "बाइक के लेफ्ट-राइट में एडजस्ट करो जो खाली जगह है")।
      //  ⚠️ बधाई वाले poster में "सभी गाड़ियाँ एक साथ" वाली तस्वीर पूरी
      //     चौड़ाई लेती है (0.94w) — पहले 0.64w थी, इसलिए गाड़ियाँ छोटी और
      //     पदकों के पीछे छिप जाती थीं। विज्ञापन में एक ही गाड़ी होती है,
      //     वहाँ पुरानी चौड़ाई ही ठीक है।
      const vehW = o.festivalMode ? w : Math.round(w * 0.64);   // ⚠️ पूरा किनारे-से-किनारे
      //  ⚠️ विज्ञापन में गाड़ी 46% तक जाती थी और "लिमिटेड पीरियड ऑफर" बटन
      //     (43% से) पर चढ़ जाती थी। अब 42.5% पर ख़त्म — साफ़ अलग।
      const vehH = o.festivalMode ? Math.round(h * 0.185) : Math.round(h * 0.18);
      //  ⚠️ यही असली वजह थी कि गाड़ियाँ पूरी चौड़ाई नहीं ले रही थीं —
      //     fit:"inside" दोनों (चौड़ाई और ऊँचाई) में से जो पहले भर जाए उसी
      //     से बँध जाता है। चौड़ी-पतली fleet तस्वीर में ऊँचाई पहले भर जाती,
      //     इसलिए चौड़ाई अधूरी रह जाती। बधाई वाले poster में अब सिर्फ़
      //     चौड़ाई से नापते हैं — तस्वीर हमेशा किनारे-से-किनारे भरती है।
      if (o.festivalMode) {
        //  ⚠️ गाड़ियाँ अब भी छोटी क्यों दिख रही थीं — तीन वजहें, तीनों ठीक:
        //   1) removeWhiteBg में withoutEnlargement:true था — तस्वीर छोटी हो
        //      तो बड़ी करता ही नहीं था।
        //   2) fit:"inside" ऊँचाई से बँध जाता है, चौड़ाई अधूरी छोड़ देता है।
        //   3) आपकी fleet तस्वीर के चारों तरफ़ ख़ाली/सफ़ेद किनारा होता है —
        //      उसी की वजह से असली गाड़ियाँ बीच में सिमटी रह जाती थीं।
        //   अब: पहले सफ़ेद हटाओ → फिर ख़ाली किनारा काटो (trim) → फिर पूरी
        //   चौड़ाई पर खींचो। गाड़ियाँ किनारे-से-किनारे, बड़ी और साफ़ दिखेंगी।
        let fleet = photoPath;
        if (o.cutout !== false) {
          try { fleet = await removeWhiteBg(photoPath, 2000, 2000); } catch (_) { fleet = photoPath; }
        }
        //  पूरी चौड़ाई पर ऊँचाई तस्वीर के अनुपात से तय होती है — बहुत ऊँची
        //  हो जाए तो नीचे की चीज़ों की जगह खा जाएगी, इसलिए 34% पर रोक दी
        //  ⚠️ गाड़ी की ऊँचाई — नीचे badges/box/features सबकी जगह बचाकर
        const MAXH = Math.round(h * 0.197);
        try {
          const t = await sharp(fleet).trim({ threshold: 12 }).png().toBuffer();
          const tm = await sharp(t).metadata();
          veh = (tm.width && tm.height && (tm.height / tm.width) * vehW > MAXH)
            ? await sharp(t).resize({ height: MAXH, withoutEnlargement: false }).png().toBuffer()
            : await sharp(t).resize({ width: vehW, withoutEnlargement: false }).png().toBuffer();
        } catch (_) {
          veh = await sharp(fleet).resize({ width: vehW, withoutEnlargement: false }).png().toBuffer();
        }
      } else if (o.cutout === false) {
        veh = await sharp(photoPath).resize(vehW, vehH, { fit: "inside" }).png().toBuffer();
      } else {
        //  ⚠️ रंगीन background भी हटे (Yakuza की फ़ोटो पूरे डिब्बे में दिखती
        //     थी), फिर ख़ाली किनारा काटो — गाड़ी अपने डिब्बे को पूरा भरे
        const cut = await removeBgSmart(photoPath, vehW * 2, vehH * 2);
        try {
          veh = await sharp(cut).trim({ threshold: 12 })
            .resize(vehW, vehH, { fit: "inside", withoutEnlargement: false }).png().toBuffer();
        } catch (_) {
          veh = await sharp(cut).resize(vehW, vehH, { fit: "inside" }).png().toBuffer();
        }
      }
    } catch (_) {}
    if (veh) {
      const meta = await sharp(veh).metadata();
      const vw = meta.width || vehW, vh = meta.height || vehH;
      //  ⚠️ बड़ी चूक पकड़ी — जब features/banks के लिए नीचे जगह बनाई (price
      //     box ऊपर खींचा), यह ध्यान नहीं रहा कि गाड़ी की फ़ोटो भी उसी
      //     जगह (0.17h-0.63h) थी — गाड़ी price/offer के ऊपर composite
      //     होकर उन्हें ढँक/काट रही थी ("लिमिटेड पीरियड" कटा हुआ दिखना)।
      //     अब गाड़ी को ऊपर खिसकाया (title भी compact किया) — आकार
      //     (height) वही रखा है, इसलिए गाड़ी छोटी नहीं दिखेगी, बस ऊपर।
      //  festivalMode: पदक ऊपर (0.19h-0.31h), गाड़ियाँ उनके नीचे (0.325h से)
      const top = Math.round(o.festivalMode ? h * 0.368 : h * 0.245), left = Math.round((w - vw) / 2);
      //  ⚠️ गाड़ी की असली ऊँचाई तस्वीर के अनुपात से बदलती है — इसलिए नीचे
      //     वाले badge भी उसी के हिसाब से खिसकते हैं, कभी नहीं टकराते
      if (o.festivalMode) o._vehBottom = top + vh;
      ...
      // ⚠️ गाड़ी के दोनों तरफ़ की खाली जगह में दो badge — बाएँ 🎁 (आसान
      //    किस्तें), दाएँ लोन अवधि। नीचे किसी और चीज़ की जगह नहीं ली,
      //    पूरा layout वही रहा — बस साइड की ख़ाली जगह इस्तेमाल हुई।
      const BR = BRANDS[brandId] || {};
      //  ⚠️ बधाई वाले poster में दोनों तरफ़ गोल त्यौहार-पदक लगते हैं, इसलिए
      //     ये badge और नीचे खिसकाए और पतले किए — टकराव न हो।
      //  ⚠️ यही वजह थी कि badge कभी नीचे जाते ही नहीं थे — Math.min(0.575h)
      //     उन्हें 57.5% पर रोक देता था, चाहे गाड़ी कितनी भी नीचे हो। वो
      //     सीमा हटाई — अब badge हमेशा गाड़ी के ठीक नीचे बैठते हैं।
      const badgeY = o.festivalMode
        ? Math.round(Math.max(h * 0.578, (o._vehBottom || h * 0.565) + h * 0.012))
        : top + vh * 0.24;
      if (o.festivalMode) o._badgeY = badgeY;
      const badgeH = o.festivalMode ? h * 0.070 : vh * 0.46;
      const mkBadge = (bw2, head, big, foot, isGift) => {
        //  ⚠️ पहले यह सादा काला डिब्बा था — बिल्कुल फीका लगता। अब असली
        //     बटन जैसा: ऊपर गहरा हिस्सा (शीर्षक), नीचे चमकीला सुनहरा
        //     हिस्सा (असली आँकड़ा), नीचे परछाईं और ऊपर हल्की चमक —
        //     3D उभरा हुआ दिखे, बाक़ी offer-बटनों से मेल खाए।
        //  ⚠️ "24 महीने" जैसा लंबा text बॉक्स से बाहर निकल जाता था — font
        //     अब text की लंबाई के हिसाब से अपने-आप छोटा होता है, तीनों
        //     पंक्तियों (शीर्षक, बड़ा अंक, नीचे की बात) के लिए अलग-अलग।
        const fit = (txt, start, minF) => {
          let f = start;
          while (f > minF && String(txt).length * f * 0.56 > bw2 * 0.86) f -= 1;
          return Math.round(f);
        };
        //  ⚠️ font सिर्फ़ चौड़ाई से बँधा था, ऊँचाई से नहीं — इसीलिए चौड़े
        //     badge में "24 महीने" बहुत बड़ा होकर ऊपर-नीचे की पंक्तियों पर
        //     चढ़ जाता था। अब ऊँचाई की सीमा भी लगाई।
        const fsBig = Math.min(fit(big, bw2 * 0.185, bw2 * 0.10), Math.round(badgeH * 0.30));
        const fsSml = Math.min(fit(head, bw2 * 0.115, bw2 * 0.072), fit(foot, bw2 * 0.115, bw2 * 0.072), Math.round(badgeH * 0.17));
        const topH = badgeH * 0.42;
        const gid = `bg${isGift ? "G" : "L"}`;
        return `<svg width="${Math.round(bw2)}" height="${Math.round(badgeH + 8)}" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <linearGradient id="${gid}" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stop-color="#ffe870"/><stop offset="55%" stop-color="#ffd400"/><stop offset="100%" stop-color="#e8a900"/>
            </linearGradient>
          </defs>
          <!-- परछाईं — उभरा हुआ दिखाने के लिए -->
          <rect x="4" y="8" width="${bw2 - 8}" height="${badgeH - 6}" rx="18" fill="#000" fill-opacity="0.35"/>
          <!-- ऊपरी गहरा हिस्सा -->
          <rect x="3" y="3" width="${bw2 - 6}" height="${badgeH - 6}" rx="18" fill="#141414"/>
          <!-- नीचे सुनहरा हिस्सा — असली आँकड़ा यहीं -->
          <path d="M 3 ${topH} H ${bw2 - 3} V ${badgeH - 21} A 18 18 0 0 1 ${bw2 - 21} ${badgeH - 3} H 21 A 18 18 0 0 1 3 ${badgeH - 21} Z" fill="url(#${gid})"/>
          <!-- ऊपर हल्की चमक -->
          <rect x="10" y="7" width="${bw2 - 20}" height="${topH * 0.34}" rx="10" fill="#fff" fill-opacity="0.13"/>
          <rect x="3" y="3" width="${bw2 - 6}" height="${badgeH - 6}" rx="18" fill="none" stroke="#ffd400" stroke-width="3"/>
          <text x="${bw2 / 2}" y="${topH * 0.46}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsSml}" font-weight="700" fill="#fff">${isGift ? "🎁 " : ""}${esc(head)}</text>
          <text x="${bw2 / 2}" y="${topH * 0.86}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(fsSml * 0.92)}" font-weight="600" fill="#ffd400" fill-opacity="0.85">${esc(foot)}</text>
          <text x="${bw2 / 2}" y="${topH + (badgeH - topH) * 0.60}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsBig}" font-weight="800" fill="#7a0016">${esc(big)}</text>
        </svg>`;
      };
      const sideParts = [];
      // दाएँ — लोन अवधि
      if (BR.loanTenure) {
        const bx = o.festivalMode ? (w - w * 0.03 - (w * 0.155)) : Math.min(left + vw + w * 0.02, w * 0.82);
        const bw2 = o.festivalMode ? w * 0.155 : Math.max(w * 0.12, w - bx - w * 0.03);
        if (bw2 >= w * 0.11) sideParts.push({ input: Buffer.from(mkBadge(bw2, "लोन अवधि", BR.loanTenure, "तक*", false)), top: Math.round(badgeY), left: Math.round(bx) });
      }
      // बाएँ — 🎁 आसान किस्तें
      if (BR.emiStart) {
        const bw2 = o.festivalMode ? w * 0.155 : Math.max(w * 0.12, Math.min(w * 0.18, left - w * 0.05));
        const bx = w * 0.03;
        //  ⚠️ यही वजह थी कि 🎁 वाला ₹1999 badge ग़ायब हो गया — गाड़ियाँ अब
        //     पूरी चौड़ाई लेती हैं, तो "गाड़ी से बाईं तरफ़ जगह है क्या" वाली
        //     पुरानी शर्त कभी पूरी होती ही नहीं थी। बधाई वाले poster में
        //     badge गाड़ियों के *नीचे* लगते हैं, इसलिए वो शर्त वहाँ लागू नहीं।
        if (bw2 >= w * 0.11 && (o.festivalMode || bx + bw2 < left - w * 0.005)) {
          sideParts.push({ input: Buffer.from(mkBadge(bw2, "आसान किस्तें", "₹" + BR.emiStart, "से शुरू*", true)), top: Math.round(badgeY), left: Math.round(bx) });
        }
      }
      //  ⚠️ नया — बधाई वाले poster में गाड़ियों के दोनों तरफ़ गोल, उभरी हुई
      //     त्यौहार की तस्वीर (जैसे गणेश जी) — सुनहरे छल्ले और परछाईं के
      //     साथ, ताकि पदक (medal) जैसी दिखे।
      if (o.festivalMode && o.festArt) {
        try {
          const md = Math.round(o.festivalMode ? w * 0.112 : Math.min(w * 0.215, vh * 1.0));
          const ring = Math.max(6, Math.round(md * 0.055));
          const circle = await sharp(Buffer.from(o.festArt, "base64"))
            .resize(md, md, { fit: "cover", position: "top" })
            .composite([{ input: Buffer.from(`<svg width="${md}" height="${md}"><circle cx="${md / 2}" cy="${md / 2}" r="${md / 2}" fill="#fff"/></svg>`), blend: "dest-in" }])
            .png().toBuffer();
          const framed = await sharp(Buffer.from(
            `<svg width="${md + ring * 4}" height="${md + ring * 4}" xmlns="http://www.w3.org/2000/svg">
               <defs><linearGradient id="mr" x1="0" y1="0" x2="0" y2="1">
                 <stop offset="0%" stop-color="#ffe870"/><stop offset="55%" stop-color="#ffd400"/><stop offset="100%" stop-color="#e8a900"/>
               </linearGradient></defs>
               <circle cx="${(md + ring * 4) / 2}" cy="${(md + ring * 4) / 2 + ring * 0.8}" r="${md / 2 + ring * 1.4}" fill="#000" fill-opacity="0.42"/>
               <circle cx="${(md + ring * 4) / 2}" cy="${(md + ring * 4) / 2}" r="${md / 2 + ring * 1.4}" fill="url(#mr)"/>
             </svg>`)).png().toBuffer();
          const withArt = await sharp(framed).composite([{ input: circle, top: ring * 2, left: ring * 2 }]).png().toBuffer();
          const myY = Math.round(o.festivalMode ? h * 0.198 : top - vh * 0.18);
          sideParts.push({ input: withArt, top: myY, left: Math.round(w * 0.02) });
          sideParts.push({ input: withArt, top: myY, left: Math.round(w - w * 0.02 - (md + ring * 4)) });
        } catch (e) { log("WARN", "[त्यौहार] गोल तस्वीर नहीं बनी", { msg: e.message }); }
      }
      if (sideParts.length) frame = await sharp(frame).composite(sideParts).png().toBuffer();
    }
    ...
    const f = `${id}_${k}.png`;
    let cBuf2;
    { const rawBuf2 = await sharp(frame).png().toBuffer(); const c2 = await compressImage(rawBuf2); cBuf2 = c2.buf; fs.writeFileSync(path.join(OUT_DIR, f), cBuf2); }
    out[k] = `/generated/${f}`;
    // ⚠️ पहले यह function base64 backup नहीं देता था — सिर्फ़ disk पर लिखता था।
    //    Render की free disk restart पर मिट जाती है (यह हम गाड़ियों की photo
    //    से पहले ही जान चुके हैं), तो review होने से पहले ही image टूट सकती
    //    थी। अब बाक़ी सारे generateImages की तरह base64 भी साथ जाता है।
    if (k === "square" || k === "story") b64[k] = "data:image/png;base64," + cBuf2.toString("base64");
  }
...
// ===========================================================================
// कितने दिन बाद बनी हुई photo/video अपने आप हट जाए।
// ⚠️ जो post अभी इस्तेमाल में है उसकी file कभी नहीं हटती, चाहे कितनी पुरानी हो —
//    सिर्फ़ बेकार पड़ी हुई हटती हैं। Render में KEEP_FILE_DAYS से बदल सकते हैं।
const KEEP_DAYS = parseInt(process.env.KEEP_FILE_DAYS || "15", 10);
...
सिर्फ़ valid JSON:
{ "posts": [ { "type": "suvichar|vigyapan|festival", "topic_hindi": "एक लाइन", "vehicle": "गाड़ी या खाली" } ] }

⚠️ ठीक ${wantPosters} items। कोई नया price/EMI/offer मत बनाओ।
⚠️ "suchna" (सूचना) कभी मत बनाओ — सूचना तभी जाती है जब दुकान में सचमुच
   कुछ बदले (छुट्टी, समय, सर्विस कैंप), वो मालिक ख़ुद बनाते हैं। रोज़ की
   पोस्ट सिर्फ़ suvichar, vigyapan और festival की हों।`,
...
- ⚠️ मूल content में जिस गाड़ी/मॉडल का नाम है, हर platform के version में
  बिल्कुल वही नाम रहे — अक्षर-दर-अक्षर वही। कोई दूसरा मॉडल नाम मत लिखो,
  मत बदलो, मत सुधारो — भले वो नाम अनजाना या अधूरा लगे। एक ही post में
  एक जगह एक गाड़ी और दूसरी जगह दूसरी गाड़ी का नाम आना सबसे बड़ी ग़लती है।
...
// ⚠️ पहले यहाँ कोई cache header नहीं था — इसलिए हर poster की तस्वीर हर बार
//    Render से दोबारा माँगी जाती थी. 20 poster वाला पन्ना = 20 request.
//    अब 1 घंटा cache. (1 दिन नहीं, क्योंकि दोबारा बनाने पर file वही नाम रखती है.)
const genOpts = { maxAge: "1h", setHeaders: (res) => { res.set("Access-Control-Allow-Origin", "*"); res.set("Cross-Origin-Resource-Policy", "cross-origin"); } };
app.use("/generated", express.static(OUT_DIR, genOpts));
...
async function fetchGeminiImage(prompt, w, h) {
  ...
  // ⚠️ पहले सिर्फ़ पुराने model माँगे जाते थे — "gemini-2.5-flash-image" और
  //    दो preview वाले। वे बदल/बंद हो चुके हैं, इसलिए key सही होने पर भी
  //    404 मिलता था और हर बार मुफ़्त वाले (pollinations) पर चला जाता था।
  //    अब नए model पहले, पुराने बाद में — कोई भी चल जाए तो काम बन जाए।
  const models = [...new Set([
    ...discovered,                      // जो Google ख़ुद बताए, वो सबसे पहले
    "gemini-3.1-flash-image",           // Nano Banana 2 — सस्ता, तेज़
    "gemini-3.1-flash-lite-image",      // सबसे सस्ता
    "gemini-3-pro-image",               // Nano Banana Pro — महँगा, सबसे अच्छा
    "gemini-2.5-flash-image",           // पुराना — अभी भी चल सकता है
...
app.post("/api/ai-bg", async (req, res) => {
  try {
    ...
    // ⚠️ मुफ़्त वाले से बनी तो भी बता दो कि Gemini क्यों नहीं चला — वरना
    //    पता ही नहीं चलता कि key काम कर रही है या नहीं।
    if (pol) return res.json({
      dataUrl: "data:image/png;base64," + pol.toString("base64"),
      source: "pollinations",
      note: g.error || "",
      geminiFailed: g.error === "no-key"
        ? "Render में GEMINI_API_KEY नहीं मिली"
        : /429/.test(g.error || "") ? "Google का मुफ़्त कोटा शून्य — billing चालू करनी होगी"
        : /404/.test(g.error || "") ? "Gemini का model नहीं मिला (नाम बदल गया होगा)"
        : /400|403/.test(g.error || "") ? "key ग़लत है या उस पर image बनाने की इजाज़त नहीं"
        : (g.error || "Gemini ने जवाब नहीं दिया"),
    });
...
app.post("/api/ai-text", async (req, res) => {
  try {
    ...
    let text = await generateText(brand, finalType, fe ? fe.name : undefined);
    if (finalType === "suvichar") text = stripBrandFromPoem(text, brand);
    res.json({ text });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
// ---- Content ----
...
// ══════════════════════════════════════════════════════════════
//  ⚠️ असली गड़बड़ी यहीं थी — parseCommandIntent को सिर्फ़ 5 प्रकार पता थे:
//     suvichar/vigyapan/festival/suchna/gift। "हायरिंग का विज्ञापन बनाओ" जैसा
//     कहने पर AI को इन 5 में से एक चुनना ही था — "विज्ञापन" शब्द पकड़कर
//     type="vigyapan" बना दिया, नतीजा: भर्ती माँगने पर मोटरसाइकिल का
//     ₹94,809 वाला ऑफ़र-poster बन गया। यही "booking" के लिए भी हुआ।
//
//  ✅ अब commandText में इन 9 दूसरे साँचों (जो अपने ख़ास editor में बनते हैं,
//     AI के सीधे text-to-poster वाले रास्ते से नहीं) के शब्द मिलें तो AI को
//     बुलाया ही नहीं जाता — साफ़ बता दिया जाता है कि यह कहाँ बनेगा।
const NON_AUTO_CAPS = {
  hiring:    { label: "भर्ती",              words: ["भर्ती", "भरती", "नौकरी", "hiring", "हायरिंग", "vacancy", "स्टाफ चाहिए", "स्टाफ़ चाहिए", "काम करने वाला", "job"] },
  booking:   { label: "बुकिंग के फ़ायदे",     words: ["बुकिंग", "बुक ", "booking", "एडवांस", "advance", "प्री बुक", "pre book"] },
  mega:      { label: "Mega Offer",          words: ["मेगा", "महाबचत", "महा ऑफर", "mega", "बम्पर", "बंपर", "blockbuster"] },
  compare:   { label: "तुलना वाला poster",    words: ["तुलना", "तुलनात्मक", "मुकाबला", "मुक़ाबला", "compare", "comparison", "बनाम", "कौन सस्ता", "सर्विस चार्ज"] },
  luckydraw: { label: "Lucky Draw",           words: ["लकी ड्रॉ", "लक्की ड्रॉ", "lucky draw", "इनाम", "कूपन", "कुपन"] },
  multibike: { label: "कई गाड़ियाँ साथ",       words: ["कई गाड़", "सभी गाड़", "सारी गाड़", "पूरी रेंज", "multibike", "multi bike"] },
  delivery:  { label: "Delivery post",        words: ["डिलीवरी", "डिलिवरी", "delivery", "चाबी सौंप", "गाड़ी सौंपी", "ग्राहक की फोटो", "ग्राहक की photo"] },
  video:     { label: "Video",                words: ["वीडियो बनाओ", "विडियो बनाओ", "रील बनाओ", "reel बनाओ", "slideshow"] },
  announce:  { label: "अनाउंसमेंट",           words: ["अनाउंस", "अनाउंसमेंट", "announce", "माइक के लिए", "भोंपू", "बोलकर सुनाओ"] },
};

function detectNonAutoCap(commandText) {
  const t = String(commandText || "").toLowerCase();
  for (const [id, cap] of Object.entries(NON_AUTO_CAPS)) {
    if (cap.words.some((w) => t.includes(w.toLowerCase()))) return { id, label: cap.label };
  }
  return null;
}

async function parseCommandIntent(commandText, defaultBrand) {
  // ⚠️ AI बुलाने से पहले ही जाँच लो — "हायरिंग का विज्ञापन" में "विज्ञापन"
  //    शब्द है, पर असली मक़सद भर्ती है। AI तक पहुँचने ही नहीं देते।
  const nonAuto = detectNonAutoCap(commandText);
  if (nonAuto) {
    return {
      brand: defaultBrand, type: null, notAutoType: true,
      suggestedCapability: nonAuto.id,
      suggestedLabel: nonAuto.label,   // हिंदी नाम — सन्देश में यही दिखेगा, id नहीं
      summary_hindi: `यह "${nonAuto.label}" वाला है — अपने ख़ास पन्ने में बनेगा, यहाँ नहीं।`,
      confidence: "high",
    };
  }
...
app.get("/api/vehicles", async (req, res) => {
  try {
    ...
    // ⚠️ सूची में photoData कभी न भेजें — 12 गाड़ियों की base64 मिलकर कई MB
    //    हो जाती है और mobile पर सूची खुलनी ही बंद हो जाती है
    const docs = await Vehicle.find(q).select("-photoData").sort({ brand: 1, name: 1 }).lean();
    res.json(docs);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// नई गाड़ी — photo बाद में /photo वाले रास्ते से चढ़ती है, यहाँ से नहीं
...
app.patch("/api/vehicles/:id", requireRole("super-admin", "admin"), async (req, res) => {
  try {
    // ⚠️ photo के ख़ाने यहाँ से कभी न बदलें।
    //    form में गाड़ी का पुराना imageUrl भरा होता है। बीच में नई photo चढ़ाई
    //    और फिर "अपडेट करें" दबाया, तो form का पुराना पता नई photo के पते के
    //    ऊपर चढ़ जाता था — नई तस्वीर चढ़कर भी दिखती नहीं थी।
    //    photo सिर्फ़ /photo वाले रास्तों से बदलेगी, यहाँ से नहीं।
    const patch = { ...req.body };
    delete patch.photoData; delete patch.photoMime;
    delete patch.photoAt;   delete patch.imageUrl;
    delete patch._id;       delete patch.__v;

    const doc = await Vehicle.findByIdAndUpdate(req.params.id, patch, { new: true }).select("-photoData");
...
app.delete("/api/vehicles/:id", requireRole("super-admin", "admin"), async (req, res) => {
  try {
    ...
    // गाड़ी हटी तो उसकी photo भी हटा दो — वरना disk में कचरा जमा होता रहेगा
    if (doc && doc.imageUrl) { try { fs.unlinkSync(path.join(__dirname, "public", doc.imageUrl.replace(/^\//, ""))); } catch (_) {} }
    res.json({ ok: true });
  }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ══════════════════════════════════════════════════════════════════════════
//  गाड़ी की अपनी photo — एक बार डालो, हमेशा के लिए उसी गाड़ी से जुड़ी रहे
//  ------------------------------------------------------------------------
//  पहले दो अलग चीज़ें थीं जो आपस में जुड़ी ही नहीं थीं:
//     • गाड़ी का record  (नाम, क़ीमत, EMI…) — `imageUrl` खाली पड़ा रहता था
//     • photo library    (/api/vehicle-photos) — ढीली photos, किसी गाड़ी से नाता नहीं
//  इसलिए poster बनाते समय गाड़ी चुनने पर सिर्फ़ नाम आता था, photo हाथ से
//  ढूँढकर डालनी पड़ती थी। अब photo गाड़ी के record में ही बैठती है।
// ══════════════════════════════════════════════════════════════════════════
// ⚠️ memory में लो, disk पर नहीं — Render की free disk मिट जाती है
const vehPhotoUpload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 12 * 1024 * 1024 },
  fileFilter: (req, f, cb) => cb(null, /^image\//.test(f.mimetype)),
});

// गाड़ी का brand पहले पता करो — तभी सही फ़ोल्डर में जाएगी
async function attachVehBrand(req, res, next) {
  try {
    const v = await Vehicle.findById(req.params.id).lean();
    if (!v) return res.status(404).json({ error: "गाड़ी नहीं मिली" });
    req.vehBrand = v.brand; req.vehOld = v.imageUrl;
    next();
  } catch (e) { res.status(500).json({ error: e.message }); }
}

// ⚠️ नया — "सभी गाड़ियाँ एक साथ" वाली एक PNG। त्यौहार/बधाई वाले poster में
//    अलग-अलग गाड़ियों की कतार (जो अक्सर बेमेल लगती थी) की जगह यही एक
//    साफ़, तैयार तस्वीर लगती है। Settings में brand के साथ सहेजी जाती है।
app.post("/api/brand-fleet-photo", requireRole("super-admin", "admin"),
  vehPhotoUpload.single("photo"), async (req, res) => {
    try {
      const brand = req.body.brand || req.query.brand;
      if (!BRANDS[brand]) return res.status(400).json({ error: "brand ग़लत है" });
      if (!req.file) return res.status(400).json({ error: "तस्वीर नहीं मिली (12MB तक)" });
      let buf = req.file.buffer;
      try {
        // PNG ही रखते हैं — पारदर्शी background बचा रहे (यही इसकी ख़ूबी है)
        buf = await sharp(buf).resize(1600, 900, { fit: "inside", withoutEnlargement: true }).png().toBuffer();
      } catch (e) { log("WARN", "[FLEET] तस्वीर छोटी नहीं हुई", { msg: e.message }); }
      if (buf.length > 5 * 1024 * 1024) return res.status(400).json({ error: "तस्वीर बहुत भारी है — छोटी करके डालें" });
      await Setting.findOneAndUpdate({ brand },
        { $set: { "creds.fleetPhoto": buf.toString("base64") } }, { upsert: true });
      log("INFO", "[FLEET] सभी गाड़ियों वाली तस्वीर सहेजी गई", { brand, kb: Math.round(buf.length / 1024) });
      res.json({ ok: true, kb: Math.round(buf.length / 1024) });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

app.get("/api/brand-fleet-photo", async (req, res) => {
  try {
    const brand = req.query.brand;
    const s = await Setting.findOne({ brand }).lean();
    res.json({ has: !!(s?.creds?.fleetPhoto) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post("/api/vehicles/:id/photo", requireRole("super-admin", "admin"),
  attachVehBrand, vehPhotoUpload.single("photo"), async (req, res) => {
    try {
      if (!req.file) return res.status(400).json({ error: "photo नहीं मिली (सिर्फ़ तस्वीर चलेगी, 12MB तक)" });

      // छोटी और हल्की कर दो — 1200px काफ़ी है, database में जगह भी कम लेगी
      let buf = req.file.buffer;
      try {
        buf = await sharp(buf)
        ...
        log("WARN", "[VEHICLE] photo छोटी नहीं हुई, जैसी है वैसी रखी", { msg: e.message });
      }

      const kb = Math.round(buf.length / 1024);
      if (buf.length > 4 * 1024 * 1024) {
        return res.status(400).json({ error: `तस्वीर बहुत भारी है (${kb}KB) — छोटी करके डालें` });
      }

      const stamp = Date.now();
      const doc = await Vehicle.findByIdAndUpdate(req.params.id, {
        photoData: buf.toString("base64"),
        photoMime: "image/jpeg",
        photoAt: new Date(stamp),
        // ?v= इसलिए कि photo बदलने पर browser पुरानी वाली न दिखाए
        imageUrl: `/vehicle-photo/${req.params.id}?v=${stamp}`,
      }, { new: true }).select("-photoData");

      // disk पर पड़ी पुरानी photo (अगर बची हो) हटा दो
      if (req.vehOld && req.vehOld.startsWith("/vehicles/")) {
        try { fs.unlinkSync(path.join(__dirname, "public", req.vehOld.replace(/^\//, ""))); } catch (_) {}
      }

      log("INFO", "[VEHICLE] photo लगी", { name: doc.name, kb });
      res.json({ ok: true, imageUrl: doc.imageUrl, sizeKB: kb, doc });
    } catch (e) { log("ERROR", "/vehicles/:id/photo", { msg: e.message }); res.status(500).json({ error: e.message }); }
  });

app.delete("/api/vehicles/:id/photo", requireRole("super-admin", "admin"), attachVehBrand, async (req, res) => {
  try {
    if (req.vehOld && req.vehOld.startsWith("/vehicles/")) {
      try { fs.unlinkSync(path.join(__dirname, "public", req.vehOld.replace(/^\//, ""))); } catch (_) {}
    }
    await Vehicle.findByIdAndUpdate(req.params.id, {
      $unset: { photoData: "", photoMime: "", photoAt: "" }, imageUrl: "",
    });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// पुराने टूटे पते एक बार साफ़ कर दो — वे disk वाली photos अब मिट चुकी हैं,
// उनका पता रखने से "✓ सेव है" तो दिखता है पर तस्वीर नहीं आती। पता हटा देंगे
// तो साफ़ लिखा आएगा "photo नहीं" और पता चल जाएगा कि दोबारा डालनी है।
// (सिर्फ़ पहली बार चलता है — database में जो अब भी /vehicles/ वाले पते हैं)
// ⚠️ पहले यह 20 सेकंड की घड़ी के भरोसे था। database देर से जुड़े (Render की
//    free service ठंडी शुरुआत में सुस्त रहती है) तो यह चूक जाता। अब database
//    जुड़ने के बाद ही चलता है — भरोसेमन्द।
mongoose.connection.once("connected", async () => {
  try {
    const r = await Vehicle.updateMany(
      { imageUrl: { $regex: "^/vehicles/" }, photoData: { $in: [null, ""] } },
      { $set: { imageUrl: "" } }
    );
    if (r.modifiedCount) {
      log("INFO", "[VEHICLE] पुराने टूटे photo पते हटाए — इन गाड़ियों में photo दोबारा डालनी होगी",
        { count: r.modifiedCount });
    }
  } catch (e) { log("WARN", "पुराने photo पते साफ़ नहीं हुए", { msg: e.message }); }
});

// ⚠️ यह रास्ता जान-बूझकर "/api/" से बाहर है — <img> tag login का token नहीं
//    भेज सकता, इसलिए /api/ के अंदर रखने पर तस्वीर कभी न दिखती।
app.get("/vehicle-photo/:id", async (req, res) => {
  try {
    const v = await Vehicle.findById(req.params.id).select("photoData photoMime").lean();
    if (!v || !v.photoData) return res.status(404).send("photo नहीं मिली");
    const buf = Buffer.from(v.photoData, "base64");
    res.set({
      "Content-Type": v.photoMime || "image/jpeg",
      "Content-Length": buf.length,
      // ?v= बदलने पर ही नई तस्वीर आती है, वरना browser अपनी copy दिखाता है
...
// एक गाड़ी का पूरा "kit" — photo + वे सब ब्यौरे जो poster पर छपते हैं।
// हर editor यही एक route बुलाए, तो हर जगह एक जैसी जानकारी जाएगी।
app.get("/api/vehicles/:id/kit", async (req, res) => {
  try {
    const v = await Vehicle.findById(req.params.id).lean();
    if (!v) return res.status(404).json({ error: "गाड़ी नहीं मिली" });
    const money = (x) => (x ? String(x).replace(/^₹?/, "₹") : "");
    const fullName = [v.name, v.variant].filter(Boolean).join(" ");
    res.json({
      id: String(v._id), brand: v.brand,
      name: v.name, variant: v.variant || "", fullName,
      category: v.category || "",
      photo: v.imageUrl ? (/^https?:/.test(v.imageUrl) ? v.imageUrl : PUBLIC_URL + v.imageUrl) : "",
      photoPath: v.imageUrl || "",
      exShowroom: money(v.exShowroom), onRoad: money(v.onRoad),
      downPayment: money(v.downPayment), emi: money(v.emi),
      emiTenure: v.emiTenure || "", roi: v.roi || "",
      cashback: money(v.cashback), exchangeBonus: money(v.exchangeBonus),
      offerNote: v.offerNote || "",
      mileage: v.mileage || "", engine: v.engine || "",
      colors: v.colors || [], features: v.features || [],
      inStock: v.inStock !== false,
      // poster पर सीधे चिपकाने लायक़ बनी-बनाई लाइनें
      headline: fullName,
      subline: [v.engine, v.mileage].filter(Boolean).join(" · "),
      priceLine: money(v.exShowroom) ? `${money(v.exShowroom)} एक्स-शोरूम` : "",
      emiLine: money(v.emi) ? `EMI ${money(v.emi)}${v.emiTenure ? ` × ${v.emiTenure}` : ""}` : "",
      dpLine: money(v.downPayment) ? `डाउन पेमेंट ${money(v.downPayment)}` : "",
      offerLine: [money(v.cashback) && `कैशबैक ${money(v.cashback)}`,
                  money(v.exchangeBonus) && `एक्सचेंज ${money(v.exchangeBonus)}`]
                  .filter(Boolean).join(" · "),
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    for (const item of plan.slice(0, 40)) {
      try {
        ...
        //  ⚠️ पहरा — AI कभी सूचना (suchna) भेज भी दे तो रोज़ की पोस्ट में
        //     नहीं जाएगी; उसकी जगह विज्ञापन बनेगा। सूचना मालिक ख़ुद बनाते हैं।
        let type = TYPES.includes(item.type) ? item.type : "vigyapan";
        if (type === "suchna") type = "vigyapan";
        // topic को context बनाकर text generate करो
        let text = await generateText(item.brand, type, undefined, item.topic_hindi || undefined);
        // ⚠️ यह "हफ़्ते का प्लान" वाला batch route है — यहाँ भी suvichar
        //    में brand का नाम घुसने से सुरक्षा चाहिए, बाक़ी जगहों जैसी
        if (type === "suvichar") text = stripBrandFromPoem(text, item.brand);
...
app.post("/api/command/execute", async (req, res) => {
  try {
    ...
    //  ⚠️ बड़ी गड़बड़ी पकड़ी — caption और poster अलग-अलग तरीक़े से गाड़ी
    //     चुनते थे! poster smart-matcher (pickVehicleForAd) से सही variant
    //     चुनता, पर caption सीधे कच्चे `vehicle` शब्द से context बनाता —
    //     इसलिए poster में "SP 125" और caption में "Livo" जैसी अलग गाड़ी आ
    //     जाती। अब दोनों एक ही, पक्की चुनी हुई गाड़ी पर चलते हैं।
    let resolvedVeh = null, resolvedName = vehicle || "";
    if (finalType === "vigyapan") {
      try {
        resolvedVeh = await pickVehicleForAd(brand, vehicle || custom_text || offer_details || "");
        if (resolvedVeh) resolvedName = [resolvedVeh.name, resolvedVeh.variant].filter(Boolean).join(" ");
      } catch (e) { log("WARN", "[execute] गाड़ी नहीं चुनी जा सकी", { msg: e.message }); }
    }

    // Text तैयार करो
    let text;
    //  ⚠️ बड़ी गड़बड़ी — जब कोई छोटा सुझाव-वाक्य (जैसे "गुड मॉर्निंग शुभप्रभात
    //     पोस्ट", CreatePanel के तैयार बटनों में से एक) custom_text के तौर
    //     पर यहाँ आ जाता, तो वही एक वाक्य सीधे पूरी "post" बन जाता — AI कभी
    //     असली, पूरी कविता लिखता ही नहीं। poster पर सिर्फ़ वही छोटा वाक्य
    //     (या ख़ाली, buildSVG की पुरानी गड़बड़ी के साथ मिलकर) दिखता।
    //
    //     सुविचार में — अगर custom_text छोटा है (जैसा कोई तैयार, पूरी
    //     कविता नहीं दिखता, सिर्फ़ एक विषय/निर्देश जैसा लगता है) — उसे
    //     "content" की जगह "विषय" मानकर AI को देते हैं, ताकि वह उस विषय
    //     पर असली, पूरी कविता लिखे। लंबा custom_text (जो सचमुच तैयार
    //     content जैसा है) पहले जैसे ही सीधे इस्तेमाल होता है।
    const looksLikeTopic = finalType === "suvichar" && custom_text && custom_text.trim().length < 40 && !custom_text.includes("\n");
    if (custom_text && custom_text.trim() && !looksLikeTopic) {
      text = cleanAIText(custom_text.trim());
    } else {
      // Vehicle KB से असली price/EMI लो — AI खुद कभी नहीं बनाएगा
      const vCtx = await vehicleContext(brand, resolvedName);
      const extraCtx = [
        //  ⚠️ AI को साफ़ हुक्म — सिर्फ़ इसी गाड़ी की बात करो, कोई और नाम
        //     मत लिखो। पहले यह नहीं था, इसलिए caption में कोई और गाड़ी
        //     आ जाती थी।
        resolvedName && `गाड़ी: ${resolvedName}\n⚠️ सख़्त नियम: सिर्फ़ और सिर्फ़ इसी गाड़ी "${resolvedName}" का नाम लिखो। किसी और गाड़ी का नाम बिल्कुल मत लिखो।`,
        vCtx && `Database से असली जानकारी (सिर्फ़ यही use करो, अपने-आप कोई number मत बनाओ):\n${vCtx}`,
        offer_details && `User ने बताया: ${offer_details}`,
        looksLikeTopic && `विषय: ${custom_text.trim()}`,
      ].filter(Boolean).join("\n");
      text = await generateText(brand, finalType, undefined, extraCtx || undefined);
      if (finalType === "vigyapan" && resolvedName) {
        text = await lockVehicleNameInText(text, brand, resolvedName);
      }
    }
    //  ⚠️ पक्का पहरा — text किसी भी वजह से ख़ाली रह जाए (AI टूटी हो, या
    //     custom_text न पहुँचा हो), तो चुपचाप ख़ाली poster मत बनाओ। पहले
    //     ऐसा ही होता था: "post बन गई" लिख आता, पर कुछ बनता ही नहीं और
    //     आप इंतज़ार करते रहते। अब साफ़ वजह बताकर रोक देते हैं।
    if (!text || !String(text).trim()) {
      log("WARN", "[execute] text ख़ाली रह गया, post नहीं बनाई", { brand, finalType });
      return res.status(400).json({ error: "सूचना/content ख़ाली है — कृपया ऊपर वाले बॉक्स में लिखकर दोबारा कोशिश कीजिए" });
    }
    //  ⚠️ यह "सिर्फ़ एक post" मोड का रास्ता है — CommandCenter/तीन-विकल्प
    //     से बिल्कुल अलग route, पहले वहाँ यह सुरक्षा जोड़ी थी, यहाँ भूल
    //     गया था। इसीलिए सुविचार में brand का नाम बार-बार आता रहा — हर
    //     बार अलग रास्ते से बनी post में यह गड़बड़ी अलग-अलग बची रह जाती
    //     थी। अब यहाँ भी वही सुरक्षा।
    if (finalType === "suvichar") text = stripBrandFromPoem(text, brand);
    ...
    if (when === "now" || when === "today") {
      const doc = await Content.create({
        brand, type: finalType, text, status: "pending",
        whatsappText: toWhatsAppMarkdown(text, brand),   // "अभी ख़ुद भेजें" पर सजा हुआ जाए
      });
      // ⚠️ यह SmartBar (बोलकर सीधी post बनाने) का मुख्य रास्ता है। ऊपर text
      //    तो सही गाड़ी (vehicle) की database-जानकारी से बना है, पर यहाँ
      //    generateImages को खाली {} भेजा जाता था — poster अपने-आप कोई और
      //    गाड़ी चुन लेता (सिर्फ़ text का पहला शब्द देखकर अंदाज़ा)।
      // ⚠️ नया — विज्ञापन के साथ त्यौहार चुना हो तो वो भी भेजो, ताकि
      //    poster के सबसे ऊपर उस त्यौहार की आकर्षक पंक्ति आए
      const imgs = await generateImages(brand, doc._id, text, finalType, {
        // ⚠️ वही, पहले से चुनी हुई गाड़ी भेजो — ताकि poster और caption
        //    दोनों बिल्कुल एक ही गाड़ी की बात करें
        ...(resolvedVeh ? { vehicleDoc: resolvedVeh } : (vehicle ? { vehicle } : {})),
        ...(req.body.festival ? { festival: String(req.body.festival) } : {}),
        ...(req.body.festBg ? { festBg: String(req.body.festBg) } : {}),
      });
      const b64 = imgs._b64 || {}; delete imgs._b64;
      doc.images = imgs;
      if (b64.square) doc.imageData = { square: b64.square, story: b64.story };
...
app.get("/api/ai/gemini-models", requireRole("super-admin", "admin"), async (req, res) => {
  try {
    ...
    res.json({
      total: usable.length,
      models: usable,
      abhiChalRahe: order("GEMINI_TEXT_MODELS", "") .concat(["gemini-3.7-flash", "gemini-3.5-flash", "gemini-2.5-flash", "gemini-flash-latest"]).filter((v, i, a) => a.indexOf(v) === i),
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    let cleanText = customText ? cleanAIText(customText) : await generateText(brand, type, fe ? fe.name : undefined);
    // ⚠️ सिर्फ़ AI-लिखे सुविचार पर — आपका ख़ुद टाइप किया content नहीं छेड़ना
    if (!customText && type === "suvichar") cleanText = stripBrandFromPoem(cleanText, brand);
    ...
    const doc = await Content.create({
      brand, type, text: cleanText + (tags ? `\n${tags}` : ""), status: "pending",
      // ⚠️ hashtag लाइन को bold/italic नहीं करना — वो अपने-आप plain टैग जैसी
      //    दिखे, इसलिए सिर्फ़ असली सुविचार वाले हिस्से (cleanText) पर लगाई
      whatsappText: toWhatsAppMarkdown(cleanText, brand) + (tags ? `\n${tags}` : ""),
    });
...
async function makeSlideshowVideo(jobId, photoPaths, opts = {}) {
  ...
  const { perPhotoDur = 3, headline = "", subLine = "", musicFile = null, fontFile,
          xfade = 0.6, voiceFile = null, musicVol = 0.45 } = opts;
  const clips = [];
  ...
  const XF = Math.max(0, Math.min(Number(xfade) || 0, perPhotoDur * 0.5));
  const N = clips.length;

  // ── जोड़ने का काम ────────────────────────────────────────────────
  //  ⚠️ पहले concat से सीधे जुड़ती थीं — एक photo से दूसरी पर झटके से कट
  //     लगता था, सस्ता लगता था। अब xfade से एक तस्वीर दूसरी में घुलती है।
  const silentV = path.join(OUT_DIR, `${jobId}_v.mp4`);

  if (N >= 2 && XF > 0.05) {
    // offset का हिसाब: हर घुलाव के बाद चली हुई लंबाई XF सेकंड घट जाती है
    const parts = [];
    let acc = perPhotoDur;
    for (let i = 1; i < N; i++) {
      const off = acc - XF;
      const src = i === 1 ? "[0]" : `[a${i - 1}]`;
      const dst = i === N - 1 ? "[v]" : `[a${i}]`;
      parts.push(`${src}[${i}]xfade=transition=fade:duration=${XF}:offset=${off.toFixed(2)}${dst}`);
      acc = off + perPhotoDur;
    }
    const args = [];
    clips.forEach((c) => args.push("-i", c));
    args.push("-filter_complex", parts.join(";"), "-map", "[v]",
      "-c:v", "libx264", "-pix_fmt", "yuv420p", "-preset", "veryfast", silentV);
    await new Promise((res, rej) => execFile("ffmpeg", ["-y", ...args], { maxBuffer: 1024 * 1024 * 20 },
      (e, _o, se) => (e ? rej(new Error("xfade: " + (se || e.message).slice(0, 200))) : res())));
  } else {
    const listFile = path.join(OUT_DIR, `${jobId}_sclist.txt`);
    fs.writeFileSync(listFile, clips.map((c) => `file '${c}'`).join("\n"));
    await new Promise((res, rej) => execFile("ffmpeg",
      ["-y", "-f", "concat", "-safe", "0", "-i", listFile, "-c", "copy", silentV],
      { maxBuffer: 1024 * 1024 * 20 },
  ...
  // ── आवाज़ चढ़ाओ ──────────────────────────────────────────────────
  // audioDuration ffprobe से format=duration पूछता है — video पर भी वही चलता है
  const vDur = (await audioDuration(silentV)) || (N * perPhotoDur - (N - 1) * XF);
  const hasMusic = musicFile && fs.existsSync(musicFile);
  const hasVoice = voiceFile && fs.existsSync(voiceFile);

  if (!hasMusic && !hasVoice) {
    fs.renameSync(silentV, out);
  } else {
    // ⚠️ पहले "-shortest" लगता था। music video से छोटा हुआ तो पूरी video
    //    उतनी ही कटकर रह जाती थी — 30 सेकंड की video 10 सेकंड की।
    //    अब music लूप होकर video की पूरी लंबाई तक चलता है।
    const args = ["-y", "-i", silentV];
    const fparts = [];
    const amix = [];
    let ai = 1;
    ...
    if (hasMusic) {
      ...
      fparts.push(
        `[${ai}:a]atrim=0:${vDur.toFixed(2)},volume=${Number(musicVol).toFixed(2)},` +
        `afade=t=in:st=0:d=1,afade=t=out:st=${Math.max(0, vDur - 1.5).toFixed(2)}:d=1.5,` +
    ...
    if (hasVoice) {
      ...
      fparts.push(`[${ai}:a]volume=1.4,apad,atrim=0:${vDur.toFixed(2)},aformat=sample_rates=44100:channel_layouts=stereo[vo]`);
      // बोली हो तो music उसके नीचे अपने आप दब जाए
      if (hasMusic) {
        fparts.push(`[vo]asplit=2[vo1][vk]`);
        fparts.push(`[m][vk]sidechaincompress=threshold=0.03:ratio=20:attack=20:release=450:makeup=1[md]`);
        amix.length = 0; amix.push("[md]", "[vo1]");
      } else { amix.push("[vo]"); }
      ai++;
    }

    fparts.push(`${amix.join("")}amix=inputs=${amix.length}:duration=first:normalize=0,alimiter=limit=0.94[a]`);
    args.push("-filter_complex", fparts.join(";"), "-map", "0:v", "-map", "[a]",
      "-c:v", "copy", "-c:a", "aac", "-b:a", "128k",
      "-t", vDur.toFixed(2), "-movflags", "+faststart", out);

    await new Promise((res, rej) => execFile("ffmpeg", args, { maxBuffer: 1024 * 1024 * 20 },
      (e, _o, se) => (e ? rej(new Error("audio: " + (se || e.message).slice(0, 200))) : res())));
    try { fs.unlinkSync(silentV); } catch (_) {}
  }
...
app.post("/api/video/slideshow", requireRole("super-admin", "admin", "manager"), async (req, res) => {
  try {
    // ⚠️ पहले यहाँ music का ख़ाना था ही नहीं। makeSlideshowVideo music डालना
    //    जानता था, पर उसे कभी दिया ही नहीं जाता था — इसलिए हर video चुपचाप
    //    बनती थी। अब music, आवाज़ और घुलाव तीनों यहाँ से जाते हैं।
    const { brand, images, headline, subLine, perPhotoDur, caption,
            music, musicVol, xfade, voiceUrl } = req.body;
    if (!BRANDS[brand]) return res.status(400).json({ error: "brand चाहिए" });
    ...
    setImmediate(async () => {
      let photoPaths = [];
      // ⚠️ try के बाहर — वरना finally में नहीं मिलते (ReferenceError)
      let musicFile = null, musicTemp = false, musicName = "";
      try {
        photoPaths = saveBase64Photos(jobId, images.slice(0, 8));
        // music: नाम दिया हो तो वही, "none" कहा हो तो कोई नहीं,
        // कुछ न कहा हो तो MUSIC_DIR की पहली धुन अपने आप
        //  music कहाँ से आए, तीन हालतें:
        //    "none"        → कोई गाना नहीं
        //    trackId दिया  → आपका चुना हुआ
        //    कुछ न कहा     → post की तरह देखकर अपने-आप
        if (music !== "none") {
          const lib = global.__MUSIC_LIB;
          if (lib) {
            const picked = await lib.pickMusicFile({
              trackId: music && /^[a-f0-9]{24}$/i.test(music) ? music : null,
              type: "delivery", brand, mood: req.body.mood,
            });
            if (picked) { musicFile = picked.file; musicTemp = picked.temp; musicName = picked.name; }
          }
          // लाइब्रेरी ख़ाली हो तो पुराने MUSIC_DIR से (अगर कुछ बचा हो)
          if (!musicFile) {
            try {
              const all = fs.readdirSync(MUSIC_DIR).filter((f) => /\.(mp3|m4a|wav)$/i.test(f));
              if (all.length) musicFile = path.join(MUSIC_DIR, music && all.includes(path.basename(music)) ? path.basename(music) : all[0]);
            } catch (_) {}
          }
        }

        // अनाउंसमेंट स्टूडियो से बनी आवाज़ भी चढ़ सकती है
        let voiceFile = null;
        if (voiceUrl) {
          const vf = path.join(OUT_DIR, path.basename(String(voiceUrl).split("?")[0]));
          if (fs.existsSync(vf)) voiceFile = vf;
        }
        ...
        const url = await makeSlideshowVideo(jobId, photoPaths, {
          ...
          musicFile, voiceFile,
          musicVol: Math.max(0, Math.min(parseFloat(musicVol) || 0.45, 1.0)),
          xfade: Math.max(0, Math.min(parseFloat(xfade) ?? 0.6, 1.5)),
        });
        ...
        // database से निकाला गया गाना अस्थायी था — हटा दो
        if (musicTemp && musicFile) { try { fs.unlinkSync(musicFile); } catch (_) {} }
      }
    });
...
async function genToPending(brand, rawType, festivalName, extraCtx) {
  ...
  //  ⚠️ पहले caption (text) पहले AI से आज़ाद ढंग से लिखवाया जाता था — इसलिए
  //     AI कभी-कभी कोई मनगढ़ंत model नाम ("Dio 125") लिख देता था, जो हमारी
  //     सूची में है ही नहीं। बाद में poster के लिए अलग से जो भी गाड़ी मिली
  //     (कभी-कभी बेतरतीब), वो लग जाती — नतीजा: caption में "Dio 125",
  //     poster पर "SP125 DLX AE" जैसी बेमेल post।
  //
  //     अब क्रम उल्टा है: विज्ञापन के लिए पहले असली गाड़ी चुनते हैं
  //     (photo वाली), फिर उसी की सही क़ीमत/नाम AI को दे देते हैं ताकि caption
  //     उसी गाड़ी की बात करे — generateText का "विशेष जानकारी" तंत्र AI को
  //     अपनी तरफ़ से price/offer बनाने से रोकता भी है।
  let vehDoc = null, vehCtx = extraCtx;
  if (type === "vigyapan") {
    try {
      vehDoc = await pickVehicleForAd(brand, extraCtx);
      //  ⚠️ यही वजह थी कि caption में कोई और गाड़ी आ जाती थी — "!extraCtx"
      //     का मतलब था कि विषय दिया हो तो चुनी गई गाड़ी का नाम caption तक
      //     पहुँचता ही नहीं, और AI अपनी मर्ज़ी की गाड़ी लिख देता। अब नाम
      //     हमेशा जाता है, साथ में सख़्त हुक्म भी।
      if (vehDoc) {
        ...
        const lock = `गाड़ी: ${model}\n⚠️ सख़्त नियम: सिर्फ़ और सिर्फ़ इसी गाड़ी "${model}" का नाम लिखो। किसी और गाड़ी का नाम बिल्कुल मत लिखो।`;
        vehCtx = `${lock}${bits ? "\n" + bits : ""}${extraCtx ? "\nविषय: " + extraCtx : ""}`;
      }
    } catch (e) {
      log("WARN", "[genToPending] गाड़ी नहीं चुनी जा सकी", { msg: e.message });
    }
  }

  // extraCtx = batch/trigger से आया हुआ विषय — इससे हर post अलग बनता है
  let text = await generateText(brand, type, festivalName, vehCtx);
  //  ⚠️ पहरा — caption में कोई और गाड़ी का नाम आ गया हो तो सही नाम से बदलो
  if (type === "vigyapan" && vehDoc) {
    text = await lockVehicleNameInText(text, brand, [vehDoc.name, vehDoc.variant].filter(Boolean).join(" "));
  }
  // ⚠️ सुविचार में AI कभी-कभी prompt की मनाही के बावजूद brand का नाम
  //    सुविचार के भीतर ही घुसा देता है — यहीं काट देते हैं, code से पक्का
  if (type === "suvichar") text = stripBrandFromPoem(text, brand);

  const doc = await Content.create({
    brand, type, text, status: "pending",
    whatsappText: toWhatsAppMarkdown(text, brand),   // "अभी ख़ुद भेजें" पर यह जाएगी
  });
  const imgs = await generateImages(brand, doc._id, text, type, {
    festival: festivalName || "",
    vehicleDoc: vehDoc,   // ⚠️ वही गाड़ी जो caption में है — poster पर भी वही
  });
    ...
    try {
      ...
      for (const cmd of due) {
        ...
        try {
          ...
          // ⚠️ cmd.vehicle schedule बनाते समय ही save हो चुका था (देखें ऊपर
          //    ScheduledCommand.create), पर यहाँ इस्तेमाल नहीं होता था —
          //    poster अपने-आप कोई और गाड़ी चुन लेता, cmd.text किसी और की।
          const imgs = await generateImages(cmd.brand, doc._id, cmd.text, cmd.type, cmd.vehicle ? { vehicle: cmd.vehicle } : {});
          const b64 = imgs._b64 || {}; delete imgs._b64;
          doc.images = imgs;
          if (b64.square) doc.imageData = { square: b64.square, story: b64.story };
          await doc.save();
...
(async () => {
  ...
  // ── GROWTH ENGINE ─────────────────────────────────────────────────────
  //  Instagram Reels/Carousel/Story, Google Business, असली scheduling,
  //  tracking links. ऊपर की कोई पुरानी लाइन इससे नहीं बदलती।
  try {
    // ⚠️ जो लौटता है उसे रख लो — one-in-many-out को Google Business
    //    भेजने के लिए यही चाहिए। पहले यह कहीं रखा ही नहीं जाता था।
    global.__GROWTH = require("./growth-engine")(app, {
  ...
  // ── तीन विकल्प — एक चुनिए, बाक़ी दो हट जाएँ ───────────────────────────
  try {
    require("./three-choices.js")(app, {
      ...
      pickVehicleForAd, stripRs,   // ⚠️ बिना इनके तीनों tone अलग-अलग गाड़ी दिखा सकते
      toWhatsAppMarkdown,          // "अभी ख़ुद भेजें" पर सजा हुआ WhatsApp text जाए
      detectFestivalName,          // बिना इसके festival-type भी सादी "general" theme में बनता
      stripBrandFromPoem,          // बिना इसके सुविचार में brand का नाम कभी-कभी घुस जाता
    });
  } catch (e) {
    log("ERROR", "three-choices चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }

  // ── एक बात कहिए → सब कुछ एक साथ ───────────────────────────────────────
  //  तीनों सलाहों में यही सबसे ऊपर था। और इसके सातों टुकड़े पहले से थे —
  //  बस एक जगह जोड़े नहीं गए थे।
  try {
    require("./one-in-many-out.js")(app, {
      ...
      pickVehicleForAd, stripRs,   // ⚠️ बिना इनके caption और poster अलग-अलग गाड़ी की बात करते
      toWhatsAppMarkdown,          // "अभी ख़ुद भेजें" पर सजा हुआ WhatsApp text जाए
      detectFestivalName,          // बिना इसके "गणेश चतुर्थी की बधाई" भी सादी "general" theme में बनती
      stripBrandFromPoem,          // बिना इसके सुविचार में brand का नाम कभी-कभी घुस जाता
    });
  } catch (e) {
    log("ERROR", "one-in-many-out चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }

  // ── AI MARKETING ORCHESTRATOR (2.0) ───────────────────────────────────
  //  एक command → पूरा campaign. साथ में असली Truth Validation (poster की
  //  हर क़ीमत Vehicle database से मिलाई जाती है) और /api/today — एक ही
  //  request में पूरे दिन का हाल.
  //  ⚠️ ऊपर की कोई पुरानी लाइन इससे नहीं बदलती. growth-engine की तरह ही
  //     अलग file है — कुछ गड़बड़ हो तो सिर्फ़ यह बन्द होगा, बाक़ी app चलेगा.
  try {
    require("./orchestrator.js")(app, {
      log, BRANDS, TYPES, TYPE_LABEL,
      Content, Vehicle, Delivery, Lead, Notification, ScheduledCommand, FESTIVALS,
    ...
    log("ERROR", "orchestrator चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }

  // ── आदेश का रास्ता — बोलिए, सही पन्ना ख़ुद खुले ────────────────────────
  //  9 editors browser में तस्वीर बनाते हैं, इसलिए server उन्हें बना नहीं
  //  सकता। पर आदेश समझकर सही editor को पूरा भरा हुआ draft दे सकता है —
  //  फिर आपको सिर्फ़ देखकर "भेज दो" दबाना है।
  try {
    require("./command-router.js")(app, {
      log, BRANDS, Vehicle, requireRole, mongoose, PUBLIC_URL,
      parseCommandIntent, generateText, cleanAIText,
    });
  } catch (e) {
    log("ERROR", "command-router चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }

  // ── त्यौहार वाले सजे-सजाए poster ───────────────────────────────────────
  try {
    global.__FESTIVE = require("./festive-poster.js")(app, {
      log, BRANDS, OUT_DIR, PUBLIC_URL, requireRole, mongoose, sharp,
      Vehicle, composeLogos, compressImage,
      fetchAIBackground,      // "AI पृष्ठभूमि" विकल्प — गणेश/नवरात्रि/दिवाली के लिए
      inferVehicleFeatures,   // features हाथ से टाइप न करने पड़ें — AI ख़ुद बना दे
      removeWhiteBg,          // त्यौहार वाले poster में गाड़ी का सफ़ेद background कटे
    });
  } catch (e) {
    log("ERROR", "festive-poster चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }

  // ── पृष्ठभूमियों की लाइब्रेरी ──────────────────────────────────────────
  //  AI से बनी तस्वीर सेव कर लीजिए — दोबारा लगाने पर पैसे नहीं लगेंगे।
  try {
    require("./bg-library.js")(app, {
      log, BRANDS, OUT_DIR, PUBLIC_URL, requireRole, mongoose, sharp,
    });
  } catch (e) {
    log("ERROR", "bg-library चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }

  // ── गानों की लाइब्रेरी ─────────────────────────────────────────────────
  //  अपने गाने चढ़ाइए — database में सुरक्षित रहेंगे (disk मिट जाती है)।
  //  video बनाते समय ख़ुद चुनिए, या अपने-आप post की तरह के हिसाब से लग जाए।
  let MUSIC_LIB = null;
  try {
    MUSIC_LIB = require("./music-library.js")(app, {
      log, BRANDS, OUT_DIR, MUSIC_DIR, PUBLIC_URL, requireRole, mongoose,
    });
  } catch (e) {
    log("ERROR", "music-library चालू नहीं हुआ (बाक़ी app चलता रहेगा)", { msg: e.message });
  }
  global.__MUSIC_LIB = MUSIC_LIB;   // slideshow इसे यहीं से उठाता है

  // ── अनाउंसमेंट स्टूडियो ────────────────────────────────────────────────
  //  लिखो → आवाज़ बने → ढोल/music के साथ मिले → download हो जाए।
  //  ढोल-धमाके की आवाज़ें ffmpeg ख़ुद बनाता है, कोई बाहरी file नहीं चाहिए।
  try {
  ...
  // ══════════════════════════════════════════════════════════════════════
  //  ⚠️ सबसे आख़िर में — /api/ पर हमेशा JSON, कभी HTML नहीं
  // ----------------------------------------------------------------------
  //  पहले यह था ही नहीं। नतीजा: multer (file लेने वाला) या कोई भी middleware
  //  टूटे तो Express अपना HTML वाला error पन्ना भेज देता था, और app में
  //  दिखता था: Unexpected token '<', "<!DOCTYPE "... is not valid JSON
  //
  //  यह हर file चढ़ाने वाली जगह पर हो सकता था — गाने, त्यौहार की तस्वीर,
  //  गाड़ी की photo, delivery photo। अब सब जगह साफ़ हिन्दी सन्देश जाएगा।
  //
  //  ⚠️ यह सारे routes के बाद ही लगना चाहिए, वरना असली routes ढँक जाएँगे।

  // कोई /api/ रास्ता मिला ही नहीं
  app.use((req, res, next) => {
    if (!req.path.startsWith("/api/")) return next();
    res.status(404).json({ error: `यह रास्ता नहीं मिला: ${req.method} ${req.path}` });
  });

  // कहीं भी कुछ टूटे — JSON में वजह बताओ
  app.use((err, req, res, next) => {
    const api = String(req.path || "").startsWith("/api/");
    let code = err.status || err.statusCode || 500;
    let msg = err.message || "कुछ गड़बड़ हो गई";

    // multer की गड़बड़ियाँ — इन्हीं से HTML वाला पन्ना आता था
    if (err.code === "LIMIT_FILE_SIZE") {
      code = 413;
      msg = "file बहुत बड़ी है — छोटी करके दोबारा डालिए";
    } else if (err.code === "LIMIT_UNEXPECTED_FILE") {
      code = 400;
      msg = "file का नाम मेल नहीं खाया — पन्ना दोबारा खोलकर कोशिश कीजिए";
    } else if (err.type === "entity.too.large") {
      code = 413;
      msg = "भेजा गया data बहुत बड़ा है (30MB से ज़्यादा)";
    } else if (err.type === "entity.parse.failed") {
      code = 400;
      msg = "भेजा गया data समझ नहीं आया";
    }

    log("ERROR", "[UNCAUGHT]", { path: req.path, code: err.code || "", msg: err.message });

    if (api) return res.status(code).json({ error: msg });
