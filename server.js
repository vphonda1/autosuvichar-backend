...
async function generateText(brandId, type, festivalName, extraCtx) {
  ...
  const suvicharPrompt = `तुम Honda motorcycle dealer "${b.name}" (${b.place}) के WhatsApp admin हो।
...
Rules: कविता पूरी होनी चाहिए, बीच में मत काटो। Emojis भरपूर use करो। Showroom mention ज़रूर करो।`;
  ...
  const promoPrompt = `तुम Honda motorcycle dealer "${b.name}" (${b.place}, Phone: ${b.phone}) के WhatsApp marketer हो।
एक exciting promotional message लिखो (Hindi) जिसमें हो:
- 🔥🏍️ emojis के साथ catchy headline
...
- Total 6-8 lines। Emojis भरपूर। पूरा message हो, अधूरा नहीं।`;
  ...
  const prompt = type === "suvichar" || type === "festival" ? suvicharPrompt + cmdExtra :
    `तुम Honda dealer "${b.name}" (${b.sub}, ${b.place}) के social media manager हो।\n` +
...
async function generateMarketingPlan(brandIds, days, notes) {
  ...
  const sys = `तुम एक Honda/EV dealer group के marketing planner हो।
...
app.post("/api/command/poster-spec", async (req, res) => {
  try {
    ...
    const spec = await generatePosterSpec(brand, command || "", vehicle, offer_details, type);
...
app.post("/api/command/execute", async (req, res) => {
  try {
    ...
    if (custom_text && custom_text.trim()) {
      ...
      // vehicle/offer details को context में जोड़कर generateText को बेहतर बनाओ
      const extraCtx = [vehicle && `Vehicle: ${vehicle}`, offer_details && `Offer: ${offer_details}`].filter(Boolean).join(". ");
    ...
    try {
      ...
      for (const cmd of due) {
        ...
        try {
          ...
          if (cmd.recurring === "daily") {
            await ScheduledCommand.findByIdAndUpdate(cmd._id, { lastRunAt: new Date(), contentId: doc._id });
          } else {
