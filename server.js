...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const OFFER_TOP = o.festivalMode ? h * 0.650 : h * 0.495;
  ...
  const featTop = o.festivalMode ? h * 0.710 : h * 0.658;
  ...
  const featCh = o.festivalMode ? h * 0.019 : h * 0.024;
  ...
  ${(bankResult.rows || o.financeLineAlways) ? (() => {
    ...
    const fby2 = o.festivalMode ? h * 0.590 : bankHeadY - fbh2 * 0.72;
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      const mkBadge = (bw2, head, big, foot, isGift) => {
        ...
        const fsBig = fit(big, bw2 * 0.185, bw2 * 0.10);
        const fsSml = Math.min(fit(head, bw2 * 0.115, bw2 * 0.072), fit(foot, bw2 * 0.115, bw2 * 0.072));
