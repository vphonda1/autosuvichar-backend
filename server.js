...
const TEXT_PROVIDERS = {
  async gemini({ parts, temperature, json, timeout, maxTokens }) {
    ...
    for (const m of models) {
      try {
        const gc = { temperature, maxOutputTokens: maxTokens || 1200 };
  ...
  async openai({ parts, temperature, json, timeout, maxTokens }) {
    ...
    try {
      ...
      const body = {
        model, temperature, max_tokens: maxTokens || 1200,
...
const AI = {
  async _chain(providers, table, args, kind) {
    const errs = [];
    for (const name of providers) {
      ...
      try {
        ...
        if (r.ok) return r;
        errs.push(`${name}:${r.error}`);
      } catch (e) { errs.push(`${name}:${(e.message || "").slice(0, 40)}`); }
    }
    log("WARN", `[AI] सब ${kind} providers fail`, { errs: errs.join(" | ").slice(0, 300) });
  ...
  // JSON माँगो — parse भी यहीं हो जाता है
  async json(prompt, o = {}) {
    const parts = Array.isArray(prompt) ? prompt : [{ text: String(prompt) }];
    const r = await AI._chain(order("AI_TEXT_ORDER", "gemini,openai,anthropic,groq,ollama"), TEXT_PROVIDERS,
      { parts, temperature: o.temperature ?? 0.4, json: true, timeout: o.timeout || 20000, maxTokens: o.maxTokens },
      "json");
    if (!r.ok) return { error: r.error };
    try {
      // कुछ models ```json ... ``` में लपेट देते हैं
      const clean = String(r.text).replace(/^```(?:json)?\s*/i, "").replace(/```\s*$/, "").trim();
      const out = JSON.parse(clean);
      out.__via = r.via;
      return out;
    } catch (e) {
      log("WARN", "[AI] JSON parse fail", { via: r.via, head: String(r.text).slice(0, 120) });
      return { error: "AI ने सही JSON नहीं दिया (" + r.via + ")" };
    }
  },
...
async function cleanupStorage(dryRun) {
  ...
  try {
    ...
    // 3) बहुत पुरानी ActivityLog entries भी हटा दो (AuditLog को हाथ मत लगाओ)
    if (!dryRun) {
...
async function safePublish(Model, docId, req) {
  const fresh = await Model.findById(docId);
...
async function pushPendingToWhatsApp(brandId, limit) {
  ...
  const docs = await Content.find({ brand: brandId, status: "pending", _id: { $nin: alreadyIds } })
    .sort({ createdAt: -1 }).limit(Math.min(limit || 5, 10));
...
async function handleApprovalReply(brandId, from, raw) {
  ...
  // delivery है तो ग्राहक को भी भेज दो (अगर चालू है और नंबर मौजूद है)
  if (pend.kind === "delivery") {
    setImmediate(async () => {
      try {
...
// ग्राहक को उसकी delivery photo/video भेजो
async function sendDeliveryToCustomer(brandId, delivery) {
  const cfg = await OwnerWA.findOne({ brand: brandId }).lean();
  if (!cfg?.enabled) return { skipped: "WhatsApp बंद है" };
  if (cfg.sendToCustomer === false) return { skipped: "ग्राहक को भेजना बंद है" };

  const to = normalizeMobile(delivery.customerMobile);
  if (!to) return { skipped: "ग्राहक का मोबाइल नंबर नहीं है" };
  if (delivery.sentToCustomerAt) return { skipped: "पहले ही भेजा जा चुका है" };

  const b = BRANDS[brandId];
  const media = delivery.video || delivery.images?.square;
  if (!media) return { skipped: "कोई photo/video नहीं" };
  const url = String(media).startsWith("http") ? media : `${PUBLIC_URL}${media}`;
  const isVideo = !!delivery.video;

  const caption =
    `🎉 बधाई हो ${delivery.customerName || ""}!\n\n` +
    `आपकी नई ${delivery.bikeName || b.vehicleWord} की डिलीवरी पर ${b.name} परिवार की ओर से ` +
    `हार्दिक शुभकामनाएँ 🙏\n\n` +
  ...
  try {
    ...
    await Delivery.findByIdAndUpdate(delivery._id, { sentToCustomerAt: new Date() });
    await activity(brandId, "publish", "success", `ग्राहक ${delivery.customerName || to} को delivery post भेजी`);
    await audit(null, { brand: brandId, action: "send", entity: "delivery", entityId: delivery._id,
    ...
    if (cfg.numbers?.length) {
      const help = isWindow
        ? `⚠️ ${delivery.customerName || to} को सीधे नहीं भेज पाए।\n\n` +
        ...
        : `⚠️ ${delivery.customerName || to} को नहीं भेज पाए: ${msg.slice(0, 100)}\n\n${url}`;
...
async function runDailyEngine(brandId, opts = {}) {
  ...
  try {
    ...
    const recent = await Content.find({ brand: brandId }).sort({ createdAt: -1 }).limit(8).select("type text").lean();
    const vCtx = await vehicleContext(brandId, "");
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v51-customer-voice-lead", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), logos: availableLogos(), storage: diskUsage(), features: ["voice", "adapter", "variants", "news", "provider-agnostic", "retry", "audit", "cleanup", "rate-limit", "batch", "triggers", "insights", "creative-video", "mcp", "daily-engine", "auto-video", "video-header", "wa-approval", "monthly-report", "customer-wa", "lead-alert", "voice-command"], ai: AI.status(), ownerLogo: fs.existsSync(path.join(LOGO_DIR, OWNER_LOGO_FILE)), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
// किसी delivery की photo ग्राहक को अभी भेजो (हाथ से)
app.post("/api/delivery/:id/send-customer", rateLimit({ windowMs: 60000, max: 10, key: "wacust" }),
  requireRole("super-admin", "admin", "manager"), async (req, res) => {
  try {
    const d = await Delivery.findById(req.params.id);
    if (!d) return res.status(404).json({ error: "not found" });

    // नंबर अभी दिया गया हो तो सहेज लो
    if (req.body.customerMobile) {
      d.customerMobile = String(req.body.customerMobile).replace(/\D/g, "");
      await d.save();
    }
    if (req.body.force) { d.sentToCustomerAt = null; await d.save(); }

    const r = await sendDeliveryToCustomer(d.brand, d);
