...
function buildPromoSVG(brandId, o, w, h) {
  ...
  //    नीचे "डाउन पेमेंट शुरू ₹X से" — जैसा आपके दिखाए reference poster
  //    में "Activa Range की शुरुआत ₹77,446 से" वाली पंक्ति थी। downPayment
  //    का data ना हो तो सिर्फ़ ऊपर वाला हिस्सा, पूरी height में।
  ...
  const hasStartFrom = !!o.downPayment;
  ...
  const exShowroomSVG = btn3d(w * 0.05, OFFER_TOP, w * 0.42, priceBoxH, "एक्स-शोरूम", esc(priceTxt), "#141414", b.accent, "#fff", Math.round(w / 34), priceFs) +
    (hasStartFrom ? btn3d(w * 0.05, OFFER_TOP + priceBoxH + h * 0.010, w * 0.42, startBoxH, "डाउन पेमेंट शुरू", "₹" + esc(o.downPayment) + "* से", "#141414", "#ffd400", "#b00018", Math.round(w / 48), Math.round(w / 32)) : "");
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.cutout === false) veh = await sharp(photoPath).resize(Math.round(w * 0.86), Math.round(h * 0.36), { fit: "inside" }).png().toBuffer();
      else veh = await removeWhiteBg(photoPath, Math.round(w * 0.86), Math.round(h * 0.36));
    } catch (_) {}
    if (veh) {
