...
function buildPromoSVG(brandId, o, w, h) {
  ...
  //     साफ़ पढ़ा जाए।
  if (o.festArt) {
    bgRect = `<rect width="${w}" height="${h}" fill="#000" fill-opacity="0.34"/>`;
  ...
  const OFFER_TOP = h * 0.495, OFFER_H = h * 0.165;
  const rightBoxes = [];
  ...
  const featCh = h * 0.022, featGap = featCh * 0.28;
  const featResult = chipRowWrap(feats, featTop, featCh, w * 0.965, w, featGap);
  ...
  const bankTop = featTop + featResult.height + h * 0.026;   // ⚠️ थोड़ा और नीचे
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      const badgeY = top + vh * 0.24, badgeH = vh * 0.46;
      ...
      if (BR.loanTenure) {
        ...
        const bw2 = Math.max(w * 0.12, w - bx - w * 0.03);
      ...
      // बाएँ — 🎁 आसान किस्तें
      if (BR.emiStart) {
        const bw2 = Math.max(w * 0.12, Math.min(w * 0.18, left - w * 0.05));
        const bx = w * 0.03;
