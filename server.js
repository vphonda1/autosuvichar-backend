...
function stripBrandFromPoem(text, brandId) {
  ...
  const names = [b.name, String(b.name).replace(/\s+/g, "")].filter(Boolean);
  let cut = -1;
...
function shopSignatureLines(b) {
  return ["आपका अपना शोरूम", b.name, "आपका दिन शुभ एवं मंगलमय हो!!", b.place];
}
    ...
    if (fleetBuf) {
      try {
        ...
        const o = {
          ...
          cashback: stripRs(b0.festCashback) || stripRs(b0.minCashback) || "5000",
          downPayment: stripRs(b0.minDownPayment) || "",
          exchangeBonus: stripRs(b0.minExchange) || "2000",
          roi: "",
          features: feats,
