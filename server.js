...
async function pickVehicleForAd(brandId, hint) {
  ...
  let v = null;
  if (hint) {
    v = await Vehicle.findOne({ ...q, name: { $regex: String(hint).split(/\s+/)[0] || hint, $options: "i" } })
      .select("name variant photoData exShowroom downPayment cashback features").lean();
  }
  if (!v) v = await Vehicle.findOne(q).select("name variant photoData exShowroom downPayment cashback features").sort({ updatedAt: -1 }).lean();
  return v;
}
    ...
    try {
      ...
      if (v) {
        ...
        const o = {
          ...
          price: stripRs(v.exShowroom), downPayment: stripRs(v.downPayment), cashback: stripRs(v.cashback),
