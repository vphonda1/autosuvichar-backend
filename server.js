...
function buildSVG(brandId, text, w, h, type) {
  ...
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="50%" y="${h * 0.945}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${Math.round(w / 28)}" font-weight="700" fill="#fff">फ़ोन ${esc(b.phone)}  •  ${esc(b.place)}</text>
...
function offerSeal(o, w, h, accent) {
  ...
  const cx = w * 0.84, cy = h * 0.40, r = w * 0.09, fs = w / 32, pts = 24;
  let poly = "";
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v11-pro-layout", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
