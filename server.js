...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${w * 0.95}" y="${h * 0.855}" text-anchor="end" font-family="Mukta, sans-serif" font-size="${Math.round(w / 55)}" fill="#ffd54f" fill-opacity="0.9">AI Generated</text>
    ${opts && opts.sticker ? buildStickerSVG(opts.sticker, w * 0.80, h * 0.30, w * 0.13, (opts.stickerText || "स्पेशल").split("|")[0].trim(), (opts.stickerText || "ऑफर").split("|")[1] ? opts.stickerText.split("|")[1].trim() : "ऑफर") : ""}</svg>`);
}
...
function offerSeal(o, w, h) {
  const n = parseInt(o.sticker || "0", 10);
  if (!n) return "";
  const { l1, l2 } = offerTextLines(o);
  return buildStickerSVG(n, w * 0.84, h * 0.40, w * 0.10, l1, l2);
}
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v12-sticker-lib", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const stickerText = req.body.offer && OFFERS[req.body.offer] ? (() => { const d = OFFERS[req.body.offer]; return d.l1 + "|" + (d.l2def || "ऑफर"); })() : (req.body.stickerText || "");
    const opts = { sticker: req.body.sticker || "", stickerText };
