...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  const SAFE_TOP = TOP_BRAND_H + h * 0.015;
  const SAFE_BOT = h * 0.775;
  const HEAD_H = h * 0.075;                     // greeting-header के लिए जगह
  const SIG_H = h * 0.185;                      // दुकान की 4 लाइनों के लिए जगह
  const SAFE_H = (SAFE_BOT - SAFE_TOP) - HEAD_H - SIG_H;
  const TEXT_W = w * 0.68;
