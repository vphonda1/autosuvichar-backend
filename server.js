...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v19c-nano-banana", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
// Google Gemini से असली AI image — gemini-2.5-flash-image (Nano Banana, FREE, बिना billing)
async function fetchGeminiImage(prompt, w, h) {
  ...
  for (const m of models) {
    try {
      ...
      if (!res.ok) { lastErr = `${m}: ${res.status} ${raw.slice(0, 120)}`; log("ERROR", "gemini img", { m, status: res.status }); continue; }
      const j = JSON.parse(raw);
      const parts = (j.candidates && j.candidates[0] && j.candidates[0].content && j.candidates[0].content.parts) || [];
      const img = parts.find((p) => p.inlineData && p.inlineData.data);
      if (!img) { lastErr = `${m}: image नहीं लौटी`; continue; }
      return { buf: await sharp(Buffer.from(img.inlineData.data, "base64")).resize(w, h, { fit: "cover" }).png().toBuffer(), error: null };
    } catch (e) { lastErr = `${m}: ${e.message}`; }
  }
  return { buf: null, error: lastErr };
}
