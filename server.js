...
function templateContent(brandId, type, festivalName) {
  ...
  const bank = {
    suvichar: ["संकल्प से ही मंज़िल मिलती है — आज का सफर शुभ हो।", "हर सुबह एक नई शुरुआत है, बस पहला कदम बढ़ाइए।", "मेहनत का रास्ता लंबा है, पर मंज़िल उतनी ही सुंदर।",
      "🌅 शुभ प्रभात!\nइंसान के हाथ में सिर्फ़ कोशिश है, कामयाबी ईश्वर देता है।\nआपका दिन शुभ एवं मंगलमय हो। 🙏",
      "🌸 सुप्रभात 🌸\nजो बीत गया उसे भूल जाइए, जो आने वाला है उसका स्वागत कीजिए।\nआज का दिन आपके लिए शुभ हो।",
      "🪷 जय श्री राम 🪷\nसुबह की पहली किरण आपके जीवन में नई उमंग लेकर आए।\nशुभ प्रभात — दिन मंगलमय हो।",
      "✨ अच्छे विचार ही अच्छे जीवन की शुरुआत हैं।\nआज मुस्कुराइए, क्योंकि यह दिन दोबारा नहीं आएगा।\nशुभ दिन! 🌞"],
...
// ===== AUTO-DESIGN: हर poster पर अलग, non-overlapping सजावट =====
// safe corner slots (केंद्र खाली रहता है — text/photo के लिए). हर slot दूर-दूर ताकि overlap न हो।
const AUTO_SLOTS = [[0.13, 0.16], [0.87, 0.16], [0.13, 0.84], [0.87, 0.84], [0.5, 0.12], [0.12, 0.5], [0.88, 0.5]];
const AUTO_THEMES = [
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v35-auto-design", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
app.get("/api/content", async (req, res) => {
  try { const q = {}; if (req.query.brand) q.brand = req.query.brand; if (req.query.status) q.status = req.query.status;
    const docs = await Content.find(q).select("-imageData").sort({ createdAt: -1 }).limit(50).lean();
...
app.post("/api/promo", photoUpload.single("photo"), async (req, res) => {
  try {
    ...
    const o = { model: vmodel, price, downPayment, cashback, features, banks, bg: bg || "light", cutout: req.body.cutout !== "false", aiPrompt: aiPrompt || "", offer: offer || "", sticker: sticker || "", decor: req.body.decor || "", autoDecor: req.body.autoDecor === true || req.body.autoDecor === "true", autoSeed: req.body.autoSeed || "", photo: req.file?.filename || vehicle };
