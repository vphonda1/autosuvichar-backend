...
2. फिर: ----------✍️---------- (separator)
3. फिर: 4-5 lines की हिंदी कविता/सुविचार (पूरी, बीच में मत काटो) — यह पूरी
   तरह सामान्य होनी चाहिए, इसमें दुकान/showroom/brand का कोई ज़िक्र नहीं
4. फिर एक ख़ाली लाइन, फिर बिल्कुल यही सीमा-रेखा: ✂️-----------------
5. उसके बाद, अलग पैराग्राफ़ में, दुकान का प्रचार — सिर्फ़ यह पाँच लाइन:
   🪷🙏 आपका अपना शो रूम 🙏🪷
   🌻🙏 ${b.name} 🙏🌻
   आपका दिन शुभ एवं मंगलमय हो!!
   🌹🌷 ${b.place} 🌷🌹
...
- "यह एक WhatsApp संदेश है", "यहाँ एक सन्देश है जिसे आप भेज सकते हैं",
  ...
  सीधे सुविचार से शुरू करो
- सुविचार वाले हिस्से (नियम 3) में दुकान/showroom/brand का नाम बिल्कुल
  मत लाओ — वह सिर्फ़ ✂️ रेखा के बाद वाले हिस्से में आए
...
function toWhatsAppMarkdown(plainText) {
  ...
  const shopPart = (idx >= 0 ? s.slice(idx + CUT.length) : "").trim();
  ...
  for (const line of poemLines) {
    ...
    // सजावटी सीमा-रेखा ("----------✍️----------") जस की तस — इटैलिक में,
    // हल्की-सी, ताकि बाक़ी टेक्स्ट से अलग जगह जैसी दिखे
  ...
  if (shopPart) {
    const shopLines = shopPart.split("\n").map((l) => l.trim()).filter(Boolean);
    // दुकान के हिस्से में हर लाइन bold — यही "यहाँ आइए" वाला असली सन्देश है
    const shopBold = shopLines.map((l) => `*${l}*`).join("\n");
    result += "\n\n" + shopBold;
  }
...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  // ── SAFE ZONE ──────────────────────────────────────────────────
  //   ऊपर  : logo पट्टी                   (h*0.10 तक कुछ नहीं लिखना)
  //   नीचे  : address bar                (h*0.775 के नीचे कुछ नहीं लिखना)
  //   बाएँ/दाएँ : sticker rails            (text सिर्फ़ बीच के 68% में)
  //  ⚠️ पहले यहाँ ऊपर एक सादा "VP Honda" bar भी था — नीचे addrBar में भी
  //     brand का नाम लिखा होता है, तो एक ही poster पर नाम दो बार, बेमेल
  //     ढंग से छपता था। अब ऊपर की सादी पट्टी हटाकर, सुविचार के ठीक नीचे
  //     एक ही, सजी हुई brand-signature लाइन रखी है — इमोजी, अलग रंग,
  //     बड़ा-bold font, तिरछा (italic), बीचोंबीच।
  const SAFE_TOP = h * 0.10, SAFE_BOT = h * 0.775;
  const HEAD_H = h * 0.095;                     // greeting-header के लिए जगह
  const SIG_H = h * 0.085;                      // brand-signature के लिए जगह
  ...
  //
  //  ⚠️ बड़ी चूक भी पकड़ी — text में ✂️ रेखा के बाद दुकान का पूरा प्रचार
  //     (कई लाइनें, brand का नाम) भी होता है। अगर पूरा text यहाँ draw कर
  //     दें, तो नीचे जोड़ी गई brand-signature और addrBar मिलाकर brand का
  //     नाम poster पर तीन बार आ जाता — बिल्कुल बेढंगा। सिर्फ़ ✂️ रेखा से
  //     पहले का हिस्सा (असली सुविचार/कविता) यहाँ काम आता है — दुकान का
  //     नाम सिर्फ़ brand-signature (नीचे) और addrBar में, एक-एक बार।
  const CUT = "✂️-----------------";
  ...
  // ⚠️ brand-signature — कविता के ठीक नीचे, अपनी अलग लाइन में। इमोजी +
  //    brand का नाम + एक बारीक़ रेखा — गोल्ड रंग में, तिरछा (italic) और
  //    बड़ा-bold, ताकि नज़र सीधे इस पर जाए।
  const sigY = poemTop + SAFE_H + SIG_H * 0.52;
  const sigLineY = poemTop + SAFE_H + SIG_H * 0.14;
  const sigFont = Math.round(w / 19);
  const brandSignature = `
    <line x1="${w * 0.30}" y1="${sigLineY}" x2="${w * 0.70}" y2="${sigLineY}" stroke="${gold}" stroke-width="2" opacity="0.55"/>
    <text x="50%" y="${sigY}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${sigFont}" font-weight="800" font-style="italic" fill="${gold}">🙏 ${esc(b.name)} 🙏</text>`;
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const doc = await Content.create({
      ...
      whatsappText: toWhatsAppMarkdown(cleanText) + (tags ? `\n${tags}` : ""),
    });
...
async function genToPending(brand, rawType, festivalName, extraCtx) {
  ...
  const doc = await Content.create({
    ...
    whatsappText: toWhatsAppMarkdown(text),   // "अभी ख़ुद भेजें" पर यह जाएगी
  });
