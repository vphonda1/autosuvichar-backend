  ...
  if (SAJA.includes(type)) {
    ...
    if (fleetBuf) {
      try {
        ...
        if (feats.length < 10) {
          feats = ["हाई माइलेज", "सेल्फ स्टार्ट", "डिस्क ब्रेक", "ट्यूबलेस टायर", "डिजिटल मीटर", "LED हेडलाइट",
            "एलॉय व्हील", "मोबाइल चार्जिंग", "5-स्टेप सस्पेंशन", "स्टाइलिश ग्राफिक्स", "कम रखरखाव", "भरोसेमंद इंजन",
            "आरामदायक सीट", "मज़बूत बॉडी", "स्मूद गियर", "कम आवाज़"];
        }
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const OFFER_HEAD_Y = h * 0.472;
  const OFFER_TOP = h * 0.495, OFFER_H = o.festivalMode ? h * 0.115 : h * 0.165;
  const rightBoxes = [];
  ...
  const rightBoxesSVG2 = !o.festivalMode ? "" : (() => {
    const cols = 2, gapX = w * 0.025, bwx = (w * 0.80 - gapX) / cols;   // ⚠️ पतले किए
    const rows2 = Math.ceil(rightBoxes.length / cols);
    const bh2 = (OFFER_H - (rows2 - 1) * rbGap) / rows2;
    return rightBoxes.map((rb, i) => {
      const cx = w * 0.10 + (i % cols) * (bwx + gapX);
      const cy = OFFER_TOP + Math.floor(i / cols) * (bh2 + rbGap);
      return btn3d(cx, cy, bwx, bh2, rb.label, rb.value, rb.dark, rb.accent, rb.txt, Math.round(w / 64), Math.round(w / 42));
    }).join("\n  ");
  })();
  ...
  const featTop = o.festivalMode ? h * 0.632 : h * 0.658;
  ...
  const featCh = h * 0.024, featGap = featCh * 0.28;
  ...
  const featResult = chipRowWrap(feats, featTop, featCh, o.festivalMode ? w * 0.72 : w * 0.965, w, featGap);
  ...
  const bankTop = featTop + featResult.height + h * 0.030;   // ⚠️ थोड़ा और नीचे
  const bankHeadY = bankTop;
  const bankCh = h * 0.028, bankGap = bankCh * 0.28;
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.cutout === false) veh = await sharp(photoPath).resize(Math.round(w * 0.64), Math.round(h * 0.215), { fit: "inside" }).png().toBuffer();
      else veh = await removeWhiteBg(photoPath, Math.round(w * 0.64), Math.round(h * 0.215));
    } catch (_) {}
    if (veh) {
      const meta = await sharp(veh).metadata();
      const vw = meta.width || Math.round(w * 0.64), vh = meta.height || Math.round(h * 0.215);
      ...
      const top = Math.round(h * 0.245), left = Math.round((w - vw) / 2);
      ...
      const badgeY = o.festivalMode ? top + vh * 0.92 : top + vh * 0.24;
      const badgeH = o.festivalMode ? vh * 0.40 : vh * 0.46;
      ...
      // दाएँ — लोन अवधि
      if (BR.loanTenure) {
        const bx = Math.min(left + vw + w * 0.02, w * 0.82);
      ...
      if (BR.emiStart) {
        ...
        const bx = w * 0.03;
        if (bw2 >= w * 0.11 && bx + bw2 < left - w * 0.005) {
      ...
      if (o.festivalMode && o.festArt) {
        try {
          const md = Math.round(Math.min(w * 0.215, vh * 1.0));   // ⚠️ बड़े, पर नीचे के box पर न चढ़ें
          ...
          const myY = Math.round(top - vh * 0.18);
