...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const feats = (o.features || []).slice(0, 30);   // ⚠️ अब 30 — तीनों पंक्तियाँ पूरी भरी
  ...
  const priceTxt = o.price ? "₹" + o.price : "कॉल करें";
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      const vehH = o.festivalMode ? Math.round(h * 0.185) : Math.round(h * 0.215);
      ...
      if (o.festivalMode) {
        ...
        veh = await removeWhiteBg(photoPath, vehW, vehH);
      }
    } catch (_) {}
    if (veh) {
