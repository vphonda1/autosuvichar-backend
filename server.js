  ...
  async groq({ parts, temperature, json, timeout, maxTokens }) {
    ...
    for (const model of GROQ_MODELS) {
      try {
        ...
        // ⚠️ "सोचने वाले" models (gpt-oss जैसे) जवाब कभी content में नहीं,
        //    reasoning field में डाल देते हैं — तब पुराना कोड "empty" कहकर
        //    हार मान लेता था (आपके log में यही दिखा)। अब दोनों देखते हैं।
        const t = (msg?.content || msg?.reasoning || "").trim();
...
async function inferVehicleFeatures(brandId, vehicleName) {
  ...
  try {
    ...
    if (r.ok && r.text) {
      const feats = r.text.split(",").map((s) => s.trim().replace(/^[-•\d.]+\s*/, "")).filter(Boolean).slice(0, 12);
...
async function pickVehicleForAd(brandId, hint) {
  ...
  let v = null;
  if (hint) {
    //  ⚠️ बड़ी गड़बड़ी पकड़ी — पहले सिर्फ़ hint का *पहला शब्द* ढूँढा जाता था
    //     (split(/\s+/)[0])। इसलिए "Shine 100 DX" माँगने पर सिर्फ़ "Shine"
    //     खोजा जाता, और पहली मिलने वाली "Shine100 STD" उठ जाती — बिल्कुल
    //     ग़लत गाड़ी का poster बन जाता (screenshot में यही दिखा)।
    //     अब पूरा नाम मिलाते हैं: पहले उसी मॉडल-परिवार की सारी गाड़ियाँ
    //     निकालकर, JS में जगह/केस हटाकर सबसे अच्छा मेल चुनते हैं।
    const norm = (s) => String(s || "").toLowerCase().replace(/[\s\-_.]/g, "");
    const hintN = norm(hint);
    const firstWord = String(hint).split(/\s+/)[0] || hint;
    const cands = await Vehicle.find({ ...q, name: { $regex: firstWord, $options: "i" } }).select(sel).lean();
    if (cands.length) {
      const score = (c) => {
        const full = norm([c.name, c.variant].filter(Boolean).join(""));
        const nameOnly = norm(c.name);
        if (full === hintN || nameOnly === hintN) return 100;          // बिल्कुल वही
        if (full.includes(hintN) || hintN.includes(full)) return 80;   // पूरा नाम अंदर
        if (nameOnly.includes(hintN) || hintN.includes(nameOnly)) return 60;
        // कितने अक्षर आगे से मिलते हैं — जितने ज़्यादा उतना बेहतर
        let i = 0; while (i < full.length && i < hintN.length && full[i] === hintN[i]) i++;
        return i;
      };
      const withPrice = cands.filter((c) => c.exShowroom);
      const pool = withPrice.length ? withPrice : cands;   // क़ीमत-भरी को पहले
      pool.sort((a, c) => score(c) - score(a));
      v = pool[0];
    }
  }
