...
async function inferVehicleFeatures(brandId, vehicleName) {
  ...
  return ["हाई माइलेज", "सेल्फ स्टार्ट", "डिस्क ब्रेक", "ट्यूबलेस टायर", "डिजिटल मीटर", "LED हेडलाइट", "एलॉय व्हील", "मोबाइल चार्जिंग"];
}
...
async function pickVehicleForAd(brandId, hint) {
  ...
  let v = null;
  if (hint) {
    const nameQ = { ...q, name: { $regex: String(hint).split(/\s+/)[0] || hint, $options: "i" } };
    // ⚠️ पहले सीधे नाम-मैच वाली पहली गाड़ी ले ली जाती थी, चाहे उसमें क़ीमत
    //    भरी हो या नहीं — "एक्स-शोरूम ₹" जैसा ख़ाली दिखता था (screenshot
    //    में यही दिखा)। अब उसी नाम की, क़ीमत-भरी गाड़ी को पहले ढूँढते हैं।
    v = await Vehicle.findOne({ ...nameQ, exShowroom: { $nin: [null, ""] } }).select(sel).lean();
    // क़ीमत-भरी ना मिले तो भी सही गाड़ी/फ़ोटो दिखे — नाम से मिलती कोई भी लो
    if (!v) v = await Vehicle.findOne(nameQ).select(sel).lean();
  }
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  if (o.downPayment) rightBoxes.push({ label: "डाउन पेमेंट", value: "₹" + esc(o.downPayment) + "*", dark: "#141414", accent: "#ffd400", txt: "#b00018" });
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      if (BRANDS[brandId]?.loanTenure) {
        ...
        if (badgeW >= w * 0.11) {
          const badgeY = top + vh * 0.30, badgeH = vh * 0.34;
          ...
          const badgeSvg = `<svg width="${Math.round(badgeW)}" height="${Math.round(badgeH)}" xmlns="http://www.w3.org/2000/svg">
            <rect x="1" y="1" width="${badgeW - 2}" height="${badgeH - 2}" rx="14" fill="#141414" stroke="#ffd400" stroke-width="2"/>
            <text x="${badgeW / 2}" y="${badgeH * 0.28}" text-anchor="middle" font-size="${Math.round(badgeW * 0.22)}">🕒</text>
            <text x="${badgeW / 2}" y="${badgeH * 0.52}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(badgeW * 0.15)}" font-weight="800" fill="#ffd400">${esc(tenure)}</text>
            <text x="${badgeW / 2}" y="${badgeH * 0.72}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(badgeW * 0.115)}" fill="#fff">तक लोन</text>
            <text x="${badgeW / 2}" y="${badgeH * 0.90}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(badgeW * 0.115)}" fill="#fff">अवधि*</text>
          </svg>`;
