...
// Settings debug endpoint — check करो क्या save है
app.get("/api/debug/settings", requireRole("super-admin", "admin"), async (req, res) => {
  try {
    await loadSettings(); // fresh reload
    const dbDocs = await Setting.find().lean();
    const out = {};
    Object.keys(BRANDS).forEach((id) => {
      const c = brandCreds(id);
      out[id] = {
        hasFbPageId: !!c.fbPageId,
        hasFbToken: !!c.fbToken,
        hasIgUserId: !!c.igUserId,
        hasWaPhoneId: !!c.waPhoneId,
        waRecipients: c.waRecipients,
        fbPageId: c.fbPageId || "(empty)",
        igUserId: c.igUserId || "(empty)",
        waPhoneId: c.waPhoneId || "(empty)",
        dbDoc: dbDocs.find(d => d.brand === id) ? "found in DB" : "NOT IN DB",
        cacheKeys: Object.keys(SETTINGS_CACHE[id] || {}),
      };
    });
    res.json({ cache: SETTINGS_CACHE, resolved: out, dbDocs: dbDocs.map(d => ({ brand: d.brand, keys: Object.keys(d.creds || {}) })) });
