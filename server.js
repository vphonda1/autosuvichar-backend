...
function buildPromoSVG(brandId, o, w, h) {
  ...
  //  क़ीमत के साथ जुड़ी बात ("बैटरी के बिना") — बॉक्स के ठीक नीचे छोटे अक्षरों में
  const priceNoteSVG = (showPriceBox && o.priceNote && !hasStartFrom)
    ? `<text x="${w * 0.26}" y="${OFFER_TOP + priceBoxH + h * 0.028}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 48)}" font-weight="700" fill="${p.textDark}">* ${esc(o.priceNote)}</text>`
    : "";
  const exShowroomSVG = !showPriceBox ? "" :
    btn3d(w * 0.05, OFFER_TOP, w * 0.42, priceBoxH, "एक्स-शोरूम", esc(priceTxt), "#141414", b.accent, "#fff", Math.round(w / 34), priceFs) +
    (hasStartFrom ? btn3d(w * 0.05, OFFER_TOP + priceBoxH + h * 0.010, w * 0.42, startBoxH, "रेंज शुरू", "₹" + esc(o.rangeStart) + "* से", "#141414", "#ffd400", "#b00018", Math.round(w / 48), Math.round(w / 32)) : "");
