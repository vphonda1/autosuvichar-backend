...
async function generateText(brandId, type, festivalName) {
  ...
  const offerExtra = (type === "promo" || type === "vigyapan") ? " ऑफर: कैशबैक ₹5000*, Low EMI ₹1999/-, ROI @7.99%*, Exchange Bonus ₹2000*, Free Gift, Scratch & Win।" : "";
  ...
  const prompt = `तुम Honda dealer "${b.name}" (${b.sub}, ${b.place}) के social media manager हो।\n` +
    `Products: ${b.products.join(", ")}. Phone: ${b.phone}.${extra}${offerExtra}\n` +
    `Style: ${sty}. Post: "${TYPE_LABEL[type]}". नियम: 2-4 Hindi lines, unique, 2-3 emojis, no hashtag, no intro — सिर्फ post text, max 160 chars।`;
...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  const gold = "#ffd400";
  // Hindi text के लिए adaptive font size और wrapping
  let fontSize = Math.round(w / 15);
  // Hindi chars wider हैं — 0.45 multiplier use करो
  let maxChars = Math.max(10, Math.floor((w * 0.80) / (fontSize * 0.48)));
  let lines = wrapLines(stripEmoji(text), maxChars);
  // ज़्यादा lines होने पर font छोटा करो
  while (lines.length > 5 && fontSize > Math.round(w / 24)) {
    fontSize = Math.round(fontSize * 0.88);
    maxChars = Math.max(10, Math.floor((w * 0.80) / (fontSize * 0.48)));
    lines = wrapLines(stripEmoji(text), maxChars);
  }
  // max 6 lines show करो
  if (lines.length > 6) lines = lines.slice(0, 6);
  const lineGap = Math.round(fontSize * 1.38);
  // Text center में रखो — poster के 35% से 75% area में
  const textAreaH = h * 0.40;
  const totalTextH = lines.length * lineGap;
  const startY = h * 0.35 + Math.max(0, (textAreaH - totalTextH) / 2) + fontSize;
  const tspans = lines.map((l, i) => {
    const y = startY + i * lineGap;
    // Text poster से बाहर न जाए
    if (y > h * 0.78) return "";
    return `<text x="50%" y="${y}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${fontSize}" font-weight="700" fill="#fff" textLength="${Math.round(w*0.88)}" lengthAdjust="spacingAndGlyphs">${esc(l)}</text>`;
  }).filter(Boolean).join("");
...
async function generateImages(brandId, id, text, type, opts) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    // Logo PromoEditor SVG में embed है — server-side duplicate logo नहीं लगाओ
    // अगर brand logo file available है तो ही composite करो
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    // Logo DeliveryEditor SVG में embed है — server-side duplicate नहीं
