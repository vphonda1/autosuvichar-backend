...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const OFFER_TOP = o.festivalMode ? h * 0.640 : h * 0.495;
  ...
  const featTop = o.festivalMode ? h * 0.703 : h * 0.658;
  ...
  const featCh = o.festivalMode ? h * 0.020 : h * 0.024;
  ...
  const bankTop = featTop + featResult.height + (o.festivalMode ? h * 0.026 : h * 0.030);   // ⚠️ थोड़ा और नीचे
  const bankHeadY = bankTop;
  const bankCh = o.festivalMode ? h * 0.023 : h * 0.028;
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.festivalMode) {
        ...
        const MAXH = Math.round(h * 0.265);   // ⚠️ नीचे badge+box+features सबकी जगह बचे
        try {
    ...
    if (veh) {
      ...
      const top = Math.round(o.festivalMode ? h * 0.285 : h * 0.245), left = Math.round((w - vw) / 2);
      ...
      const badgeY = o.festivalMode ? Math.round(Math.min(h * 0.575, Math.max(h * 0.500, (o._vehBottom || h * 0.52) + h * 0.010))) : top + vh * 0.24;
