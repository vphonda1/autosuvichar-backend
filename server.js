...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const featCh = o.festivalMode ? h * 0.0185 : h * 0.024;
  ...
  ${bankResult.rows ? `<text x="${w * 0.5}" y="${bankHeadY}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 44)}" font-weight="800" fill="#fff" stroke="#9c0019" stroke-width="0.8">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>${bankResult.svg}` : ""}
  <!-- आकर्षक address बटन -->
  ${addrBar(b, w, h)}
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      if (o.festivalMode && o.festArt) {
        try {
          const md = Math.round(o.festivalMode ? w * 0.125 : Math.min(w * 0.215, vh * 1.0));
          ...
          const myY = Math.round(o.festivalMode ? h * 0.205 : top - vh * 0.18);
