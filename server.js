...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  // हर बार नया लुक — 3 layout variants में से random
  const vnt = Math.floor(Math.random() * 3);
  const vDecor = vnt === 1
    ? `<rect x="${w * 0.05}" y="${h * 0.27}" width="${w * 0.9}" height="${h * 0.5}" rx="30" fill="#fff" fill-opacity="0.08" stroke="${gold}" stroke-width="2.5" stroke-opacity="0.85"/>`
    : vnt === 2
      ? `<rect x="0" y="${h * 0.27}" width="${w * 0.018}" height="${h * 0.5}" fill="${gold}"/><rect x="${w * 0.982}" y="${h * 0.27}" width="${w * 0.018}" height="${h * 0.5}" fill="${gold}"/><polygon points="0,${h * 0.11} ${w * 0.16},${h * 0.11} 0,${h * 0.24}" fill="#fff" opacity="0.10"/><polygon points="${w},${h * 0.11} ${w - w * 0.16},${h * 0.11} ${w},${h * 0.24}" fill="#fff" opacity="0.10"/>`
      : "";
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${opts && opts.decor ? decorSVG(opts.decor, w, h, w * 0.038, [[0.10, 0.165], [0.90, 0.165], [0.10, 0.795], [0.90, 0.795]]) : ""}</svg>`);
}
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v29-variety", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const cleanText = await generateText(brand, type, fe ? fe.name : undefined);
    const tags = (req.body.tags || "").trim();
    const opts = { sticker: req.body.sticker || "", offer: req.body.offer || "", decor: req.body.decor || "", bg: req.body.bg || "auto", themeColor: fe ? fe.color : undefined, themeColor2: fe ? fe.color2 : undefined };
    const doc = await Content.create({ brand, type, text: cleanText + (tags ? `\n${tags}` : ""), status: "pending" });
    doc.images = await generateImages(brand, doc._id, cleanText, type, opts);
...
app.post("/api/promo", photoUpload.single("photo"), async (req, res) => {
  try {
    ...
    const o = { model: vmodel, price, downPayment, cashback, features, banks, bg: bg || "light", cutout: req.body.cutout !== "false", aiPrompt: aiPrompt || "", offer: offer || "", sticker: sticker || "", decor: req.body.decor || "", photo: req.file?.filename || vehicle };
...
(async () => {
  ...
  if (process.env.RESET_ADMIN_PASS) {
    const email = (process.env.SEED_ADMIN_EMAIL || "admin@vphonda.com").toLowerCase();
    await User.findOneAndUpdate({ email }, { passwordHash: await bcrypt.hash(process.env.RESET_ADMIN_PASS, 10) }, { upsert: true, setDefaultsOnInsert: true });
    log("INFO", `Admin password RESET हुआ (${email}) — अब RESET_ADMIN_PASS env हटा दें`);
  }
