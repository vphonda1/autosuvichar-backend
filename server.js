...
function cleanAIText(t) {
  ...
  //     वाला पहला पैराग्राफ़ जोड़ देता है ("यहाँ एक WhatsApp सन्देश है
  //     जिसे आप VP Honda के admin के रूप में भेज सकते हैं...")। असली
  //     सुविचार/caption की पहली लाइन कभी ऐसी नहीं होती, इसलिए अगर पहला
  //     पैराग्राफ़ इस तरह का दिखे और छोटा हो, तो उसे पूरा हटा दो — बाक़ी
  //     असली content को हाथ मत लगाओ।
  const paras = s.split(/\n\s*\n/);
  if (paras.length > 1) {
    const first = paras[0];
    const looksLikeMeta =
      first.length < 220 &&
      (/(संदेश|सन्देश|message)\s+(है|hai)\b/i.test(first) ||
       /^(यहाँ|यहां|here'?s?|below is|as (a|the|your))/i.test(first.trim()));
...
async function generateImages(brandId, id, text, type, opts) {
  ...
  //  ⚠️ पहले सिर्फ़ festival और gift यहाँ आते थे। विज्ञापन सादे text वाले
  //     रास्ते पर चला जाता था — यानी उस post में गाड़ी की photo लगती ही
  //     नहीं थी, जबकि गाड़ी सबसे ज़्यादा वहीं चाहिए।
  const SAJA = ["festival", "gift", "vigyapan"];
  if (SAJA.includes(type) && global.__FESTIVE) {
    try {
      const lines = String(text || "").split(/\n+/).map((x) => x.trim()).filter(Boolean);
      const ad = type === "vigyapan";
      const b = BRANDS[brandId] || {};

      const imgs = await global.__FESTIVE.buildFestivePoster(id, {
        brand: brandId,
        theme: ad ? "vigyapan" : undefined,
        festival: ad ? "" : (opts?.festival || lines[0] || ""),
        kicker: ad ? (b.name || "") : (lines.length > 2 ? lines[0] : ""),
        title: ad
          ? (opts?.vehicle || lines[0] || "आज का ऑफ़र")
          : (opts?.festival || (lines.length > 2 ? lines[1] : lines[0]) || "शुभकामनाएं"),
        sub: ad ? (lines[1] || "") : (lines.length > 2 ? lines[2] : (lines[1] || "")),
...
async function genToPending(brand, rawType, festivalName, extraCtx) {
  ...
  //  ⚠️ पहले generateImages को ख़ाली {} जाता था — इसलिए poster को पता ही
  //     नहीं चलता था कि किस गाड़ी की बात हो रही है। विज्ञापन में गाड़ी का
  //     नाम शीर्षक पर आना चाहिए, इसलिए यहीं चुन लेते हैं।
  let vehName = "";
  if (type === "vigyapan") {
    try {
      // पहले उस गाड़ी को देखो जिसका ज़िक्र लिखाई में है
      const list = await Vehicle.find({ brand, active: true, inStock: true })
        .select("name variant imageUrl").lean();
      const hit = list.find((v) => v.name && String(text).includes(v.name));
      // वरना जिसकी photo लगी हो, वही — poster अच्छा बनेगा
      const withPic = list.filter((v) => v.imageUrl);
      const pick = hit || withPic[Math.floor(Math.random() * withPic.length)] || list[0];
      if (pick) vehName = [pick.name, pick.variant].filter(Boolean).join(" ");
    } catch (e) {
