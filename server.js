...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v26-content-bg", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    let text = await generateText(brand, type, fe ? fe.name : undefined);
