...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v19-gemini-img", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
...
async function fetchGeminiImage(prompt, w, h) {
  ...
  try {
    ...
    if (!res.ok) { log("ERROR", "gemini img", { status: res.status, body: (await res.text()).slice(0, 200) }); return null; }
    const j = await res.json();
    const p = j.predictions && j.predictions[0];
    const b64 = p && (p.bytesBase64Encoded || (p.image && p.image.imageBytes));
    if (!b64) return null;
    return await sharp(Buffer.from(b64, "base64")).resize(w, h, { fit: "cover" }).png().toBuffer();
  } catch (e) { log("ERROR", "gemini img fail", { msg: e.message }); return null; }
}
app.post("/api/ai-bg", async (req, res) => {
  try {
    ...
    let buf = await fetchGeminiImage(prompt, w, h);          // असली AI (key हो तो)
    let src = "gemini";
    if (!buf) { buf = await fetchAIBackground(req.body.brand || "vp_honda", { aiPrompt: req.body.prompt || "" }, w, h); src = "pollinations"; } // free fallback
    if (!buf) return res.status(502).json({ error: "AI background अभी नहीं बना — key डालें (GEMINI_API_KEY) या थोड़ी देर बाद फिर कोशिश करें" });
    res.json({ dataUrl: "data:image/png;base64," + buf.toString("base64"), source: src });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
