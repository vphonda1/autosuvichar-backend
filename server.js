...
async function postFacebook(c, item) {
  ...
  // restart-safe URL: /api/image/... पहले try करो, disk URL fallback
  const imgUrl = item.imgUrl ? `${process.env.PUBLIC_URL || "https://autosuvichar-backend.onrender.com"}${item.imgUrl}` : item.images?.landscape || item.images?.square;
  if (!imgUrl) throw new Error("FB: image URL नहीं मिला");
  const r = await fetch(`${GRAPH}/${c.fbPageId}/photos`, { method: "POST", body: new URLSearchParams({ url: imgUrl, caption: item.text, access_token: c.fbToken }) });
  const d = await r.json();
  if (d.error) { log("ERROR", "FB post error", { err: d.error, imgUrl }); throw new Error("FB: " + d.error.message); }
...
async function postInstagram(c, item) {
  ...
  const cr = await (await fetch(`${GRAPH}/${c.igUserId}/media`, { method: "POST", body: new URLSearchParams({ image_url: imgUrl, caption: item.text, access_token: c.fbToken }) })).json();
  if (cr.error) { log("ERROR", "IG container error", { err: cr.error, imgUrl }); throw new Error("IG container: " + cr.error.message); }
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v40-publish-fix", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
