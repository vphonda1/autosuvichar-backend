...
function buildPromoSVG(brandId, o, w, h) {
  ...
  <!-- गाड़ी का नाम (बड़ा, center में; logo दोनों तरफ़ अलग से लगते हैं) -->
  ${(() => {
    const modelName = String(o.model || "");
    const safeW = w * 0.68;
    let fs = Math.round(w / 14);
    const minFs = Math.round(w / 22);
    while (fs > minFs && modelName.length * fs * 0.56 > safeW) fs -= 2;
    return `<text x="${w * 0.5}" y="${h * 0.135}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="800" fill="${p.textDark}">${esc(modelName)}</text>`;
  })()}
  <rect x="${w * 0.5 - w * 0.09}" y="${h * 0.150}" width="${w * 0.18}" height="6" rx="3" fill="${b.accent}"/>
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.cutout === false) veh = await sharp(photoPath).resize(Math.round(w * 0.64), Math.round(h * 0.325), { fit: "inside" }).png().toBuffer();
      else veh = await removeWhiteBg(photoPath, Math.round(w * 0.64), Math.round(h * 0.325));
    } catch (_) {}
    if (veh) {
      const meta = await sharp(veh).metadata();
      const vw = meta.width || Math.round(w * 0.64), vh = meta.height || Math.round(h * 0.325);
      ...
      const top = Math.round(h * 0.172), left = Math.round((w - vw) / 2);
