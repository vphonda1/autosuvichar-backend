...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const rightBoxesSVG = rightBoxes.map((rb, i) => {
    ...
    return btn3d(w * 0.52, y, w * 0.43, rbH, rb.label, rb.value, rb.dark, rb.accent, rb.txt, rbLabelFs, rbValueFs);
  }).join("\n  ");
  ...
  const hasStartFrom = !!o.rangeStart;
  const priceBoxH = hasStartFrom ? OFFER_H * 0.58 : OFFER_H;
  const startBoxH = hasStartFrom ? OFFER_H - priceBoxH - h * 0.010 : 0;
  const exShowroomSVG = btn3d(w * 0.05, OFFER_TOP, w * 0.42, priceBoxH, "एक्स-शोरूम", esc(priceTxt), "#141414", b.accent, "#fff", Math.round(w / 34), priceFs) +
  ...
  ${(() => {
    ...
    return `<defs><linearGradient id="nmg" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#2b2b2b"/><stop offset="50%" stop-color="#141414"/><stop offset="100%" stop-color="#000"/>
      </linearGradient></defs>
