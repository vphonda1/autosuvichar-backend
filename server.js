...
const AI = {
  ...
  status() {
    ...
    return {
      ...
      video: { order: ["ffmpeg-local"], ready: { "ffmpeg-local": ENABLE_VIDEO } },
    };
  },
};
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v44-voice-news-variants", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), features: ["voice", "adapter", "variants", "news", "provider-agnostic"], ai: AI.status(), ownerLogo: fs.existsSync(path.join(LOGO_DIR, OWNER_LOGO_FILE)), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
// ══════════════════════════════════════════════════════════════
app.post("/api/variants/caption", async (req, res) => {
  try {
...
app.post("/api/variants/poster", async (req, res) => {
  try {
...
app.post("/api/news/fetch", requireRole("super-admin", "admin", "manager"), async (req, res) => {
  try {
