...
function buildPromoSVG(brandId, o, w, h) {
  ...
  if (o.festArt) {
    //  ⚠️ त्यौहार की तस्वीर अब वॉटरमार्क जितनी हल्की — दिखती तो है, पर
      ...
      bgRect = `<rect width="${w}" height="${h}" fill="${b.accent2}" fill-opacity="0.80"/>`
      + `<defs><linearGradient id="fsg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${b.accent}" stop-opacity="0.42"/><stop offset="50%" stop-color="${b.accent2}" stop-opacity="0.30"/><stop offset="100%" stop-color="#000" stop-opacity="0.45"/></linearGradient></defs>`
  ...
  const OFFER_TOP = o.festivalMode ? h * 0.612 : h * 0.495;
  ...
  const featTop = o.festivalMode ? h * 0.685 : h * 0.658;
  ...
  const bankTop = featTop + featResult.height + (o.festivalMode ? h * 0.020 : h * 0.030);   // ⚠️ थोड़ा और नीचे
  const bankHeadY = bankTop;
  const bankCh = o.festivalMode ? h * 0.025 : h * 0.028;
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.cutout === false) veh = await sharp(photoPath).resize(vehW, vehH, { fit: "inside" }).png().toBuffer();
      else veh = await removeWhiteBg(photoPath, vehW, vehH);
    } catch (_) {}
    if (veh) {
      ...
      const top = Math.round(o.festivalMode ? h * 0.345 : h * 0.245), left = Math.round((w - vw) / 2);
      ...
      const badgeY = o.festivalMode ? Math.round(h * 0.532) : top + vh * 0.24;
