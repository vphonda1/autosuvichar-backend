  ...
  if (type === "vigyapan") {
    try {
      ...
      if (v) {
        ...
        let rangeStart = "";
        try {
          // मॉडल-परिवार पहचानो: नाम में से अंक तक का हिस्सा (Shine 100,
          // Activa 110, SP 125) — variant शब्द (STD/DX/DLX) हटाकर
          const famRaw = String(v.name || "").replace(/\b(std|dx|dlx|ae|drum|disc|2b|standard|deluxe)\b/gi, "").trim();
          const baseWord = famRaw.split(/\s+/)[0] || String(v.name || "").split(/\s+/)[0];
          if (baseWord) {
            ...
            const stds = siblings.filter(isStd).map((s) => num(s.exShowroom)).filter((n) => !isNaN(n) && n > 0);
            const all = siblings.map((s) => num(s.exShowroom)).filter((n) => !isNaN(n) && n > 0);
            const mine = num(v.exShowroom);
            let pick = stds.length ? Math.min(...stds) : (all.length >= 2 ? Math.min(...all) : 0);
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  ${o.festivalLine ? (() => {
    ...
    const minFs = Math.round(w / 48);
    while (fs > minFs && line.length * fs * 0.50 > safeW) fs -= 1;
    return `<text x="${w * 0.5}" y="${h * 0.045}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="700" fill="${b.accent}">✦ ${esc(line)} ✦</text>`;
  })() : ""}
  ...
  ${(() => {
    ...
    return `<text x="${w * 0.5}" y="${h * 0.105}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="800" fill="${p.textDark}">${esc(modelName)}</text>`;
  })()}
  <rect x="${w * 0.5 - w * 0.09}" y="${h * 0.12}" width="${w * 0.18}" height="6" rx="3" fill="${b.accent}"/>
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.cutout === false) veh = await sharp(photoPath).resize(Math.round(w * 0.64), Math.round(h * 0.36), { fit: "inside" }).png().toBuffer();
      else veh = await removeWhiteBg(photoPath, Math.round(w * 0.64), Math.round(h * 0.36));
    } catch (_) {}
    if (veh) {
      const meta = await sharp(veh).metadata();
      const vw = meta.width || Math.round(w * 0.64), vh = meta.height || Math.round(h * 0.36);
      ...
      const top = Math.round(h * 0.145), left = Math.round((w - vw) / 2);
      ...
      const mkBadge = (bw2, head, big, foot, isGift) => {
        const fsBig = Math.round(bw2 * 0.17), fsSml = Math.round(bw2 * 0.115);
        return `<svg width="${Math.round(bw2)}" height="${Math.round(badgeH)}" xmlns="http://www.w3.org/2000/svg">
          <rect x="3" y="3" width="${bw2 - 6}" height="${badgeH - 6}" rx="16" fill="#141414" fill-opacity="0.94" stroke="#ffd400" stroke-width="3"/>
          <text x="${bw2 / 2}" y="${badgeH * 0.28}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsSml}" font-weight="700" fill="#fff">${isGift ? "🎁 " : ""}${esc(head)}</text>
          <line x1="${bw2 * 0.20}" y1="${badgeH * 0.37}" x2="${bw2 * 0.80}" y2="${badgeH * 0.37}" stroke="#ffd400" stroke-width="2" opacity="0.6"/>
          <text x="${bw2 / 2}" y="${badgeH * 0.62}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsBig}" font-weight="800" fill="#ffd400">${esc(big)}</text>
          <text x="${bw2 / 2}" y="${badgeH * 0.82}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsSml}" font-weight="700" fill="#fff">${esc(foot)}</text>
        </svg>`;
      };
      const sideParts = [];
      // दाएँ — लोन अवधि
