...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  const gold2 = "#ffd400";
  // हर बार नया प्रोफेशनल लुक — 5 designs में से random
  const vnt = Math.floor(Math.random() * 5);
  let vDecor = "";
  if (vnt === 0) { // gold-border glass card + कोनों पर डबल-कोने
  ...
  } else if (vnt === 1) { // बड़े quote marks (सुविचार-स्टाइल)
  ...
  } else if (vnt === 2) { // sunburst किरणें (त्यौहार-स्टाइल)
    let rays = "";
  ...
  } else if (vnt === 3) { // तिरछी modern पट्टियाँ + side gold bars
  ...
  } else if (vnt === 4) { // कोनों पर मंडला-आर्क + dotted ring
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v31-ai-designs", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
