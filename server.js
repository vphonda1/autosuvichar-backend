...
async function inferVehicleFeatures(brandId, vehicleName) {
  ...
  const prompt = `"${vehicleName}" ${b.oem || b.name} कंपनी की एक ${b.vehicleWord || "गाड़ी"} है। ` +
    `इसकी 7-8 आम खूबियाँ (features) हिंदी में, बहुत छोटे-छोटे (2-3 शब्द के) टैग के रूप में बताओ, comma से अलग करके। ` +
    `जैसे: हाई माइलेज, LED हेडलाइट, डिस्क ब्रेक, ट्यूबलेस टायर, सेल्फ स्टार्ट, डिजिटल मीटर, मोबाइल चार्जिंग। ` +
    `सिर्फ comma-separated list दो — कोई नंबरिंग नहीं, कोई व्याख्या नहीं, कुछ और मत लिखो।`;
  try {
    const r = await AI.text(prompt, { temperature: 0.6, maxTokens: 150, timeout: 12000 });
    if (r.ok && r.text) {
      const feats = r.text.split(",").map((s) => s.trim().replace(/^[-•\d.]+\s*/, "")).filter(Boolean).slice(0, 9);
...
async function pickVehicleForAd(brandId, hint) {
  ...
  const sel = "name variant photoData exShowroom downPayment cashback features";
  let v = null;
  if (hint) {
  ...
  if (type === "vigyapan") {
    try {
      ...
      if (v) {
        ...
        //     screenshot में दिखा)। अब अगर features कम/ख़ाली हों, AI ख़ुद
        //     गाड़ी के नाम से पहचान कर सामान्य खूबियाँ बना देता है — और
        //     एक बार बनने के बाद डेटाबेस में हमेशा के लिए सेव हो जाती
        //     हैं, अगली बार दोबारा AI नहीं बुलानी पड़ती।
        let vFeatures = v.features || [];
        if (vFeatures.length < 3) {
          const inferred = await inferVehicleFeatures(brandId, [v.name, v.variant].filter(Boolean).join(" "));
          if (inferred.length) {
            vFeatures = inferred;
            Vehicle.findByIdAndUpdate(v._id, { features: inferred }).catch((e) =>
        ...
        const o = {
          ...
          // ⚠️ पहले सिर्फ़ 4 तक सीमित था — मैन्युअल editor जैसा 9 तक
          //    दिखाने के लिए बढ़ाया (buildPromoSVG अब ख़ुद अपने-आप कई
          //    पंक्तियों में wrap कर लेता है, चाहे कितने भी हों)
          features: vFeatures.slice(0, 9),
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  // ⚠️ पहले सिर्फ़ 4 features तक सीमित था — मैन्युअल editor जैसा 9 तक
  //    दिखाने के लिए अब यह सीमा हटाई। असली गिनती database में जो भी
  //    गाड़ी में भरी हो, उतनी ही दिखेगी (ज़्यादा से ज़्यादा 9)।
  const feats = (o.features || []).slice(0, 9);
  ...
  //  ⚠️ नीचे का पूरा हिस्सा (price/offer/features/banks) पहले सिर्फ़ 1
  //     feature-पंक्ति मान के, fixed जगह पर लिखा था। अब features कई
  //     पंक्तियाँ ले सकते हैं (9 तक), इसलिए यहीं हिसाब लगाकर तय करते हैं
  //     कि features कहाँ से शुरू हों, कितनी जगह लें, और banks उसके ठीक
  //     नीचे — कोई भी हिस्सा एक-दूसरे पर या addrBar (0.845h) पर न चढ़े।
  const priceY = h * 0.60, offerHeadY = h * 0.585, dpY = h * 0.598, cbY = h * 0.648;
  const featTop = h * 0.705;
  const featCh = h * 0.028, featGap = featCh * 0.30;
  const featResult = chipRowWrap(feats, featTop, featCh, w * 0.94, w, featGap);

  const bankTop = featTop + featResult.height + h * 0.014;
  const bankHeadY = bankTop;
  const bankCh = h * 0.022, bankGap = bankCh * 0.30;
  const bankResult = (o.banks && o.banks.length) || (b.financePartners || []).length
    ? chipRowWrap((o.banks && o.banks.length) ? o.banks : (b.financePartners || []), bankTop + h * 0.020, bankCh, w * 0.94, w, bankGap)
    : { svg: "", rows: 0, height: 0 };
  ...
  <!-- ⚠️ पहले यह बिल्कुल बाएँ किनारे से चिपका था (x=5%, कोई center-alignment
       नहीं) — इसीलिए लंबे नाम कट जाते और logo से टकरा जाते थे, जैसा
       screenshot में दिखा ("125 STD" कटा हुआ, logo से सटा हुआ)। logo दोनों
       तरफ़ ~13% जगह लेते हैं (composeLogos देखें), इसलिए बीच के 68% में ही
       title रखा है, और लंबा नाम हो तो font अपने आप छोटा हो जाता है। -->
  ${(() => {
  ...
  <!-- ex-showroom price (3D बटन) — ⚠️ पहले price ख़ाली होने पर भी यह बॉक्स
       बनता था, सिर्फ़ "₹" (बिना किसी अंक के) दिखता — जैसा screenshot में
       दिखा। अब price ना हो तो साफ़ "कॉल करके पूछें" दिखे, अकेला ₹ नहीं। -->
  ${btn3d(w * 0.05, priceY, w * 0.42, h * 0.065, "एक्स-शोरूम", o.price ? "₹" + esc(o.price) : "कॉल करें", "#141414", b.accent, "#fff", Math.round(w / 42), Math.round(w / 22))}
  <!-- offers (दाएँ, 3D बटन) -->
  <text x="${w * 0.52}" y="${offerHeadY}" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 34)}" font-weight="800" fill="${p.textDark}">लिमिटेड पीरियड ऑफर</text>
  ${o.downPayment ? btn3d(w * 0.52, dpY, w * 0.43, h * 0.045, "डाउन पेमेंट", "₹" + esc(o.downPayment), "#141414", "#ffd400", "#b00018", Math.round(w / 50), Math.round(w / 32)) : ""}
  ${o.cashback ? btn3d(w * 0.52, cbY, w * 0.43, h * 0.045, "कैशबैक", "₹" + esc(o.cashback), "#141414", b.accent, "#fff", Math.round(w / 50), Math.round(w / 32)) : ""}
  <!-- feature चिप-बटन — ⚠️ अब ज़रूरत पड़ने पर अपने-आप अगली पंक्ति में जाते
       हैं (chipRowWrap), मैन्युअल जैसे 9 तक साफ़-सुथरे दिखते हैं -->
  ${featResult.svg}
  <!-- finance/bank चिप-बटन — features कितनी पंक्तियाँ लें, उसके ठीक नीचे -->
  ${bankResult.rows ? `<text x="${w * 0.5}" y="${bankHeadY}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 52)}" font-weight="800" fill="#ffd400">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>${bankResult.svg}` : ""}
  <!-- आकर्षक address बटन -->
  ${addrBar(b, w, h)}
