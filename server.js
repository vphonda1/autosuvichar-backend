...
async function loadSettings() {
  ...
  console.log("[loadSettings]", docs.map(d => `${d.brand}:${Object.keys(d.creds||{}).join(",")}`).join(" | ") || "NO DOCS IN DB");
}
...
async function publish(item) {
  await loadSettings(); // हर publish पर fresh load — cache stale न रहे
  const c = brandCreds(item.brand);
  console.log("[publish-debug]", item.brand, { fbPageId: c.fbPageId ? "SET" : "EMPTY", fbToken: c.fbToken ? "SET" : "EMPTY", igUserId: c.igUserId ? "SET" : "EMPTY", waPhoneId: c.waPhoneId ? "SET" : "EMPTY", waToken: c.waToken ? "SET" : "EMPTY", cacheKeys: Object.keys(SETTINGS_CACHE[item.brand] || {}) });
