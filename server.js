...
function buildPromoSVG(brandId, o, w, h) {
  ...
  <!-- feature strip + finance/bank line -->
  <rect x="0" y="${h * 0.762}" width="${w}" height="${h * 0.040}" fill="${b.accent}"/>
  <text x="${w * 0.5}" y="${h * 0.79}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 48)}" font-weight="700" fill="#fff">${esc(featLine)}</text>
  <rect x="0" y="${h * 0.804}" width="${w}" height="${h * 0.040}" fill="#0d0d0d"/><rect x="0" y="${h * 0.804}" width="${w}" height="3" fill="#ffd400"/>
  <text x="${w * 0.5}" y="${h * 0.823}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 56)}" font-weight="700" fill="#ffd400">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>
  <text x="${w * 0.5}" y="${h * 0.84}" text-anchor="middle" font-family="Mukta,sans-serif" font-size="${Math.round(w / 64)}" fill="#fff">${esc(bankLine)}</text>
  <!-- आकर्षक address बटन -->
  ${addrBar(b, w, h)}
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v21-promo3d", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
