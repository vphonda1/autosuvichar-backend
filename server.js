...
async function pickVehicleForAd(brandId, hint) {
  ...
  const sel = "name variant photoData exShowroom downPayment cashback features roi exchangeBonus";
  let v = null;
  if (hint) {
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const lowerLabel = o.price2 ? "बैटरी सहित" : (hasStartFrom ? "रेंज शुरू" : (o.kmRange ? "रेंज" : ""));
  const lowerValue = o.price2 ? "₹" + esc(o.price2) + "*"
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      const mkBadge = (bw2, head, big, foot, isGift) => {
        ...
        const fsBig = Math.round(bw2 * 0.185), fsSml = Math.round(bw2 * 0.115);
