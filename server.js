...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v50-whatsapp-approval", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), logos: availableLogos(), storage: diskUsage(), features: ["voice", "adapter", "variants", "news", "provider-agnostic", "retry", "audit", "cleanup", "rate-limit", "batch", "triggers", "insights", "creative-video", "mcp", "daily-engine", "auto-video", "video-header", "wa-approval", "monthly-report"], ai: AI.status(), ownerLogo: fs.existsSync(path.join(LOGO_DIR, OWNER_LOGO_FILE)), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
...
app.post("/api/lead", rateLimit({ windowMs: 60000, max: 5, key: "lead", message: "बहुत तेज़ — एक मिनट रुककर दोबारा भेजें" }), async (req, res) => { // PUBLIC
  try {
    ...
    if (BRANDS[brand]) fireTriggerAsync("new_lead", brand, { summary: `${name || mobile} को ${vehicleInterest || "गाड़ी"} में रुचि` });
