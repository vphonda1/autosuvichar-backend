...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const bankTop = featTop + featResult.height + (o.festivalMode ? h * 0.004 : h * 0.030);   // ⚠️ थोड़ा और नीचे
  const bankHeadY = bankTop;
  const bankCh = o.festivalMode ? h * 0.021 : h * 0.028;
  ...
  const bankResult = fullBanks.length
    ? chipRowWrap(fullBanks, bankTop + (o.festivalMode ? h * 0.010 : h * 0.024), bankCh, w * 0.965, w, bankGap)
    : { svg: "", rows: 0, height: 0 };
