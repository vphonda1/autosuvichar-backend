...
async function compressImage(buf, maxKB = 400) {
  ...
  try {
    const j = await sharp(buf).jpeg({ quality: 82, mozjpeg: true }).toBuffer();
    if (j.length < maxKB * 1024) return { buf: j, ext: "jpg", mime: "image/jpeg" };
    // बड़ा है — quality 60 try करो
    const j2 = await sharp(buf).jpeg({ quality: 60, mozjpeg: true }).toBuffer();
...
async function generateImages(brandId, id, text, type, opts) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    { const rawBuf = await sharp(base).png().toBuffer(); const { buf: cBuf } = await compressImage(rawBuf); fs.writeFileSync(path.join(OUT_DIR, f), cBuf); }
    out[k] = `/generated/${f}`;
...
async function sendWhatsApp(c, item) {
  ...
  for (const to of c.waRecipients) {
    const r = await fetch(`${GRAPH}/${c.waPhoneId}/messages`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${waToken}` }, body: JSON.stringify({ messaging_product: "whatsapp", to, type: "image", image: { link: item.images.square, caption: item.text } }) });
...
if (ENABLE_CRON) {
  // ── CRON 1: सुबह 10 बजे — सुविचार / त्यौहार (तीनों brands) ──
  cron.schedule("0 10 * * *", async () => {
    const today = new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" });
    const fest = FESTIVALS.find((f) => f.date === today);
    log("INFO", "[CRON-10AM] सुविचार/त्यौहार generate", { today, festival: fest?.name || null });
    for (const brand of Object.keys(BRANDS)) {
      try {
        ...
        doc.status = ok.length ? "sent" : "failed";
        doc.channels = ok; doc.sentAt = new Date(); doc.results = results;
        await doc.save();
        if (!ok.length) {
          await notify("post_failed", `${BRANDS[brand].name}: सुविचार auto-post fail — ${results.map(r=>r.platform+":"+r.error).join(", ")}`, brand);
        } else {
          log("INFO", `[CRON-10AM] ${brand} → sent: ${ok.join(", ")}`);
        }
      } catch (e) { 
        log("ERROR", "[CRON-10AM] failed", { brand, msg: e.message });
        await notify("post_failed", `${BRANDS[brand]?.name||brand}: सुबह 10 बजे auto-post error — ${e.message}`, brand);
      }
    }
  }, { timezone: "Asia/Kolkata" });

  // ── CRON 2: सुबह 11 बजे — विज्ञापन (तीनों brands) ──
  cron.schedule("0 11 * * *", async () => {
    log("INFO", "[CRON-11AM] विज्ञापन generate");
    for (const brand of Object.keys(BRANDS)) {
      try {
        const doc = await genToPending(brand, "promo", "");
        const results = await publish({ ...doc.toObject(), platforms: { fb: true, ig: true, wa: false, yt: false } });
        const ok = results.filter(r => r.ok).map(r => r.platform);
        doc.status = ok.length ? "sent" : "failed";
        doc.channels = ok; doc.sentAt = new Date(); doc.results = results;
        await doc.save();
        if (!ok.length) {
          await notify("post_failed", `${BRANDS[brand].name}: विज्ञापन auto-post fail — ${results.map(r=>r.platform+":"+r.error).join(", ")}`, brand);
        } else {
          log("INFO", `[CRON-11AM] ${brand} विज्ञापन → sent: ${ok.join(", ")}`);
        }
      } catch (e) {
        log("ERROR", "[CRON-11AM] failed", { brand, msg: e.message });
        await notify("post_failed", `${BRANDS[brand]?.name||brand}: विज्ञापन auto-post error — ${e.message}`, brand);
      }
    }
  }, { timezone: "Asia/Kolkata" });

  // ── CRON 3: शाम 8 बजे — pending Delivery photos auto-share ──
  cron.schedule("0 20 * * *", async () => {
    log("INFO", "[CRON-8PM] Delivery auto-share");
    try {
      ...
      for (const d of pendingDeliveries) {
        try {
          ...
          if (!ok.length) {
            await notify("post_failed", `${BRANDS[d.brand]?.name||d.brand}: डिलीवरी photo auto-share fail — Manual share करें`, d.brand);
          } else {
            log("INFO", `[CRON-8PM] Delivery ${d._id} → sent: ${ok.join(", ")}`);
          }
        } catch (e) {
          log("ERROR", "[CRON-8PM] delivery failed", { id: d._id, msg: e.message });
          await notify("post_failed", `डिलीवरी photo share error — ${e.message} — Manual share करें`, d.brand || null);
        }
      }
      if (!pendingDeliveries.length) log("INFO", "[CRON-8PM] कोई pending delivery नहीं");
    } catch (e) { log("ERROR", "[CRON-8PM] error", { msg: e.message }); }
  }, { timezone: "Asia/Kolkata" });

  log("INFO", "CRON jobs: 10AM सुविचार | 11AM विज्ञापन | 8PM Delivery");
}
