...
const Content = model("Content", new mongoose.Schema({
  ...
  promo: { model: String, price: String, downPayment: String, cashback: String, features: [String], bg: String, cutout: Boolean, aiPrompt: String, offer: String, sticker: String, photo: String },
...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${tspans}
    <!-- stylish address bar (हर poster में सबसे नीचे) -->
    <rect x="0" y="${h * 0.88}" width="${w}" height="${h * 0.12}" fill="#111"/>
    <rect x="0" y="${h * 0.88}" width="${w}" height="5" fill="${festive ? gold : b.accent}"/>
    <text x="50%" y="${h * 0.928}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${Math.round(w / 26)}" font-weight="800" fill="#fff">${esc(b.name)}  •  फ़ोन ${esc(b.phone)}</text>
    <text x="50%" y="${h * 0.965}" text-anchor="middle" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${Math.round(w / 38)}" fill="#ddd">${esc(b.place)}</text>
    ...
    ${opts && opts.sticker ? stickersSVG(opts.sticker, opts.offer, {}, w, h, [[0.80, 0.30], [0.20, 0.30], [0.80, 0.52]], w * 0.12) : ""}</svg>`);
}
...
// कई offers → कई stickers (multi-select)
function stickersSVG(stickerStyle, offerCsv, amounts, w, h, positions, r) {
  const n = parseInt(stickerStyle || "0", 10);
  if (!n) return "";
  ...
  return offers.slice(0, positions.length).map((off, i) => {
    ...
    return buildStickerSVG(n, w * positions[i][0], h * positions[i][1], r, l1, l2);
  }).join("");
}
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  <!-- footer (dealer + phone + address, बिना emoji) -->
  <rect x="0" y="${h * 0.86}" width="${w}" height="${h * 0.14}" fill="#141414"/>
  <rect x="0" y="${h * 0.86}" width="${w}" height="5" fill="${b.accent}"/>
  <text x="${w * 0.5}" y="${h * 0.915}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 30)}" font-weight="800" fill="#fff">${esc(b.name)}  •  फ़ोन ${esc(b.phone)}</text>
  <text x="${w * 0.5}" y="${h * 0.958}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 40)}" fill="#ddd">${esc(b.place)}</text>
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v13-multi-addr", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const opts = { sticker: req.body.sticker || "", offer: req.body.offer || "" };
...
app.post("/api/promo", photoUpload.single("photo"), async (req, res) => {
  try {
    ...
    const o = { model: vmodel, price, downPayment, cashback, features, bg: bg || "light", cutout: req.body.cutout !== "false", aiPrompt: aiPrompt || "", offer: offer || "", sticker: sticker || "", photo: req.file?.filename || vehicle };
