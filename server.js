...
⚠️ ठीक ${wantPosters} items। कोई नया price/EMI/offer मत बनाओ।`,
    ...
    await activity(brandId, "generate", out.errors.length ? "warning" : "success",
      `आज का content अपने आप तैयार — ${out.posters} poster, ${out.videos} video (सब Review में)`);
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v49-logo-config", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), logos: availableLogos(), storage: diskUsage(), features: ["voice", "adapter", "variants", "news", "provider-agnostic", "retry", "audit", "cleanup", "rate-limit", "batch", "triggers", "insights", "creative-video", "mcp", "daily-engine", "auto-video", "video-header"], ai: AI.status(), ownerLogo: fs.existsSync(path.join(LOGO_DIR, OWNER_LOGO_FILE)), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
