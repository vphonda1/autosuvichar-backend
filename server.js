...
sharp.cache(false); // memory leak रोको
sharp.concurrency(1); // एक साथ एक ही image process
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    doc.images = imgs;
    if (b64.square) {
      // 2MB से बड़ा base64 MongoDB में नहीं रखो — memory leak रोको
      const sqSize = Buffer.byteLength(b64.square || "", "utf8");
      if (sqSize < 2 * 1024 * 1024) {
        doc.imageData = { square: b64.square, story: b64.story };
      } else {
        log("WARN", "imageData too large, skipping DB save", { kb: Math.round(sqSize/1024) });
      }
    }
...
app.post("/api/promo-image", outUpload.fields([{ name: "square", maxCount: 1 }, { name: "story", maxCount: 1 }]), async (req, res) => {
  try {
    ...
    try {
      ...
      const sqB64 = "data:image/png;base64," + sqBuf.toString("base64");
      if (Buffer.byteLength(sqB64, "utf8") < 2 * 1024 * 1024) {
        doc.imageData = { square: sqB64 };
