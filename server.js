      ...
      try {
        ...
        if (feats.length < 24) {
          feats = [...new Set([...feats,
            ...
            "लो मेंटेनेंस", "शानदार लुक", "मज़बूत चेसिस", "आसान हैंडलिंग"])].slice(0, 24);
        }
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const feats = (o.features || []).slice(0, 24);   // ⚠️ अब 24 — तीनों पंक्तियाँ भरी रहें
  ...
  const featCh = o.festivalMode ? h * 0.018 : h * 0.024;
  ...
  const bankTop = featTop + featResult.height + (o.festivalMode ? h * 0.010 : h * 0.030);   // ⚠️ थोड़ा और नीचे
  const bankHeadY = bankTop;
  const bankCh = o.festivalMode ? h * 0.019 : h * 0.028;
  const bankGap = bankCh * 0.28;
  const bankResult = fullBanks.length
    ? chipRowWrap(fullBanks, bankTop + h * 0.024, bankCh, w * 0.965, w, bankGap)
    : { svg: "", rows: 0, height: 0 };
