...
async function generateImages(brandId, id, text, type, opts) {
  ...
  if (SAJA.includes(type)) {
    ...
    if (fleetBuf) {
      try {
        const festName = opts?.festival || detectFestivalName(text) || "";
        ...
        const o = {
          ...
          cashback: stripRs(b0.festCashback) || "",
          downPayment: "", exchangeBonus: "", roi: "",
          features: feats,
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      if (o.festivalMode && o.festArt) {
        try {
          const md = Math.round(Math.min(w * 0.19, vh * 0.92));
          const ring = Math.max(5, Math.round(md * 0.045));
          ...
          const myY = Math.round(top - vh * 0.06);
