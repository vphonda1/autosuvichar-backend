...
async function loadSettings() {
  ...
  console.log("[loadSettings]", Object.keys(SETTINGS_CACHE).map(b => `${b}: keys=${Object.keys(SETTINGS_CACHE[b]||{}).join(",")}`).join(" | "));
}
...
async function sendWhatsApp(c, item) {
  const waToken = process.env.WA_TOKEN || process.env[`${item.brand.toUpperCase()}_WA_TOKEN`] || brandCreds(item.brand).waToken || "";
  if (!c.waPhoneId || !waToken) throw new Error("WA creds missing");
...
async function publish(item) {
  ...
  console.log("[publish]", item.brand, JSON.stringify({ hasFbPageId: !!c.fbPageId, hasFbToken: !!c.fbToken, hasWaPhoneId: !!c.waPhoneId, hasWaToken: !!c.waToken, platforms: item.platforms }));
...
app.get("/api/settings", requireRole("super-admin", "admin"), async (req, res) => {
  ...
  for (const id of Object.keys(BRANDS)) {
    ...
    out[id] = { // tokens masked
      ...
      ytRefreshToken: c.ytRefreshToken ? "••••set" : "", waPhoneId: c.waPhoneId || "", waToken: c.waToken ? "••••set" : "", waRecipients: c.waRecipients,
    };
  }
  res.json(out);
});
