...
const Content = model("Content", new mongoose.Schema({
  ...
  promo: { model: String, price: String, downPayment: String, cashback: String, features: [String], bg: String, cutout: Boolean, aiPrompt: String, photo: String },
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v8-ai-bg", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
...
app.post("/api/promo", photoUpload.single("photo"), async (req, res) => {
  try {
    const { brand, model: vmodel, price, downPayment, cashback, bg, vehicle, aiPrompt } = req.body;
    if (!BRANDS[brand]) return res.status(400).json({ error: "invalid brand" });
    ...
    const o = { model: vmodel, price, downPayment, cashback, features, bg: bg || "light", cutout: req.body.cutout !== "false", aiPrompt: aiPrompt || "", photo: req.file?.filename || vehicle };
