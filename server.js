...
async function inferVehicleFeatures(brandId, vehicleName) {
  ...
  const prompt = `"${vehicleName}" ${b.oem || b.name} कंपनी की एक ${b.vehicleWord || "गाड़ी"} है। ` +
    `इसकी 11-12 आम खूबियाँ (features) हिंदी में, बहुत छोटे-छोटे (2-3 शब्द के) टैग के रूप में बताओ, comma से अलग करके। ` +
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const feats = (o.features || []).slice(0, 12);
  ...
  const featCh = h * 0.026, featGap = featCh * 0.28;
  ...
  const bankTop = featTop + featResult.height + h * 0.016;
  ...
  <text x="${w * 0.5}" y="${OFFER_HEAD_Y}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 30)}" font-weight="800" fill="#ffd400">✦ लिमिटेड पीरियड ऑफर ✦</text>
  ...
  ${bankResult.rows ? `<text x="${w * 0.5}" y="${bankHeadY}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 48)}" font-weight="800" fill="#ffd400">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>${bankResult.svg}` : ""}
  <!-- आकर्षक address बटन -->
  ${addrBar(b, w, h)}
