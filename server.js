...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v46-complete", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), storage: diskUsage(), features: ["voice", "adapter", "variants", "news", "provider-agnostic", "retry", "audit", "cleanup", "rate-limit", "batch", "triggers", "insights", "creative-video", "mcp"], ai: AI.status(), ownerLogo: fs.existsSync(path.join(LOGO_DIR, OWNER_LOGO_FILE)), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
