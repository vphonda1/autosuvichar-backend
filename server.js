...
const Content = model("Content", new mongoose.Schema({
  ...
  promo: { model: String, price: String, downPayment: String, cashback: String, features: [String], bg: String, cutout: Boolean, aiPrompt: String, offer: String, photo: String },
...
function offerSeal(o, w, h, accent) {
  const def = OFFERS[o.offer];
  if (!def) return "";
  const l1 = def.l1;
  const l2 = def.amt && o[def.amt] ? "₹" + o[def.amt] : (def.l2def || "ऑफर");
  const cx = w * 0.82, cy = h * 0.30, r = w * 0.11, fs = w / 28, pts = 24;
  let poly = "";
  for (let i = 0; i < pts * 2; i++) { const a = (Math.PI / pts) * i; const rad = i % 2 === 0 ? r : r * 0.82; poly += `${cx + rad * Math.cos(a)},${cy + rad * Math.sin(a)} `; }
  return `<polygon points="${poly}" fill="${accent}" stroke="#ffd400" stroke-width="4"/>` +
    `<text x="${cx}" y="${cy - fs * 0.15}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="800" fill="#fff">${esc(l1)}</text>` +
    `<text x="${cx}" y="${cy + fs * 1.05}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs * 1.2}" font-weight="800" fill="#fff">${esc(l2)}</text>`;
}
function buildPromoSVG(brandId, o, w, h) {
  ...
  // readability scrims (AI background पर text साफ़ दिखे)
  const scrim = p.scrim ? `<defs><linearGradient id="st" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#000" stop-opacity="0.6"/><stop offset="100%" stop-color="#000" stop-opacity="0"/></linearGradient><linearGradient id="sb" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#000" stop-opacity="0"/><stop offset="100%" stop-color="#000" stop-opacity="0.7"/></linearGradient></defs><rect x="0" y="0" width="${w}" height="${h * 0.26}" fill="url(#st)"/><rect x="0" y="${h * 0.62}" width="${w}" height="${h * 0.38}" fill="url(#sb)"/>` : "";
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
  ${bgRect}
  ${scrim}
  ...
  <text x="${w * 0.96}" y="${h * 0.05}" text-anchor="end" font-family="Mukta,sans-serif" font-size="${Math.round(w / 60)}" fill="#ccc">AI Generated</text>
  ${offerSeal(o, w, h, b.accent)}
  </svg>`);
}
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    // गाड़ी की फोटो — background पर सीधे (cutout से सफ़ेद हटता है)
    let veh = null;
    try {
      if (o.cutout === false) veh = await sharp(photoPath).resize(Math.round(w * 0.64), Math.round(h * 0.40), { fit: "inside" }).png().toBuffer();
      else veh = await removeWhiteBg(photoPath, Math.round(w * 0.64), Math.round(h * 0.40));
    } catch (_) {}
    if (veh) {
      const meta = await sharp(veh).metadata();
      const vw = meta.width || Math.round(w * 0.64), vh = meta.height || Math.round(h * 0.40);
      const top = Math.round(h * 0.20), left = Math.round((w - vw) / 2);
      const shW = Math.round(vw * 0.85), shH = Math.max(18, Math.round(vh * 0.14));
      const shadow = await sharp(Buffer.from(`<svg width="${shW}" height="${shH}"><ellipse cx="${shW / 2}" cy="${shH / 2}" rx="${shW / 2}" ry="${shH / 2}" fill="#000" fill-opacity="0.28"/></svg>`)).blur(12).png().toBuffer();
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v10-relative-img", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
...
app.post("/api/promo", photoUpload.single("photo"), async (req, res) => {
  try {
    const { brand, model: vmodel, price, downPayment, cashback, bg, vehicle, aiPrompt, offer } = req.body;
    if (!BRANDS[brand]) return res.status(400).json({ error: "invalid brand" });
    ...
    const o = { model: vmodel, price, downPayment, cashback, features, bg: bg || "light", cutout: req.body.cutout !== "false", aiPrompt: aiPrompt || "", offer: offer || "", photo: req.file?.filename || vehicle };
