...
function buildPromoSVG(brandId, o, w, h) {
  ...
  <!-- feature चिप-बटन + finance/bank चिप-बटन (हर एक अलग रंग) -->
  ${chipRow(feats.length ? feats : ["आसान EMI", "एक्सचेंज बोनस", "फाइनेंस सुविधा"], h * 0.752, h * 0.036, w * 0.94, w)}
  <text x="${w * 0.5}" y="${h * 0.80}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${Math.round(w / 52)}" font-weight="800" fill="#ffd400">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>
  ${chipRow((o.banks && o.banks.length) ? o.banks : ["HDB", "IDFC First", "Shriram Finance", "Bajaj Finance"], h * 0.808, h * 0.030, w * 0.94, w)}
  <!-- आकर्षक address बटन -->
  ${addrBar(b, w, h)}
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v22-chips", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS), aiImageKey: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), aiTextKey: !!process.env.OPENAI_API_KEY }));
