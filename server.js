...
function buildPromoSVG(brandId, o, w, h) {
  ...
  const OFFER_TOP = h * 0.48, OFFER_H = h * 0.18;
  const rightBoxes = [];
  ...
  const rightBoxesSVG = rightBoxes.map((rb, i) => {
    ...
    return btn3d(w * 0.52, y, w * 0.43, rbH, rb.label, rb.value, rb.dark, rb.accent, rb.txt, Math.round(w / 52), Math.round(w / 34));
  }).join("\n  ");
  ...
  ${(() => {
    // ⚠️ लंबी क़ीमत (जैसे "1,25,999") भी box के अंदर आराम से आ जाए —
    //    छोटी क़ीमत के लिए बड़ा, बड़ी क़ीमत के लिए अपने-आप छोटा font
    const priceTxt = o.price ? "₹" + o.price : "कॉल करें";
    let vFs = Math.round(w / 16);
    const minVFs = Math.round(w / 22);
    while (vFs > minVFs && priceTxt.length * vFs * 0.56 > w * 0.19) vFs -= 2;
    return btn3d(w * 0.05, OFFER_TOP, w * 0.42, OFFER_H, "एक्स-शोरूम", esc(priceTxt), "#141414", b.accent, "#fff", Math.round(w / 34), vFs);
  })()}
