...
function buildPromoSVG(brandId, o, w, h) {
  ...
  //     साफ़ पढ़ा जाए।
  if (o.festArt) {
    bgRect = `<rect width="${w}" height="${h}" fill="#000" fill-opacity="0.52"/>`
      + `<defs><linearGradient id="fsg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${b.accent2}" stop-opacity="0.55"/><stop offset="45%" stop-color="#000" stop-opacity="0.25"/><stop offset="100%" stop-color="${b.accent2}" stop-opacity="0.62"/></linearGradient></defs>`
  ...
  const OFFER_HEAD_Y = o.festivalMode ? h * 0.578 : h * 0.472;
  const OFFER_TOP = o.festivalMode ? h * 0.598 : h * 0.495;
  ...
  const featTop = o.festivalMode ? h * 0.663 : h * 0.658;
  ...
  const featCh = o.festivalMode ? h * 0.021 : h * 0.024;
  ...
  const featResult = chipRowWrap(feats, featTop, featCh, o.festivalMode ? w * 0.80 : w * 0.965, w, featGap);
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      const vehW = o.festivalMode ? Math.round(w * 0.94) : Math.round(w * 0.64);
      const vehH = o.festivalMode ? Math.round(h * 0.155) : Math.round(h * 0.215);
    ...
    if (veh) {
      ...
      const top = Math.round(o.festivalMode ? h * 0.305 : h * 0.245), left = Math.round((w - vw) / 2);
      ...
      const badgeY = o.festivalMode ? Math.round(h * 0.468) : top + vh * 0.24;
      ...
      if (o.festivalMode && o.festArt) {
        try {
          const md = Math.round(o.festivalMode ? w * 0.19 : Math.min(w * 0.215, vh * 1.0));
          ...
          const myY = Math.round(o.festivalMode ? h * 0.175 : top - vh * 0.18);
