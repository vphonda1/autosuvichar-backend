...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    try {
      ...
      if (o.festivalMode) {
        veh = o.cutout === false
          ? await sharp(photoPath).resize({ width: vehW }).png().toBuffer()
          : await removeWhiteBg(photoPath, vehW, Math.round(h * 0.42));
    ...
    if (veh) {
      ...
      const top = Math.round(o.festivalMode ? h * 0.318 : h * 0.245), left = Math.round((w - vw) / 2);
      ...
      const badgeY = o.festivalMode ? Math.round(h * 0.562) : top + vh * 0.24;
