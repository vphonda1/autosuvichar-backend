...
const vehPhotoUpload = multer({
  storage: multer.diskStorage({
    destination: (req, f, cb) => {
      const d = path.join(VEHICLE_DIR, req.vehBrand || "vp_honda");
      fs.mkdirSync(d, { recursive: true });
      cb(null, d);
    },
    filename: (req, f, cb) => {
      const ext = (path.extname(f.originalname) || ".jpg").toLowerCase();
      cb(null, `v${req.params.id}_${Date.now()}${ext}`);
    },
  }),
...
app.post("/api/vehicles/:id/photo", requireRole("super-admin", "admin"),
  attachVehBrand, vehPhotoUpload.single("photo"), async (req, res) => {
    try {
      ...
      // ⚠️ बड़ी photo poster में देर लगाती है और phone की memory खाती है।
      //    1200px से बड़ी हो तो छोटी कर दो — दिखने में फ़र्क़ नहीं पड़ता।
      const full = req.file.path;
      try {
        const meta = await sharp(full).metadata();
        if (meta.width > 1200 || meta.height > 1200) {
          const buf = await sharp(full).resize(1200, 1200, { fit: "inside", withoutEnlargement: true })
            .jpeg({ quality: 88 }).toBuffer();
          fs.writeFileSync(full, buf);
        }
      } catch (_) {}

      const url = `/vehicles/${req.vehBrand}/${req.file.filename}`;
      const doc = await Vehicle.findByIdAndUpdate(req.params.id, { imageUrl: url }, { new: true });

      // पुरानी photo हटा दो — एक गाड़ी की एक ही photo
      if (req.vehOld && req.vehOld !== url) {
        try { fs.unlinkSync(path.join(__dirname, "public", req.vehOld.replace(/^\//, ""))); } catch (_) {}
      }
      log("INFO", "[VEHICLE] photo लगी", { name: doc.name, url });
      res.json({ ok: true, imageUrl: url, doc });
...
app.delete("/api/vehicles/:id/photo", requireRole("super-admin", "admin"), attachVehBrand, async (req, res) => {
  try {
    if (req.vehOld) { try { fs.unlinkSync(path.join(__dirname, "public", req.vehOld.replace(/^\//, ""))); } catch (_) {} }
    await Vehicle.findByIdAndUpdate(req.params.id, { imageUrl: "" });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/api/vehicles/:id/kit", async (req, res) => {
  try {
    ...
    res.json({
      ...
      category: v.category || "",
      photo: v.imageUrl ? PUBLIC_URL + v.imageUrl : "",
