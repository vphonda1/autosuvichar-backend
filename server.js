  ...
  if (type === "vigyapan") {
    try {
      ...
      if (v) {
        ...
        // ⚠️ नया — "रेंज की शुरुआत ₹X से" — असली database से, उसी मॉडल-लाइन
        //    (जैसे "SP125" के सारे variant — STD/DLX/DLX2B वग़ैरह) में जो
        //    सबसे सस्ता variant मिले, उसकी क़ीमत। मनगढ़ंत आँकड़ा नहीं।
        let rangeStart = "";
        try {
          const baseWord = String(v.name || "").split(/\s+/)[0];
          if (baseWord) {
            const siblings = await Vehicle.find({
              brand: brandId, active: true, name: { $regex: baseWord, $options: "i" },
              exShowroom: { $nin: [null, ""] },
            }).select("exShowroom").lean();
            const prices = siblings.map((s) => parseInt(String(s.exShowroom).replace(/[^\d]/g, ""), 10)).filter((n) => !isNaN(n) && n > 0);
            if (prices.length >= 2) rangeStart = Math.min(...prices).toLocaleString("en-IN");
          }
...
function buildPromoSVG(brandId, o, w, h) {
  ...
  ${(() => {
    ...
    return `<text x="${w * 0.5}" y="${h * 0.08}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fs}" font-weight="800" fill="${p.textDark}">${esc(modelName)}</text>`;
  })()}
  <rect x="${w * 0.5 - w * 0.09}" y="${h * 0.095}" width="${w * 0.18}" height="6" rx="3" fill="${b.accent}"/>
...
async function generatePromoImages(brandId, id, o, photoPath) {
  ...
  for (const [k, [w, h]] of Object.entries(sizes)) {
    ...
    if (veh) {
      ...
      const top = Math.round(h * 0.11), left = Math.round((w - vw) / 2);
      ...
      // ⚠️ नया — "लोन की अवधि — 60 महीने" — dealer-नीति (BRANDS.loanTenure),
      //    गाड़ी के दाईं तरफ़ बची खाली जगह में — नीचे किसी और चीज़ की जगह
      //    नहीं ली, डिज़ाइन वही रहा, बस साइड की ख़ाली जगह इस्तेमाल हुई
      if (BRANDS[brandId]?.loanTenure) {
        const badgeX = Math.min(left + vw + w * 0.02, w * 0.83);
        const badgeW = Math.max(w * 0.12, w - badgeX - w * 0.03);
        if (badgeW >= w * 0.11) {
          //  ⚠️ पहले यह badge तंग और भद्दा दिखता था — 4 पंक्तियाँ बहुत कम
          //     जगह में ठूँसी थीं, "तक लोन अवधि*" आपस में सट जाता था।
          //     अब ऊँचाई बढ़ाई, पंक्तियाँ 3 कीं और हर एक को साँस लेने की
          //     जगह दी — साफ़ और भरा-भरा दिखे।
          const badgeY = top + vh * 0.24, badgeH = vh * 0.46;
          const tenure = BRANDS[brandId].loanTenure;
          const fsBig = Math.round(badgeW * 0.17), fsSml = Math.round(badgeW * 0.115);
          const badgeSvg = `<svg width="${Math.round(badgeW)}" height="${Math.round(badgeH)}" xmlns="http://www.w3.org/2000/svg">
            <rect x="3" y="3" width="${badgeW - 6}" height="${badgeH - 6}" rx="16" fill="#141414" fill-opacity="0.94" stroke="#ffd400" stroke-width="3"/>
            <text x="${badgeW / 2}" y="${badgeH * 0.30}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsSml}" font-weight="700" fill="#fff">लोन अवधि</text>
            <line x1="${badgeW * 0.22}" y1="${badgeH * 0.38}" x2="${badgeW * 0.78}" y2="${badgeH * 0.38}" stroke="#ffd400" stroke-width="2" opacity="0.6"/>
            <text x="${badgeW / 2}" y="${badgeH * 0.62}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsBig}" font-weight="800" fill="#ffd400">${esc(tenure)}</text>
            <text x="${badgeW / 2}" y="${badgeH * 0.82}" text-anchor="middle" font-family="Noto Sans Devanagari,Mukta,sans-serif" font-size="${fsSml}" font-weight="700" fill="#fff">तक*</text>
          </svg>`;
          frame = await sharp(frame).composite([{ input: Buffer.from(badgeSvg), top: Math.round(badgeY), left: Math.round(badgeX) }]).png().toBuffer();
        }
      }
    }
...
app.post("/api/command/execute", async (req, res) => {
  try {
    ...
    if (when === "now" || when === "today") {
      ...
      const imgs = await generateImages(brand, doc._id, text, finalType, vehicle ? { vehicle } : {});
      const b64 = imgs._b64 || {}; delete imgs._b64;
      doc.images = imgs;
      if (b64.square) doc.imageData = { square: b64.square, story: b64.story };
      await doc.save();
