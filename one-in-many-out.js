...
module.exports = function mountOneInManyOut(app, deps) {
  const {
    ...
    pickVehicleForAd, stripRs, toWhatsAppMarkdown,
  } = deps;
  ...
  async function build(camp) {
    ...
    if (!text) {
      ...
      try {
        text = cleanAIText
          ? cleanAIText(await generateText(brand, camp.type || "vigyapan", null, vehCtx))
          : await generateText(brand, camp.type || "vigyapan", null, vehCtx);
      } catch (e) {
        text = camp.command;
    ...
    //       गाड़ी चुन लेगा और caption से मेल नहीं खाएगी
    try {
      const imgs = await generateImages(brand, camp._id, text, camp.type || "vigyapan",
        { ...(vehDoc ? { vehicleDoc: vehDoc } : {}), offerHint: camp.offerHint || camp.command });
    ...
    try {
      ...
      const doc = await Content.create({
        ...
        whatsappText: toWhatsAppMarkdown(wa?.text || text),
        imgUrl: p?.url || "",
