...
describe("1. तीनों brands सही तरह से बने हैं", () => {
  ...
  test("🌱 Yakuza का रंग हरा है (इलेक्ट्रिक वाहन — green energy)", () => {
    const blk = SRC.slice(SRC.indexOf("yakuza: {"), SRC.indexOf("minimetro: {"));
    assert.ok(/accent:\s*"#0EA36A"/i.test(blk), "Yakuza accent हरा होना चाहिए");
    assert.ok(!/#ED1C24/i.test(blk), "लाल रंग हटना चाहिए था");
  });
...
describe("4. AI layer provider-agnostic है (PRD #36)", () => {
  ...
  test("अब कहीं भी सीधा Gemini call बचा नहीं (सिर्फ़ provider के अंदर)", () => {
    ...
    // 3 = AI provider + image helper + models-list route (यह तीनों जायज़ हैं)
    assert.ok(count("generativelanguage.googleapis.com") <= 4,
...
describe("15. ग्राहक को WhatsApp, Lead alert, बोलकर command", () => {
  test("ग्राहक को भेजने का रास्ता है", () => {
    assert.ok(has("async function sendDeliveryToCustomer"));
    assert.ok(has("customerMobile"));
  });

  test("⚠️ editor में बनी photo भी ग्राहक को जा सकती है", () => {
    // DeliveryEditor/AI Delivery की photo Content बनती है, Delivery नहीं
    const i = SRC.indexOf('model("Content"');
    const blk = SRC.slice(i, i + 2000);
    assert.ok(blk.includes("customerMobile"), "Content में भी ग्राहक का नंबर होना चाहिए");
    assert.ok(blk.includes("sentToCustomerAt"), "Content पर भी दोबारा भेजने से रोक चाहिए");
  });

  test("दोनों models के लिए send-customer route है", () => {
    assert.ok(SRC.includes('"/api/delivery/:id/send-customer"'));
    assert.ok(SRC.includes('"/api/content/:id/send-customer"'), "editor की photo के लिए भी चाहिए");
  });

  test("model का पता अंदाज़े से नहीं लगाया जाता", () => {
    const i = SRC.indexOf("async function sendDeliveryToCustomer");
    assert.ok(SRC.slice(i, i + 2000).includes("constructor?.modelName"),
      "mongoose से सीधे पूछना चाहिए");
  });

  test("editor की photo के साथ ग्राहक की जानकारी सहेजी जाती है", () => {
    const i = SRC.indexOf('app.post("/api/promo-image"');
    assert.ok(SRC.slice(i, i + 1500).includes("customerMobile"), "DeliveryEditor का रास्ता");
    const j = SRC.indexOf("async function saveEditorImage");
    assert.ok(SRC.slice(j, j + 1500).includes("customerMobile"), "AI Delivery का रास्ता");
  });

  test("हाथ से भेजने पर switch बंद हो तब भी जाता है", () => {
    const i = SRC.indexOf("async function sendCustomerHandler");
    assert.ok(SRC.slice(i, i + 900).includes("manual: true"),
      "बटन दबाया है तो भेजना चाहिए");
  });
...
// ═══════════════════════════════════════════════════════════
describe("16. Memory — Render पर crash न हो", () => {
  test("⚠️ कोई भी bulk query base64 नहीं उठाती", () => {
    // .find( जो एक से ज़्यादा docs लाती है, उसमें -imageData होना ही चाहिए
    const lines = SRC.split("\n");
    const bad = [];
    lines.forEach((ln, i) => {
      if (!/\b(Content|Delivery)\.find\(/.test(ln)) return;
      if (/countDocuments/.test(ln)) return;
      const chunk = lines.slice(i, i + 4).join(" ");
      if (!/select\(/.test(chunk)) bad.push(`line ${i + 1}: ${ln.trim().slice(0, 70)}`);
    });
    assert.strictEqual(bad.length, 0,
      "इन queries में select नहीं है — base64 RAM में आ जाएगा:\n" + bad.join("\n"));
  });

  test("8PM delivery cron पर limit है", () => {
    const i = SRC.indexOf("[CRON-8PM]");
    const blk = SRC.slice(i, i + 900);
    assert.ok(blk.includes("-imageData"), "बिना इसके सारी deliveries base64 समेत आती थीं");
    assert.ok(/\.limit\(\d+\)/.test(blk), "limit भी ज़रूरी है");
  });

  test("पुराने base64 अपने आप हटते हैं", () => {
    assert.ok(has("async function trimImageData"));
    assert.ok(has('cron.schedule("15 */6 * * *"'), "हर 6 घंटे चलना चाहिए");
  });

  test("⚠️ trim नई pending पोस्ट का base64 नहीं छूता", () => {
    const i = SRC.indexOf("async function trimImageData");
    const blk = SRC.slice(i, i + 1200);
    assert.ok(blk.includes("weekAgo"), "7 दिन की सुरक्षा चाहिए");
    assert.ok(blk.includes('$in: ["sent", "rejected", "failed"]'),
      "सिर्फ़ निपटी हुई पोस्ट का base64 हटे");
  });

  test("safePublish base64 नहीं उठाता", () => {
    const i = SRC.indexOf("async function safePublish");
    assert.ok(SRC.slice(i, i + 400).includes("-imageData"));
  });

  test("memory पर नज़र रखी जाती है", () => {
    assert.ok(has("function memoryWatch"));
    assert.ok(has("_memWarnedAt"), "बार-बार warning न आए");
  });

  test("mongoose का connection pool छोटा है", () => {
    assert.ok(has("maxPoolSize: 5"), "हर connection RAM लेता है");
  });

  test("sharp की cache बंद है", () => {
    assert.ok(has("sharp.cache(false)"));
    assert.ok(has("sharp.concurrency(1)"));
  });

  test("start script में memory की सीमा है", () => {
    const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "package.json"), "utf-8"));
    assert.ok(/--max-old-space-size/.test(pkg.scripts.start),
      "Node को बताना ज़रूरी है वरना वो 512MB पार कर जाता है");
  });
});

// ═══════════════════════════════════════════════════════════
describe("17. AI का जवाब टूटे तो अपने आप संभले", () => {
  test("⚠️ Gemini 2.5 की 'thinking' बंद है", () => {
    const i = SRC.indexOf("async gemini({ parts");
    const blk = SRC.slice(i, i + 2500);
    assert.ok(blk.includes("thinkingBudget: 0"),
      "वरना सारे tokens सोचने में चले जाते हैं और JSON अधूरा आता है");
  });

  test("जवाब बीच में कटे तो अगला model आज़माता है", () => {
    const i = SRC.indexOf("async gemini({ parts");
    assert.ok(SRC.slice(i, i + 2500).includes("MAX_TOKENS"),
      "अधूरा JSON लौटाना नहीं चाहिए");
  });

  test("JSON के लिए ज़्यादा जगह मिलती है", () => {
    const i = SRC.indexOf("async gemini({ parts");
    assert.ok(/json \? 3000 : 1200/.test(SRC.slice(i, i + 2500)));
  });

  test("⚠️ JSON टूटने पर अगला provider try होता है", () => {
    const i = SRC.indexOf("async _chain(providers");
    const blk = SRC.slice(i, i + 1500);
    assert.ok(blk.includes("validate"),
      "जाँच chain के अंदर होनी चाहिए, वरना fallback कभी नहीं चलेगा");
    assert.ok(blk.includes("continue"), "टूटा जवाब मिले तो अगला आज़माओ");
  });

  test("टूटे JSON को सुधारने की कोशिश होती है", () => {
    assert.ok(has("function parseLooseJSON"));
    const i = SRC.indexOf("function parseLooseJSON");
    const blk = SRC.slice(i, i + 1400);
    assert.ok(blk.includes("```"), "code fence हटाना चाहिए");
    assert.ok(blk.includes("lastIndexOf"), "आगे-पीछे की बात हटाकर JSON निकालना चाहिए");
  });

  test("असली टूटे जवाबों पर parseLooseJSON सही चलता है", () => {
    const i = SRC.indexOf("function parseLooseJSON");
    const j = SRC.indexOf("// ── मुख्य AI object");
    // eslint-disable-next-line no-eval
    eval(SRC.slice(i, j));
    const fn = eval("parseLooseJSON");
    assert.deepStrictEqual(fn('{"a":1}'), { a: 1 });
    assert.deepStrictEqual(fn('```json\n{"a":1}\n```'), { a: 1 });
    assert.deepStrictEqual(fn('यह रहा:\n{"a":1}\nधन्यवाद'), { a: 1 });
    assert.deepStrictEqual(fn('{"a":1,}'), { a: 1 });
    assert.strictEqual(fn("माफ़ कीजिए"), null, "कचरा null देना चाहिए ताकि अगला provider चले");
    assert.strictEqual(fn('{"a":'), null, "अधूरा भी null");
  });

  test("गड़बड़ी का message हिंदी में और काम का हो", () => {
    const i = SRC.indexOf("async json(prompt");
    assert.ok(SRC.slice(i, i + 1600).includes("थोड़ी देर बाद दोबारा कोशिश करें"),
      "'AI ने सही JSON नहीं दिया' जैसा तकनीकी message नहीं दिखना चाहिए");
  });

  test("⚠️ जो model photo नहीं देख सकता वो साफ़ मना करता है", () => {
    // वरना वो सिर्फ़ text पढ़कर photo के बारे में मनगढ़ंत जवाब दे देगा
    for (const prov of ["groq", "ollama"]) {
      const i = SRC.indexOf(`  async ${prov}({ parts`);
      const blk = SRC.slice(i, i + 700);
      assert.ok(blk.includes("p.inline_data"), `${prov} में photo की जाँच नहीं है`);
      assert.ok(blk.includes("photo नहीं देख सकता"), `${prov} को साफ़ मना करना चाहिए`);
    }
  });

  test("Groq का बंद हो चुका model इस्तेमाल नहीं होता", () => {
    // Groq ने जून 2026 में llama-3.3-70b बंद कर दिया
    assert.ok(!SRC.includes('GROQ_MODEL || "llama-3.3-70b-versatile"'),
      "बंद हो चुका model default में नहीं होना चाहिए");
  });

  test("कौन-सा AI चल रहा है — जाँचने का रास्ता है", () => {
    assert.ok(SRC.includes('"/api/ai/test"'));
    assert.ok(SRC.includes('"/api/ai/providers"'));
  });
});
