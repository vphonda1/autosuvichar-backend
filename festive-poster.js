...
module.exports = function mountFestivePoster(app, deps) {
  const {
    ...
    Vehicle, composeLogos, compressImage,
    // ⚠️ नया — "AI पृष्ठभूमि" विकल्प (गणेश चतुर्थी/नवरात्रि/दिवाली के लिए)।
    //    देवी-देवता की आकृति बनवाने की कोशिश जान-बूझकर नहीं की — AI कभी-कभी
    //    ग़लत/अनुचित अनुपात बना देता है, धार्मिक तस्वीर में यह असम्मानजनक
    //    होगा। सिर्फ़ त्यौहार का माहौल (दीये, फूल, रंगोली) बनवाया जाता है,
    //    कोई मूर्ति/आकृति नहीं। बेहतर तरीक़ा — "गाने और त्यौहार की तस्वीरें"
    //    सेटिंग से असली तस्वीर ख़ुद चढ़ाना (FestiveArt), वह इससे बेहतर दिखेगी।
    fetchAIBackground,
    // ⚠️ नया — features हाथ से टाइप करने की ज़रूरत नहीं, AI गाड़ी के नाम
    //    से पहचान कर बना देता है (देखें fetchOneVehicle)
    inferVehicleFeatures,
    // ⚠️ नया — त्यौहार वाले poster में भी गाड़ी का सफ़ेद background कटे
    removeWhiteBg,
  } = deps;
  ...
  const THEMES = {
    ...
    // ⚠️ रोज़ का विज्ञापन — इसमें त्यौहार वाली सजावट नहीं चाहिए, पर
    //    गाड़ियों की असली photo सबसे ज़्यादा यहीं चाहिए। पहले यह type
    //    इस file तक पहुँचता ही नहीं था, इसलिए सादा text वाला poster बनता था।
    vigyapan: {
      label: "विज्ञापन", head: "#0B7A3B", title: "#111111",
      bg1: "#FFFFFF", bg2: "#F1F4F7", gold: "#C9A227",
      motifs: ["sparkle"],
    },
  };

  const themeFor = (name = "", type = "") => {
    if (type === "vigyapan" || type === "suchna") return "vigyapan";
  ...
  /**
   * ⚠️ विज्ञापन का अपना layout — बाएँ लिखाई, दाएँ गाड़ी। त्यौहार वाला
   *    "सब कुछ बीच में" layout विज्ञापन पर लगाने से गाड़ी और शीर्षक एक
   *    दूसरे पर चढ़ जाते थे। यह उसी गड़बड़ी का हल है।
   */
  function buildAdSVG(W, H, o) {
    const t = THEMES.vigyapan;
    let s = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">`;
    s += marbleBg(W, H, t);
    s += frame(W, H, t);
    s += sparkles(W, H, t);

    const T = (x, y, sz, txt, fill, wt, anchor) =>
      `<text x="${x}" y="${Math.round(y)}" text-anchor="${anchor || "start"}" font-family="Noto Sans Devanagari, Mukta, sans-serif" font-size="${Math.round(sz)}" font-weight="${wt || 800}" fill="${fill}">${esc(txt)}</text>`;

    const LX = W * 0.06, LW = W * 0.42;   // बायाँ हिस्सा — लिखाई
    let y = H * 0.12;

    if (o.kicker) { s += T(LX, y, W * 0.038, o.kicker, t.head); y += H * 0.05; }

    // शीर्षक — लम्बा नाम हो तो अपने-आप छोटा
    const titleSz = (o.title || "").length > 14 ? 0.052 : 0.066;
    s += T(LX, y, W * titleSz, o.title || "नई गाड़ी", t.title, 900);
    y += H * 0.075;
    s += `<rect x="${LX}" y="${y - H * 0.01}" width="${W * 0.18}" height="6" fill="${t.head}"/>`;
    y += H * 0.05;

    if (o.sub) { s += T(LX, y, W * 0.032, o.sub, "#444", 700); y += H * 0.05; }

    // ── असली फ़ीचर — Vehicle.features से ────────────────────────
    if (o.features?.length) {
      for (const f of o.features) {
        s += `<circle cx="${LX + 8}" cy="${y - 8}" r="5" fill="${t.head}"/>`;
        s += T(LX + 24, y, W * 0.026, f, "#222", 700);
        y += H * 0.042;
      }
      y += H * 0.02;
    }

    // ── क़ीमत / EMI — सिर्फ़ अगर database में भरी हो ────────────
    if (o.price || o.emi) {
      const boxY = y, boxH = H * 0.09;
      s += `<rect x="${LX}" y="${boxY}" width="${LW}" height="${boxH}" rx="14" fill="${t.head}" opacity="0.08"/>`;
      s += `<rect x="${LX}" y="${boxY}" width="6" height="${boxH}" fill="${t.head}"/>`;
      let py = boxY + boxH * 0.38;
      if (o.price) { s += T(LX + 20, py, W * 0.028, `एक्स-शोरूम ${o.price}`, "#222", 800); py += H * 0.038; }
      if (o.emi)   { s += T(LX + 20, py, W * 0.028, `EMI ${o.emi}`, t.head, 800); }
      y = boxY + boxH + H * 0.04;
    }

    if (o.line1) { s += T(LX, y, W * 0.034, o.line1, t.title, 900); y += H * 0.04; }
    if (o.line2) { s += T(LX, y, W * 0.024, o.line2, "#555", 700); }
  ...
  function buildFestiveSVG(W, H, o) {
    ...
    let s = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">`;
    // ⚠️ AI-पृष्ठभूमि इस्तेमाल हो रही हो तो अपना रंगीन background मत बनाओ
    //    (transparent रहे) — नीचे AI photo, ऊपर सिर्फ़ frame+टेक्स्ट+motifs
    if (!o.skipBg) s += marbleBg(W, H, t);
    s += frame(W, H, t);
  ...
  async function vehicleRow(brandId, W, H, maxN) {
    ...
    const n = rows.length;
    //  ⚠️ पहले गाड़ियाँ छोटी (0.20H) और ऊपर (0.34H) थीं — नीचे आधा poster
    //     बिल्कुल ख़ाली रह जाता था (screenshot में साफ़ दिखा)। demo वाले
    //     पेशेवर poster में गाड़ियाँ बड़ी होकर नीचे, ज़मीन पर खड़ी दिखती
    //     हैं। अब वही — बड़ी (0.30H) और नीचे (0.46H से), ताकि poster
    //     भरा-भरा और पेशेवर लगे।
    const bandW = W * 0.94, slot = bandW / n;
    const vh = Math.round(H * 0.30);
    const rowTop = H * 0.46;
    const parts = [];
    ...
    for (let i = 0; i < n; i++) {
      try {
        ...
        //  ⚠️ बड़ी गड़बड़ी पकड़ी — यहाँ कच्ची फ़ोटो सीधे चिपकाई जाती थी, सफ़ेद
        //     background हटाए बिना। इसलिए हर गाड़ी एक सफ़ेद/चेकदार डिब्बे
        //     में चिपकी दिखती थी (आपके screenshot में साफ़ दिखा), जबकि
        //     विज्ञापन वाले poster में removeWhiteBg से साफ़ कटी हुई आती है।
        //     अब त्यौहार वाले poster में भी वही सफ़ाई।
        let img;
        if (removeWhiteBg) {
          try { img = await removeWhiteBg(buf, Math.round(slot * 0.94), vh); } catch (_) { img = null; }
        }
        if (!img) img = await sharp(buf).resize(Math.round(slot * 0.94), vh, { fit: "inside", withoutEnlargement: false }).png().toBuffer();
        const meta = await sharp(img).metadata();
        const left = Math.round(W * 0.03 + i * slot + (slot - (meta.width || slot)) / 2);
        const top = Math.round(rowTop + (vh - (meta.height || vh)));
    ...
    return { parts, count: n, top: rowTop, height: vh };
  }

  /**
   * ⚠️ विज्ञापन के लिए अलग function — यही असली गड़बड़ी की जड़ थी।
   *    पहले हर type (त्यौहार हो या विज्ञापन) एक ही vehicleRow() से गुज़रता
   *    था, जो हमेशा 4 गाड़ियों की छोटी कतार बना देता था — भले टाइटल में
   *    एक ही गाड़ी का नाम क्यों न लिखा हो। "Livo 110 DLX" शीर्षक के नीचे
   *    4 अलग-अलग बेमेल गाड़ियाँ इसीलिए दिख रही थीं।
   *
   *    विज्ञापन में असली सोच अलग है: एक गाड़ी, बड़ी करके, दाईं तरफ़; और
   *    उसकी असली specs (Digital Meter, Tubeless Tyre वग़ैरह) फ़ीचर की
   *    तरह — Vehicle.features में यह data पहले से था, बस इस्तेमाल नहीं
   *    होता था।
   */
  async function fetchOneVehicle(brandId, hint) {
    const q = { brand: brandId, active: true, inStock: true, photoData: { $nin: [null, ""] } };
    let v = null;
    if (hint) {
      v = await Vehicle.findOne({ ...q, name: { $regex: hint, $options: "i" } })
        .select("name variant photoData exShowroom onRoad downPayment emi features engine mileage")
        .sort({ updatedAt: -1 }).lean();
    }
    if (!v) {
      v = await Vehicle.findOne(q)
        .select("name variant photoData exShowroom onRoad downPayment emi features engine mileage")
        .sort({ updatedAt: -1 }).lean();
    }
    // ⚠️ यहाँ भी वही सुरक्षा — features कम/ख़ाली हों तो AI ख़ुद गाड़ी के
    //    नाम से पहचान कर बना दे, और डेटाबेस में हमेशा के लिए सेव हो जाए
    if (v && (!v.features || v.features.length < 3) && inferVehicleFeatures) {
      const inferred = await inferVehicleFeatures(brandId, [v.name, v.variant].filter(Boolean).join(" "));
      if (inferred.length) {
        v.features = inferred;
        Vehicle.findByIdAndUpdate(v._id, { features: inferred }).catch(() => {});
      }
    }
    return v;
  }

  /** एक बड़ी गाड़ी, दाईं तरफ़, ज़मीन की परछाईं के साथ */
  async function singleVehicleImage(v, W, H) {
    if (!v?.photoData) return null;
    const buf = Buffer.from(v.photoData, "base64");
    const bw = Math.round(W * 0.46), bh = Math.round(H * 0.34);
    const img = await sharp(buf).resize(bw, bh, { fit: "inside", withoutEnlargement: false }).png().toBuffer();
    const meta = await sharp(img).metadata();
    const left = Math.round(W * 0.50 + (bw - (meta.width || bw)) / 2);
    const top = Math.round(H * 0.30);
    const shW = Math.round((meta.width || bw) * 0.75), shH = Math.max(10, Math.round(bh * 0.06));
    const shadow = await sharp({ create: { width: shW, height: shH, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0.25 } } })
      .png().blur(7).toBuffer();
    return {
      count: 1, top: H * 0.30, height: bh,
      parts: [
        { input: shadow, top: top + (meta.height || bh) - Math.round(shH / 2), left: left + Math.round(((meta.width || bw) - shW) / 2) },
        { input: img, top, left },
      ],
    };
  }
   ...
   *               ad: true पर विज्ञापन वाला layout — एक गाड़ी + फ़ीचर }
   */
  //  ⚠️ हर थीम के लिए सम्मानजनक prompt — जान-बूझकर देवी-देवता की आकृति
  //     बनवाने को नहीं कहा (AI कभी-कभी अनुचित/ग़लत अनुपात बना देता है, धार्मिक
  //     तस्वीर में यह असम्मानजनक लगेगा) — सिर्फ़ त्यौहार का माहौल-सजावट।
  const AI_BG_PROMPT = {
    ganesh: "Ganesh Chaturthi festival decoration backdrop, marigold flower garlands, brass bells, diya oil lamps, golden rangoli pattern, warm festive lighting, elegant Indian traditional festive background, no human figures, no deity idol, no statue",
    navratri: "Navratri festival decoration backdrop, colorful dandiya sticks pattern, marigold flowers, diya lamps, vibrant festive Indian background, no human figures, no deity idol, no statue",
    diwali: "Diwali festival decoration backdrop, diya oil lamps, rangoli pattern, golden fairy lights, marigold flowers, warm festive Indian background, no human figures, no deity idol, no statue",
  };
  ...
  async function buildFestivePoster(id, opts) {
    ...
    //  ⚠️ "AI पृष्ठभूमि" — सिर्फ़ तभी आज़माते हैं जब आपने ख़ुद कोई तस्वीर
    //     सेटिंग से नहीं चढ़ाई (art ना हो), और आपने साफ़ तौर पर यह विकल्प
    //     माँगा हो (opts.tryAIBg) — अपने-आप कभी नहीं चलता, ताकि बिना
    //     जाँचे कोई अजीब तस्वीर poster पर ना चढ़ जाए।
    const canTryAI = opts.tryAIBg && !art && fetchAIBackground && AI_BG_PROMPT[theme];

    const sizes = opts.sizes || { square: [1080, 1080], story: [1080, 1920], landscape: [1200, 630] };
    const out = {}; const b64 = {};
    let vehCount = 0;      // कितनी गाड़ियों की photo सचमुच लगी

    //  ⚠️ विज्ञापन में एक बार गाड़ी ढूँढ लो — तीनों नाप (square/story/
    //     landscape) में वही एक गाड़ी और उसके असली specs इस्तेमाल होंगे।
    const isAd = theme === "vigyapan";
    const oneVeh = isAd ? await fetchOneVehicle(brandId, opts.title) : null;
    if (isAd && oneVeh) {
      // असली नाम — अंदाज़े का "Livo 110 DLX" नहीं, database का सही नाम
      opts = { ...opts, title: [oneVeh.name, oneVeh.variant].filter(Boolean).join(" ") || opts.title };
    }

    for (const [key, [W, H]] of Object.entries(sizes)) {
      const veh = isAd
        ? (await singleVehicleImage(oneVeh, W, H)) || { parts: [], count: 0, top: H * 0.34, height: H * 0.20 }
        : await vehicleRow(brandId, W, H, W > H ? 5 : 4);
      vehCount = Math.max(vehCount, veh.count || 0);

      const svgOpts = {
        theme, hasArt: !!art,
        kicker: opts.kicker, title: opts.title, sub: opts.sub,
        line1: opts.line1, line2: opts.line2, line3: opts.line3,
        // ⚠️ पहले 4 तक सीमित था — features अब AI से भी बन सकते हैं (देखें
        //    fetchOneVehicle), तो थोड़ा बढ़ाया; यह layout सूची-रूप में है
        //    (नीचे तक बढ़ती है), इसलिए 9 तक नहीं — बाक़ी elements से टकरा
        //    सकता है, 6 सुरक्षित है
        features: isAd ? (oneVeh?.features || []).slice(0, 6) : [],
        price: isAd ? (oneVeh?.exShowroom || "") : "",
        emi: isAd ? (oneVeh?.emi || "") : "",
        vehTop: veh.top || H * 0.34, vehH: veh.height || H * 0.20,
      };
      // ⚠️ AI-पृष्ठभूमि — पहले कोशिश करके देख लो, कामयाब हो तो नीचे असली
      //    photo, ऊपर सिर्फ़ पारदर्शी SVG (frame+टेक्स्ट); fail हो तो
      //    पुराने रंग-आधारित background पर अपने-आप वापस।
      let aiBg = null;
      if (canTryAI) {
        try { aiBg = await fetchAIBackground(brandId, { aiPrompt: AI_BG_PROMPT[theme] }, W, H); }
        catch (_) { aiBg = null; }
      }
      const svgOptsFinal = { ...svgOpts, skipBg: !!aiBg };
      // ⚠️ विज्ञापन का अपना layout — त्यौहार वाला बीच-में-सब-कुछ यहाँ
      //    नहीं चलता, वरना गाड़ी और शीर्षक टकरा जाते हैं (जो पहले हो रहा था)
      let svg = isAd ? buildAdSVG(W, H, svgOpts) : buildFestiveSVG(W, H, svgOptsFinal);

      let base = aiBg
        ? await sharp(aiBg).composite([{ input: await sharp(Buffer.from(svg)).png().toBuffer() }]).png().toBuffer()
    ...
    out._b64 = b64;
    //  ⚠️ पहले यहाँ हमेशा 0 लिखा जाता था — log से पता ही नहीं चलता था कि
    //     गाड़ी की photo लगी या नहीं। अब असली गिनती जाती है।
    out._meta = {
      theme, vehicles: vehCount, hasArt: !!art,
      warn: vehCount === 0
        ? "किसी गाड़ी की photo सेव नहीं — poster में गाड़ियाँ नहीं आईं"
        : "",
    };
    return out;
  }
