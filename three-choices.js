...
module.exports = function mountThreeChoices(app, deps) {
  ...
  async function build(ch) {
    ...
    for (let i = 0; i < TONES.length; i++) {
      ...
      try {
        ...
        if ((ch.type || "") === "suvichar" && stripBrandFromPoem) text = stripBrandFromPoem(text, ch.brand);
  ...
  app.post("/api/three/make", requireRole("super-admin", "admin", "manager"), async (req, res) => {
    try {
      ...
      // ⚠️ उपयोगकर्ता dropdown से प्रकार चुन ले तो AI की अटकल मत मानो
  ...
  app.post("/api/three/:id/pick", requireRole("super-admin", "admin", "manager"), async (req, res) => {
    try {
      ...
      const doc = await Content.create({
        ...
        text: pick.text,
        whatsappText: toWhatsAppMarkdown ? toWhatsAppMarkdown(pick.text, c.brand) : undefined,
