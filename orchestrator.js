...
const CampaignSchema = new mongoose.Schema(
  {
    ...
    vehicle: String,
    customerName: String,
    deliveryId: { type: mongoose.Schema.Types.ObjectId },   // अधूरी delivery, photo का इंतज़ार
    offerDetails: String,

    // हालत
    status: {
      type: String,
      // needs_photo = सब तैयार है, बस ग्राहक की photo बाक़ी है
      enum: ["running", "review", "scheduled", "published", "failed", "blocked", "needs_photo"],
      default: "running",
      index: true,
    },
 ...
 */
/**
 * क्या यह delivery वाली post है?
 * ग्राहक की तस्वीर वाली post — बोलकर बनाई ही नहीं जा सकती, photo चाहिए।
 */
function isDeliveryPost(camp, text) {
  if (camp.type === "delivery") return true;
  const hay = ((camp.command || "") + " " + (camp.intent || "") + " " + (text || "")).toLowerCase();
  // "डिलीवरी", "delivery", "गाड़ी दी", "चाबी सौंपी", "नई गाड़ी मुबारक"
  return /डिलीवरी|डिलिवरी|delivery|चाबी\s*(सौंप|दे)|गाड़ी\s*(दी|सौंपी)|नई\s*गाड़ी\s*मुबारक|बधाई.*नई\s*गाड़ी/.test(hay);
}
...
module.exports = function mountOrchestrator(app, deps) {
  ...
  async function runPipeline(campId, req) {
    ...
    try {
      ...
      //    सुविचार के भीतर ही घुसा देता है — यहीं काट देते हैं
      if (camp.type === "suvichar" && deps.stripBrandFromPoem) text = deps.stripBrandFromPoem(text, brandId);
      ...
      const doc = await Content.create({
        ...
        type: camp.type,
        text,
        whatsappText: deps.toWhatsAppMarkdown ? deps.toWhatsAppMarkdown(text, brandId) : undefined,
        status: "pending",
      });
      ...
      // ══════════════════════════════════════════════════════════════════
      //  ⚠️ DELIVERY वाली post — यहाँ poster नहीं बनना चाहिए
      // ------------------------------------------------------------------
      //  पहले क्या होता था: बोलकर "डिलीवरी post बनाओ" कहने पर यह रास्ता
      //  generateImages() बुला देता था, जो सिर्फ़ text का poster बनाता है।
      //  नतीजा — बीच में ख़ाली धारीदार डिब्बा और उसमें लिखा "डिलीवरी फोटो"।
      //  ग्राहक की असली तस्वीर कहीं आती ही नहीं थी।
      //
      //  असली delivery poster buildDeliverySlides() बनाता है, और उसे
      //  ग्राहक की photo चाहिए। photo बोलकर नहीं दी जा सकती।
      //
      //  इसलिए अब: बेकार poster बनाने के बजाय एक अधूरी Delivery बना देते हैं
      //  और साफ़ कहते हैं कि बस photo डाल दीजिए। नाम, गाड़ी, ऑफ़र — जो AI ने
      //  समझा वो पहले से भरा मिलेगा, दोबारा टाइप नहीं करना पड़ेगा।
      if (isDeliveryPost(camp, text)) {
        try {
          const d = await Delivery.create({
            brand: brandId,
            customerName: camp.customerName || camp.customer || "",
            bikeName: camp.vehicle || "",
            offer: camp.offer || "",
            text,
            status: "pending",
          });
          camp.deliveryId = d._id;
          camp.markModified("assets");
          await camp.save();
          await mark(camp, "poster", "skip", "delivery — ग्राहक की photo चाहिए");
        } catch (e) {
          L("ERROR", "[CAMPAIGN] delivery draft नहीं बना", { msg: e.message });
        }
        // अधूरा content हटा दो — वरना "आज" में ख़ाली डिब्बे वाला poster दिखेगा
        try { await Content.findByIdAndDelete(doc._id); } catch (_) {}
        camp.contentId = null;
        await camp.save();

        return await stop(camp,
          "डिलीवरी post के लिए ग्राहक की photo चाहिए — वो बोलकर नहीं दी जा सकती। " +
          "बनाओ → 🎥 Delivery post में जाइए, वहाँ नाम और गाड़ी पहले से भरी मिलेगी, बस photo डाल दीजिए।",
          "needs_photo"
        );
      }

      try {
        // ⚠️ text को ऊपर verifyAgainstTruth() से पक्का किया जा चुका है कि वह
        //    camp.vehicle की सच्ची जानकारी बोल रहा है। पर generateImages को
        //    खाली {} दिया जाता था — वो अपने-आप कोई और गाड़ी चुन लेता (सिर्फ़
        //    text का पहला शब्द देखकर अंदाज़ा), तो caption किसी गाड़ी की बात
        //    करता, poster पर कोई और होती। camp.vehicle सीधे भेजकर यह रोकते हैं।
  ...
  /**
   * जिन delivery posts में सिर्फ़ photo बाक़ी है।
   * "आज" वाले पर्दे पर यही सूची दिखेगी — नाम और गाड़ी पहले से भरी,
   * बस ग्राहक की तस्वीर डालनी है।
   */
  app.get("/api/campaign/needs-photo", async (req, res) => {
    try {
      const q = { status: "needs_photo" };
      if (BRANDS[req.query.brand]) q.brand = req.query.brand;
      const camps = await Campaign.find(q).sort({ createdAt: -1 }).limit(30).lean();

      const ids = camps.map((c) => c.deliveryId).filter(Boolean);
      const dels = await Delivery.find({ _id: { $in: ids }, photo: { $in: [null, ""] } })
        .select("customerName bikeName offer text status").lean();
      const dmap = Object.fromEntries(dels.map((d) => [String(d._id), d]));

      const rows = camps
        .filter((c) => c.deliveryId && dmap[String(c.deliveryId)])
        .map((c) => {
          const d = dmap[String(c.deliveryId)];
          return {
            campaignId: String(c._id),
            deliveryId: String(c.deliveryId),
            brand: c.brand,
      ...
      res.json({
        ...
        note: rows.length
          ? "इनमें बस ग्राहक की photo डालनी है — बाक़ी सब भरा हुआ है"
          : "",
      });
