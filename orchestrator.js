...
module.exports = function mountOrchestrator(app, deps) {
  ...
  async function runPipeline(campId, req) {
    ...
    try {
      ...
      const needsMoney = camp.type === "vigyapan" || camp.type === "gift" || !!camp.vehicle;
      let vCtx = null;
      if (needsMoney) {
        ...
        await mark(camp, "truthload", "ok", vCtx.split("\n")[0]);
      } else {
  ...
  app.get("/api/today", async (req, res) => {
    try {
      ...
      const [pendingContent, pendingDeliv, newLeads, unread, liveCamps, recentCamps, sentToday, failedToday] = await Promise.all([
        ...
        Notification ? Notification.countDocuments({ read: { $ne: true } }).catch(() => 0) : 0,
        Campaign.find({ brand, status: "running" }).sort({ createdAt: -1 }).limit(5).select("-intent -truth").lean().catch(() => []),
        Campaign.find({ brand, createdAt: { $gte: since } }).sort({ createdAt: -1 }).limit(10).select("-intent -truth -assets").lean().catch(() => []),
        Content.countDocuments({ brand, status: "sent", sentAt: { $gte: new Date(new Date().setHours(0, 0, 0, 0)) } }).catch(() => 0),
        Content.countDocuments({ brand, status: "failed" }).catch(() => 0),
      ]);
      ...
      res.json({
        ...
        blocked: blocked.map((c) => ({ _id: c._id, reason: c.stopReason, vehicle: c.vehicle, at: c.createdAt })),
      });
    } catch (e) {
