...
function buildSVG(brandId, text, w, h, type, opts) {
  ...
  return Buffer.from(`<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${opts && opts.sticker ? stickersSVG(opts.sticker, opts.offer, {}, w, h, [[0.80, 0.30], [0.20, 0.30], [0.80, 0.52]], w * 0.12) : ""}
    ${opts && opts.decor ? decorSVG(opts.decor, w, h, w * 0.05) : ""}</svg>`);
}
...
function decorSVG(decorCsv, w, h, r) {
  let names = String(decorCsv || "").split(",").map((s) => s.trim()).filter((s) => DECOR_NAMES.includes(s));
  if (!names.length) return "";
  const pos = [[0.09, 0.20], [0.91, 0.20], [0.09, 0.52], [0.91, 0.52], [0.50, 0.155], [0.50, 0.55]];
...
app.get("/api/health", (req, res) => res.json({ ok: true, version: "v14-decor-multi", testMode: TEST_MODE, video: ENABLE_VIDEO, cron: ENABLE_CRON ? CRON_SCHEDULE : false, brands: Object.keys(BRANDS) }));
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const opts = { sticker: req.body.sticker || "", offer: req.body.offer || "", decor: req.body.decor || "" };
